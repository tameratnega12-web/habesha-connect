-- Habesha Agenagn V7.8.321 — TRUE RENTALS-ONLY SCHEMA RESET
-- WARNING: Deletes only the new rental_properties and rental_requests tables and their data.

create extension if not exists pgcrypto;

-- Remove only the rebuilt Rental objects so no old column layout survives.
drop function if exists public.admin_decide_rental_property_v319(uuid,boolean);
drop function if exists public.release_rental_contact_v319(uuid);
drop function if exists public.decide_rental_request_v319(uuid,text);
drop function if exists public.create_rental_request_v319(uuid,text);
drop function if exists public.attach_rental_photos_v319(uuid,text[]);
drop function if exists public.create_rental_property_v319(text,text,numeric,text,text,text);
drop function if exists public.current_profile_id_v319();
drop function if exists public.is_admin_v319();
...
create policy rental_properties_read_v321 on public.rental_properties for select to authenticated using (
 status='approved'
 or owner_profile_id in (select id from public.profiles where auth_user_id=auth.uid())
 or exists(select 1 from public.profiles where auth_user_id=auth.uid() and role='admin')
);

create policy rental_requests_read_v321 on public.rental_requests for select to authenticated using (
 owner_profile_id in (select id from public.profiles where auth_user_id=auth.uid())
 or seeker_profile_id in (select id from public.profiles where auth_user_id=auth.uid())
 or exists(select 1 from public.profiles where auth_user_id=auth.uid() and role='admin')
);
...
drop policy if exists rental_photos_public_read_v321 on storage.objects;
drop policy if exists rental_photos_owner_insert_v321 on storage.objects;

create policy rental_photos_public_read_v321 on storage.objects for select using (bucket_id='rental-photos');
create policy rental_photos_owner_insert_v321 on storage.objects for insert to authenticated with check (
 bucket_id='rental-photos'
 and (storage.foldername(name))[1] in (select id::text from public.profiles where auth_user_id=auth.uid())
);

create or replace function public.current_profile_id_v319() returns uuid
language sql stable security definer set search_path=public
as $$select id from public.profiles where auth_user_id=auth.uid() limit 1$$;

create or replace function public.is_admin_v319() returns boolean
language sql stable security definer set search_path=public
as $$select exists(select 1 from public.profiles where auth_user_id=auth.uid() and role='admin')$$;

create or replace function public.create_rental_property_v319(
 p_title text,p_city text,p_monthly_rent numeric,p_property_type text,p_address text,p_description text
) returns uuid language plpgsql security definer set search_path=public as $$
declare v_profile uuid:=public.current_profile_id_v319(); v_id uuid;
begin
...
begin
 ...
 where id=p_property_id and owner_profile_id=public.current_profile_id_v319();
 if not found then raise exception 'Property not found or not owned by you'; end if;
end$$;

create or replace function public.create_rental_request_v319(p_property_id uuid,p_message text default null)
returns uuid language plpgsql security definer set search_path=public as $$
declare v_seeker uuid:=public.current_profile_id_v319(); v_owner uuid; v_name text; v_id uuid;
begin
 if v_seeker is null then raise exception 'Profile not found'; end if;
 ...
 values(p_property_id,v_owner,v_seeker,v_name,nullif(trim(p_message),''))
...
returns void language plpgsql security definer set search_path=public as $$
declare v_owner uuid:=public.current_profile_id_v319(); v_property uuid;
begin
...
declare v_seeker uuid:=public.current_profile_id_v319(); v_owner uuid; v_result jsonb;
begin
...
returns void language plpgsql security definer set search_path=public as $$
begin
 if not public.is_admin_v319() then raise exception 'Admin access required'; end if;
