-- Habesha Connect Marketplace SQL
-- Run in Supabase SQL Editor. Do not delete old tables.
...
create policy "Sellers can create marketplace listings"
...
create policy "Sellers can view their own marketplace listings"
...
create policy "Everyone can view approved marketplace listings"
on marketplace_listings for select to authenticated
...
create policy "Sellers can update their own marketplace listings"
...
create policy "Buyers can create marketplace purchase requests"
...
create policy "Buyers can view their own marketplace requests"
...
drop policy if exists "Users can save marketplace items" on marketplace_saved_items;
create policy "Users can save marketplace items"
on marketplace_saved_items for insert to authenticated
with check (auth.uid() = user_id);

drop policy if exists "Users can view their saved marketplace items" on marketplace_saved_items;
create policy "Users can view their saved marketplace items"
on marketplace_saved_items for select to authenticated
using (auth.uid() = user_id);

drop policy if exists "Users can remove their saved marketplace items" on marketplace_saved_items;
create policy "Users can remove their saved marketplace items"
on marketplace_saved_items for delete to authenticated
using (auth.uid() = user_id);
