...
function marketStatusToUi(status){
  const map={pending_admin:'Pending Admin Approval',admin_approved:'Approved',admin_declined:'Declined',sold_waiting_admin:'Sold Waiting Admin Verification',sold_admin_approved:'Sold Verified',removed:'Removed'};
  return map[status]||status||'Pending Admin Approval';
}
function marketStatusToDb(status){
  const map={'Pending Admin Approval':'pending_admin','Approved':'admin_approved','Available':'admin_approved','Declined':'admin_declined','Sold Waiting Admin Verification':'sold_waiting_admin','Sold Verified':'sold_admin_approved','Removed':'removed'};
  return map[status]||status||'pending_admin';
}
function marketReqStatusToUi(status){
  ...
  return map[status]||status||'Buyer Request Pending Admin';
}
function marketReqStatusToDb(status){
...
function mapDbMarketListing(row){
  return {id:row.id,dbId:row.id,source:'supabase',title:row.title||'Marketplace Item',category:row.category||'Other',condition:row.item_condition||row.condition||'Good',price:Number(row.price||0),city:row.city||'',description:row.description||'',photos:Array.isArray(row.photo_urls)?row.photo_urls:[],seller:row.seller_name||'Seller',sellerEmail:row.seller_email||'',sellerPhone:row.seller_phone||'',status:marketStatusToUi(row.status),createdAt:row.created_at?new Date(row.created_at).toLocaleString():''};
}
function mapDbMarketRequest(row){
  return {id:row.id,dbId:row.id,source:'supabase',itemId:row.listing_id,itemTitle:row.item_title||row.listing_title||'Marketplace Item',buyerName:row.buyer_name||'Buyer',buyerEmail:row.buyer_email||'',buyerPhone:row.buyer_phone||'',sellerName:row.seller_name||'Seller',sellerEmail:row.seller_email||'',sellerPhone:row.seller_phone||'',message:row.message||'',status:marketReqStatusToUi(row.status),createdAt:row.created_at?new Date(row.created_at).toLocaleString():''};
}
async function loadSupabaseMarketplace(){
  if(!authReady())return;
  try{
    let {data:listings,error}=await hcSupabase.from('marketplace_listings').select('*').order('created_at',{ascending:false});
    if(error){console.warn('Marketplace listings load error',error);return;}
    data.market=(listings||[]).map(mapDbMarketListing);
    let {data:reqs,error:reqErr}=await hcSupabase.from('marketplace_purchase_requests').select('*').order('created_at',{ascending:false});
    if(!reqErr){data.marketRequests=(reqs||[]).map(mapDbMarketRequest);}else{console.warn('Marketplace requests load error',reqErr);}
    persistOnly();
  }catch(e){console.warn('Marketplace Supabase load failed',e);}
}
async function updateMarketplaceListingDb(item,fields){
  if(!authReady()||!item.dbId)return {error:null};
  return await hcSupabase.from('marketplace_listings').update(fields).eq('id',item.dbId);
}
async function updateMarketplaceRequestDb(req,fields){
...
function marketplace(mode){
 ...
 <div class="grid two">
  <div class="card" id="marketPostPanel"><h3>➕ Post Item</h3>
   ...
   <p class="small">Admin must approve the item before buyers can see it.</p>
   <button class="btn primary" onclick="addMarket()">Submit Item for Admin Approval</button>
  </div>
  <div class="card" id="marketBrowsePanel"><h3>🔍 Browse Marketplace</h3><p class="muted">Only admin-approved available items show to buyers.</p>
...
async function addMarket(){
 ...
 if(authReady()){
   let payload={seller_id:currentUser.auth_user_id||currentUser.id||null,seller_name:item.seller,seller_email:item.sellerEmail,seller_phone:item.sellerPhone,title:item.title,description:item.description,category:item.category,price:item.price,item_condition:item.condition,city:item.city,photo_urls:item.photos,status:marketStatusToDb(item.status)};
   let {data:row,error}=await hcSupabase.from('marketplace_listings').insert(payload).select('*').single();
   if(error)return alert('Marketplace item was not saved to Supabase. Please run supabase/v7_8_37_marketplace_flow_fix.sql, then try again. Error: '+error.message);
   item=mapDbMarketListing(row);
 }
...
async function marketRequestBuy(id){
 ...
 let r={id:'MR'+Date.now().toString().slice(-5),itemId:m.id,itemTitle:m.title,buyerName:currentUser.name,buyerEmail:currentUser.email,buyerPhone:currentUser.phone||'',sellerName:m.seller,sellerEmail:m.sellerEmail,sellerPhone:m.sellerPhone||'',status:'Buyer Request Pending Admin',createdAt:new Date().toLocaleString()};
 if(authReady()){
   let payload={listing_id:m.dbId||m.id,buyer_id:currentUser.auth_user_id||currentUser.id||null,buyer_name:r.buyerName,buyer_email:r.buyerEmail,buyer_phone:r.buyerPhone,seller_name:r.sellerName,seller_email:r.sellerEmail,seller_phone:r.sellerPhone,item_title:r.itemTitle,message:'',status:marketReqStatusToDb(r.status)};
   let {data:row,error}=await hcSupabase.from('marketplace_purchase_requests').insert(payload).select('*').single();
   if(error)return alert('Buyer request was not saved to Supabase. Please run supabase/v7_8_37_marketplace_flow_fix.sql, then try again. Error: '+error.message);
   r=mapDbMarketRequest(row);
 }
...
async function marketAdminApproveBuyerReq(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=data.marketRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!r)return;r.status='Admin Approved - Waiting Seller';let res=await updateMarketplaceRequestDb(r,{status:'admin_approved'});if(res.error)return alert('Could not approve buyer request: '+res.error.message);addNote(r.sellerEmail,'Marketplace buyer request approved by admin. Please accept or decline: '+r.itemTitle);addNote(r.buyerEmail,'Your marketplace request is approved by admin and waiting for seller response: '+r.itemTitle);await marketRefreshAfterAction();}
async function marketAdminDeclineBuyerReq(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=data.marketRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!r)return;r.status='Admin Declined';let res=await updateMarketplaceRequestDb(r,{status:'admin_declined'});if(res.error)return alert('Could not decline buyer request: '+res.error.message);addNote(r.buyerEmail,'Your marketplace buyer request was declined: '+r.itemTitle);await marketRefreshAfterAction();}
async function marketSellerAcceptReq(id){let r=data.marketRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!r||!currentUser||r.sellerEmail!==currentUser.email)return alert('Seller only.');r.status='Seller Accepted - Waiting Admin';let res=await updateMarketplaceRequestDb(r,{status:'seller_accepted_waiting_admin'});if(res.error)return alert('Could not accept request: '+res.error.message);addNote('admin.habeshaconnect@gmail.com','Marketplace seller accepted buyer request. Admin connection approval needed: '+r.itemTitle);if(typeof sendAdminEmailNotice==='function')sendAdminEmailNotice('Marketplace seller accepted request','Seller accepted a buyer request and admin approval is needed before contact sharing.',{Seller:r.sellerName,Buyer:r.buyerName,Item:r.itemTitle,Status:r.status},'marketplace');await marketRefreshAfterAction();}
async function marketSellerDeclineReq(id){let r=data.marketRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!r||!currentUser||r.sellerEmail!==currentUser.email)return alert('Seller only.');r.status='Seller Declined';let res=await updateMarketplaceRequestDb(r,{status:'seller_declined'});if(res.error)return alert('Could not decline request: '+res.error.message);addNote(r.buyerEmail,'Seller declined your marketplace request: '+r.itemTitle);await marketRefreshAfterAction();}
async function marketAdminApproveConnection(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=data.marketRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!r)return;r.status='Connection Approved';let res=await updateMarketplaceRequestDb(r,{status:'connection_admin_approved'});if(res.error)return alert('Could not approve connection: '+res.error.message);addNote(r.buyerEmail,'Marketplace connection approved. Seller contact is now available for '+r.itemTitle);addNote(r.sellerEmail,'Marketplace connection approved. Buyer contact is now available for '+r.itemTitle);await marketRefreshAfterAction();}
async function marketMarkSold(id){let m=data.market.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!m||!currentUser||m.sellerEmail!==currentUser.email)return alert('Seller only.');m.status='Sold Waiting Admin Verification';let r=await updateMarketplaceListingDb(m,{status:'sold_waiting_admin'});if(r.error)return alert('Could not mark sold: '+r.error.message);addNote('admin.habeshaconnect@gmail.com','Marketplace item marked sold and waiting for verification: '+m.title);if(typeof sendAdminEmailNotice==='function')sendAdminEmailNotice('Marketplace sold item verification needed','Seller marked item as sold. Admin verification is needed.',{Seller:m.seller,Item:m.title,Price:money(m.price),Status:m.status},'marketplace');await marketRefreshAfterAction();}
async function marketAdminVerifySale(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let m=data.market.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!m)return;m.status='Sold Verified';let r=await updateMarketplaceListingDb(m,{status:'sold_admin_approved'});if(r.error)return alert('Could not verify sale: '+r.error.message);addNote(m.sellerEmail,'Marketplace sold item verified: '+m.title);await marketRefreshAfterAction();}
...
/* V7.8.32 Taxi/Limo Supabase sync helpers - removes local/demo-only flow */
...
/* V7.8.39 Global button response safety patch
   Fixes stuck buttons after sign-in by never waiting for slow Supabase calls
   before changing the visible dashboard/navigation. */
(function(){
  const expose=()=>{
    ['show','logout','switchRole','saveRoleChoices','render','services','profile','home','marketplace','jobs','truck','rentals','shipping','admin'].forEach(k=>{
      try{ if(typeof window[k]==='undefined' && typeof eval(k)==='function') window[k]=eval(k); }catch(e){}
    });
  };
  ...
  if(typeof show==='function'){
    ...
    let hcNavToken=0;
    show=async function(page){
      hcNavToken++;
      const token=hcNavToken;
      // Always unlock navigation before a new click. This prevents one slow render
      // from freezing logout, My Services, role switching, or mobile menu taps.
      renderingPage=false;
      try{
        return await originalShow(page);
      }catch(err){
        console.error('Safe show error',err);
        renderingPage=false;
        try{
          const fallback=currentUser?'services':'account';
          pages.forEach(x=>$(x)?.classList.add('hide'));
          $(fallback)?.classList.remove('hide');
          if($(fallback)) $(fallback).innerHTML='<div class="card"><h2>Page loading problem</h2><p>Please refresh once, or use My Services to continue.</p><button class="btn primary" onclick="location.reload()">Refresh App</button></div>';
        }catch(e){}
      }finally{
        if(token===hcNavToken) renderingPage=false;
      }
    };
  }

  // Logout must work even if Supabase signOut is slow/unavailable.
  logout=function(){
    const oldEmail=currentUser&&currentUser.email;
    renderingPage=false;
    currentUser=null;
    try{ data.currentUser=null; localStorage.setItem('hc_v35',JSON.stringify(data)); }catch(e){}
    try{ render(); }catch(e){}
    try{ show('home'); }catch(e){}
    if(typeof authReady==='function' && authReady()){
      try{ hcSupabase.auth.signOut().catch(e=>console.warn('Supabase signOut delayed',e)); }catch(e){ console.warn('Supabase signOut error',e); }
    }
    if(oldEmail) console.log('Logged out:',oldEmail);
  };

  // Role/service switching should update the UI first, then sync Supabase in background.
  switchRole=function(role){
    if(!currentUser) return;
    const roles=currentUser.roles||[currentUser.role||'customer'];
    if(!roles.includes(role)) return alert('Please add this service to your profile first.');
    currentUser.role=role;
    currentUser.active_role=role;
    try{ localStorage.setItem('hc_active_role_'+currentUser.email,role); }catch(e){}
    const u=(data.users||[]).find(x=>cleanEmail(x.email)===cleanEmail(currentUser.email));
    if(u){ u.role=role; u.active_role=role; }
    persistOnly();
    renderingPage=false;
    render();
    show(role==='marketplace'?'marketplace':'profile');
    if(typeof authReady==='function' && authReady() && currentUser.id){
      hcSupabase.from('profiles').update({active_role:role,role:role}).eq('id',currentUser.id)
        .then(({error})=>{ if(error) console.warn('Could not update active role',error); })
        .catch(e=>console.warn('Active role sync delayed',e));
    }
  };

  // Save services locally immediately. Sync database after the page responds.
  saveRoleChoices=function(){
    if(!currentUser) return;
    let roles=[...document.querySelectorAll('.svcCheck:checked')].map(x=>x.value);
    if((currentUser.roles||[]).includes('admin')&&!roles.includes('admin')) roles.push('admin');
    if(!roles.length) return alert('Choose at least one service.');
    ...
    currentUser.active_role=currentUser.role;
    const u=(data.users||[]).find(x=>cleanEmail(x.email)===cleanEmail(currentUser.email));
    if(u){ u.roles=currentUser.roles; u.role=currentUser.role; u.active_role=currentUser.role; }
    persistOnly();
    renderingPage=false;
    alert('Your services were saved. You can switch services now.');
    render();
    show('services');
    if(typeof authReady==='function' && authReady()){
      let query=hcSupabase.from('profiles').update({roles:currentUser.roles,active_role:currentUser.role,role:currentUser.role});
      let req=currentUser.id?query.eq('id',currentUser.id):query.eq('email',currentUser.email);
      req.then(({error})=>{ if(error) alert('Services saved on this device, but Supabase did not update: '+error.message); })
         .catch(e=>console.warn('Role sync delayed',e));
    }
  };

  // The older one-click protection was too broad on the Services page. Keep buttons tappable.
  document.addEventListener('click',function(e){
    const btn=e.target.closest('button');
    if(!btn) return;
    if(btn.disabled && /Current Dashboard|Continue as|My Services|Logout|Open|Profile/i.test(btn.textContent||'')){
      btn.disabled=false;
      btn.dataset.busy='0';
      btn.style.opacity='';
      btn.style.cursor='';
  ...
  window.saveRoleChoices=saveRoleChoices;
})();
