...
function sendMessage(){if(!requireLogin())return;let to=$('msgTo').value||'admin.habeshaconnect@gmail.com',txt=$('msgText').value;if(!txt)return alert('Write a message first');data.messages.unshift({from:currentUser.email,to,text:txt,time:new Date().toLocaleString()});addNote(to,'New message from '+currentUser.name);save();show('messages')}
...
async function addRental(){
  ...
  addNote('admin.habeshaconnect@gmail.com','Rental listing from '+currentUser.name+' needs approval.');
...
async function requestViewing(id){
 ...
 addNote('admin.habeshaconnect@gmail.com','New rental viewing request for '+r.title+' from '+currentUser.name+'.');
...
async function declineRentalReq(id){let q=data.rentalRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!q)return;q.status='Declined';let r=data.rentals.find(x=>String(x.id)===String(q.rentalId)||String(x.dbId)===String(q.rentalId));if(r&&r.status==='Viewing Pending')r.status='Approved';if(authReady()&&q.dbId){let {error}=await hcSupabase.from('rental_requests').update({status:'Declined'}).eq('id',q.dbId);if(error)return alert('Could not decline rental request: '+error.message);}addNote(q.seekerEmail,'Your rental request was declined.');persistOnly(); if(currentPage==='admin'){await admin();}else{await rentals();}}
...
function submitReport(){let email=($('rpEmail')?.value||'').trim(),page=$('rpPage')?.value||'Other',text=($('rpText')?.value||'').trim();if(!email||!text)return alert('Please enter your email and what happened.');data.notifications.unshift({to:'admin.habeshaconnect@gmail.com',text:'Problem report from '+email+' on '+page+': '+text,time:new Date().toLocaleString(),read:false});if(currentUser)addNote(currentUser.email,'Thank you. Your problem report was sent.');persistOnly();alert('Thank you. Your report was saved for admin review.');show('home');}
function privacy(){
