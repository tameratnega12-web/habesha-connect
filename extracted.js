...
function businessMoney(v){return Number(v)||0}
function businessIncomeTotal(){return businessRecords().reduce((s,b)=>s+businessMoney(b.income||b.sales||b.amount||(b.recordType==='invoice'?b.invoiceAmount:0)),0)}
function businessExpenseTotal(){return businessRecords().reduce((s,b)=>s+businessMoney(b.expense||b.expenses||b.cost||b.payrollAmount||b.contractorAmount||0),0)}
function businessUserKey(){return currentUser?(currentUser.id||currentUser.auth_user_id||currentUser.email||'local'):'local'}
function mapBusinessRecordFromDb(row){let d=row.details||{};return Object.assign({},d,{id:row.local_ref||row.id,dbId:row.id,recordType:row.record_type||d.recordType||'business_report',businessName:row.business_name||d.businessName||d.name||'Business',name:row.business_name||d.name||'Business',type:row.business_type||d.type||'Other',amount:businessMoney(row.amount),sales:businessMoney(d.sales||row.income_amount||0),income:businessMoney(d.income||row.income_amount||0),expenses:businessMoney(d.expenses||row.expense_amount||0),expense:businessMoney(d.expense||row.expense_amount||0),period:row.period||d.period||'This Month',date:row.record_date||d.date||new Date().toISOString().slice(0,10),source:'supabase'});}
async function loadBusinessFromSupabase(){if(!authReady()||!currentUser)return;try{let res=await hcSupabase.from('business_records').select('*').eq('owner_email',currentUser.email||'').order('created_at',{ascending:false});if(res.error){console.warn('Business Manager Supabase load error',res.error);return;}let remote=(res.data||[]).map(mapBusinessRecordFromDb);let localOnly=businessRecords().filter(x=>x.source!=='supabase'&&!x.dbId);data.businesses=[...remote,...localOnly];persistOnly();if(currentPage==='business')business();}catch(e){console.warn('Business Manager background load failed',e)}}
async function syncBusinessRecordToSupabase(rec){if(!authReady()||!currentUser||!rec)return {error:null};try{let income=businessMoney(rec.income||rec.sales||rec.amount||(rec.recordType==='invoice'?rec.invoiceAmount:0));let expense=businessMoney(rec.expense||rec.expenses||rec.cost||rec.payrollAmount||rec.contractorAmount||0);let payload={local_ref:rec.id,business_name:rec.businessName||rec.name||'Business',business_type:rec.type||'Other',record_type:rec.recordType||'business_report',period:rec.period||'This Month',record_date:rec.date||new Date().toISOString().slice(0,10),income_amount:income,expense_amount:expense,amount:income||expense||businessMoney(rec.amount),notes:rec.notes||rec.description||'',owner_email:currentUser.email||'',owner_id:businessUserKey(),details:rec};let res=await hcSupabase.from('business_records').upsert(payload,{onConflict:'local_ref'}).select().single();if(!res.error&&res.data){rec.dbId=res.data.id;rec.source='supabase';persistOnly();}else if(res.error){console.warn('Business Manager Supabase save error',res.error);}return res;}catch(e){console.warn('Business Manager background save failed',e);return {error:e};}}
function addBusinessLocal(rec){data.businesses.unshift(rec);persistOnly();fireAndForget(syncBusinessRecordToSupabase(rec),'business record sync');business();return rec}
function business(){let totalSales=businessIncomeTotal(),totalExpenses=businessExpenseTotal();let rows=businessRecords().slice(0,50).map(b=>`<tr><td>${b.businessName||b.name||'Business'}</td><td>${b.recordType||'report'}</td><td>${b.type||'Other'}</td><td>${b.period||''}</td><td>${money(b.income||b.sales||b.amount||0)}</td><td>${money(b.expense||b.expenses||b.cost||b.payrollAmount||b.contractorAmount||0)}</td><td>${b.source==='supabase'?'Saved':'Local / syncing'}</td></tr>`).join('')||'<tr><td colspan="7" class="muted">No business records yet.</td></tr>';$('business').innerHTML=`<h2>📊 Business Manager</h2><div class="actions" style="margin-bottom:16px"><button class="btn primary" onclick="showBusinessTool('dashboard')">📊 Dashboard</button><button class="btn primary" onclick="showBusinessTool('businessType')">🏪 Business Type</button><button class="btn primary" onclick="showBusinessTool('taxi')">🚕 Taxi / Rideshare</button><button class="btn primary" onclick="showBusinessTool('income')">💰 Income</button><button class="btn primary" onclick="showBusinessTool('expenses')">🧾 Expenses</button><button class="btn primary" onclick="showBusinessTool('employees')">👥 Employees</button><button class="btn primary" onclick="showBusinessTool('inventory')">📦 Inventory</button><button class="btn primary" onclick="showBusinessTool('invoices')">🧾 Invoices</button><button class="btn primary" onclick="showBusinessTool('estimates')">📄 Estimates</button><button class="btn primary" onclick="showBusinessTool('payroll')">💵 Payroll W-2</button><button class="btn primary" onclick="showBusinessTool('contractors')">📋 Contractors 1099</button><button class="btn primary" onclick="showBusinessTool('reports')">📈 Reports</button><button class="btn primary" onclick="showBusinessTool('taxes')">🧮 Taxes</button></div><div id="businessTool" class="card" style="margin-bottom:16px"><h3>Business Dashboard</h3><div class="grid"><div><p class="muted">Total Income</p><div class="stat">${money(totalSales)}</div></div><div><p class="muted">Total Expenses</p><div class="stat">${money(totalExpenses)}</div></div><div><p class="muted">Estimated Profit</p><div class="stat">${money(totalSales-totalExpenses)}</div></div></div><p class="muted">Business Manager now saves records locally first and syncs to Supabase in the background.</p></div><div class="grid"><div class="card"><h3>Add Quick Business Report</h3><input id="bName" placeholder="Business name"><select id="bType"><option>Restaurant</option><option>Store / Retail</option><option>Trucking</option><option>Taxi / Rideshare</option><option>Home Service</option><option>Other</option></select><input id="bSales" type="number" placeholder="Sales / income"><input id="bExpenses" type="number" placeholder="Expenses"><select id="bPeriod"><option>This Month</option><option>This Quarter</option><option>This Year</option></select><button class="btn primary" onclick="addBusiness()">Save Report</button></div><div class="card"><h3>Tax Ready Reports</h3><p>Track sales, expenses, payroll, inventory, invoices, estimates, profit & loss, mileage, and year-end summaries.</p><p class="muted">This is an organizer only. Official tax filing should be reviewed by a tax professional.</p></div></div><table><tr><th>Business</th><th>Record</th><th>Type</th><th>Period</th><th>Income</th><th>Expense</th><th>Status</th></tr>${rows}</table>`;fireAndForget(loadBusinessFromSupabase(),'business manager load');}
function bmCommonInputs(prefix='bm'){return `<input id="${prefix}Biz" placeholder="Business name"><select id="${prefix}Type"><option>Restaurant</option><option>Store / Retail</option><option>Trucking</option><option>Taxi / Rideshare</option><option>Home Service</option><option>Other</option></select><input id="${prefix}Date" type="date" value="${new Date().toISOString().slice(0,10)}"><select id="${prefix}Period"><option>This Month</option><option>This Quarter</option><option>This Year</option></select>`}
function showBusinessTool(tool){let titles={dashboard:'📊 Business Dashboard',businessType:'🏪 Business Type',taxi:'🚕 Taxi / Rideshare',income:'💰 Income',expenses:'🧾 Expenses',employees:'👥 Employees',inventory:'📦 Inventory',invoices:'🧾 Invoices',estimates:'📄 Estimates',payroll:'💵 Payroll W-2',contractors:'📋 Contractors 1099',reports:'📈 Reports',taxes:'🧮 Taxes'};let body='';let totalSales=businessIncomeTotal(),totalExpenses=businessExpenseTotal();if(tool==='dashboard'){body=`<div class="grid"><div><p class="muted">Total Income</p><div class="stat">${money(totalSales)}</div></div><div><p class="muted">Total Expenses</p><div class="stat">${money(totalExpenses)}</div></div><div><p class="muted">Estimated Profit</p><div class="stat">${money(totalSales-totalExpenses)}</div></div></div><p class="muted">Records save immediately, then sync to Supabase in the background.</p>`;}else if(tool==='businessType'){body='<p class="muted">Use one Business Manager for restaurants, stores, trucking, taxi/rideshare, home services, or other small businesses.</p><div class="grid"><div class="card"><h4>Restaurant</h4><p>Food sales, supplies, rent, payroll, delivery fees.</p></div><div class="card"><h4>Store / Retail</h4><p>Sales, inventory, cost of goods, rent, utilities.</p></div><div class="card"><h4>Trucking</h4><p>Loads, fuel, maintenance, insurance, dispatch, trailer rent.</p></div><div class="card"><h4>Taxi / Rideshare</h4><p>Cash rides, Uber/Lyft, tips, gas, repair, tolls, mileage.</p></div><div class="card"><h4>Home Service</h4><p>Service income, supplies, tools, helper payments, estimates.</p></div><div class="card"><h4>Other</h4><p>General income and expenses.</p></div></div>';}else if(tool==='taxi'){body=`<p class="muted">Taxi and rideshare drivers can organize Schedule C records.</p>${bmCommonInputs('taxi')}<div class="grid two"><div><h4>Income</h4><input id="taxiUber" type="number" placeholder="Uber / Lyft income"><input id="taxiCash" type="number" placeholder="Cash rides"><input id="taxiTips" type="number" placeholder="Tips"></div><div><h4>Expenses</h4><input id="taxiGas" type="number" placeholder="Gas"><input id="taxiRepair" type="number" placeholder="Repair / maintenance"><input id="taxiInsurance" type="number" placeholder="Insurance"><input id="taxiTolls" type="number" placeholder="Parking / tolls"><input id="taxiMileage" type="number" placeholder="Business miles"></div></div><textarea id="taxiNotes" placeholder="Mileage notes or weekly summary"></textarea><button class="btn primary" onclick="saveBusinessRecord('taxi')">Save Taxi Record</button>`;}else if(tool==='income'){body=`${bmCommonInputs('inc')}<div class="row"><input id="incSource" placeholder="Customer / source"><input id="incAmount" type="number" placeholder="Income amount"></div><select id="incCategory"><option>General Sales</option><option>Cash Rides</option><option>Uber / Lyft</option><option>Tips</option><option>Service Income</option><option>Other Income</option></select><textarea id="incNotes" placeholder="Income notes"></textarea><button class="btn primary" onclick="saveBusinessRecord('income')">Save Income</button>`;}else if(tool==='expenses'){body=`${bmCommonInputs('exp')}<div class="row"><input id="expVendor" placeholder="Vendor / expense name"><input id="expAmount" type="number" placeholder="Expense amount"></div><select id="expCategory"><option>Rent</option><option>Food / Supplies</option><option>Fuel / Gas</option><option>Car Repair / Maintenance</option><option>Insurance</option><option>Parking / Tolls</option><option>Phone</option><option>Utilities</option><option>Payroll</option><option>Other</option></select><textarea id="expNotes" placeholder="Expense notes"></textarea><button class="btn primary" onclick="saveBusinessRecord('expense')">Save Expense</button>`;}else if(tool==='employees'){body=`${bmCommonInputs('emp')}<div class="row"><input id="empName" placeholder="Employee name"><input id="empPosition" placeholder="Position"></div><div class="row"><input id="empRate" type="number" placeholder="Hourly rate / salary"><input id="empContact" placeholder="Phone or email"></div><button class="btn primary" onclick="saveBusinessRecord('employee')">Save Employee</button>`;}else if(tool==='inventory'){body=`${bmCommonInputs('inv')}<div class="row"><input id="invItem" placeholder="Item name"><input id="invQty" type="number" placeholder="Quantity"></div><div class="row"><input id="invCost" type="number" placeholder="Cost"><input id="invPrice" type="number" placeholder="Sale price"></div><button class="btn primary" onclick="saveBusinessRecord('inventory')">Save Inventory</button>`;}else if(tool==='invoices'){body=`${bmCommonInputs('invoice')}<div class="row"><input id="invoiceCustomer" placeholder="Customer name"><input id="invoiceAmount" type="number" placeholder="Invoice amount"></div><textarea id="invoiceDesc" placeholder="Invoice description"></textarea><button class="btn primary" onclick="saveBusinessRecord('invoice')">Create Invoice</button>`;}else if(tool==='estimates'){body=`${bmCommonInputs('estimate')}<div class="row"><input id="estimateCustomer" placeholder="Customer name"><input id="estimateAmount" type="number" placeholder="Estimate amount"></div><textarea id="estimateDesc" placeholder="Work / service description"></textarea><button class="btn primary" onclick="saveBusinessRecord('estimate')">Create Estimate</button>`;}else if(tool==='payroll'){body=`<p class="muted">Keep employee wages organized for year-end W-2 preparation.</p>${bmCommonInputs('pay')}<div class="row"><input id="payEmployee" placeholder="Employee name"><input id="payAmount" type="number" placeholder="Gross wages"></div><button class="btn primary" onclick="saveBusinessRecord('payroll')">Save Payroll</button>`;}else if(tool==='contractors'){body=`<p class="muted">Track contractor payments for possible 1099 records.</p>${bmCommonInputs('con')}<div class="row"><input id="conName" placeholder="Contractor name"><input id="conAmount" type="number" placeholder="Amount paid"></div><input id="conService" placeholder="Service provided"><button class="btn primary" onclick="saveBusinessRecord('contractor')">Save Contractor</button>`;}else if(tool==='reports'){body=`<div class="grid"><div><p class="muted">Profit & Loss</p><div class="stat">${money(totalSales-totalExpenses)}</div></div><div><p class="muted">Total Income</p><div class="stat">${money(totalSales)}</div></div><div><p class="muted">Total Expenses</p><div class="stat">${money(totalExpenses)}</div></div><div><p class="muted">Year-End Summary</p><button class="btn primary" onclick="showBusinessTool('taxes')">Open Tax Organizer</button></div></div>`;}else if(tool==='taxes'){body=`<p class="muted">Schedule C organizer totals. Use this to prepare records for your tax professional.</p><table><tr><th>Category</th><th>Amount</th></tr><tr><td>Gross income</td><td>${money(totalSales)}</td></tr><tr><td>Expenses</td><td>${money(totalExpenses)}</td></tr><tr><td>Estimated net profit</td><td>${money(totalSales-totalExpenses)}</td></tr></table>`;}let box=$('businessTool');if(box)box.innerHTML='<h3>'+titles[tool]+'</h3>'+body;}
function saveBusinessRecord(kind){if(!requireLogin())return;let p={taxi:'taxi',income:'inc',expense:'exp',employee:'emp',inventory:'inv',invoice:'invoice',estimate:'estimate',payroll:'pay',contractor:'con'}[kind]||'bm';let rec={id:'BM'+Date.now().toString().slice(-8),recordType:kind,businessName:($(p+'Biz')&&$(p+'Biz').value)||'Business',name:($(p+'Biz')&&$(p+'Biz').value)||'Business',type:($(p+'Type')&&$(p+'Type').value)||'Other',date:($(p+'Date')&&$(p+'Date').value)||new Date().toISOString().slice(0,10),period:($(p+'Period')&&$(p+'Period').value)||'This Month',createdBy:currentUser.email,time:new Date().toLocaleString()};if(kind==='income'){rec.sourceName=$('incSource').value;rec.income=businessMoney($('incAmount').value);rec.category=$('incCategory').value;rec.notes=$('incNotes').value;}else if(kind==='expense'){rec.vendor=$('expVendor').value;rec.expense=businessMoney($('expAmount').value);rec.category=$('expCategory').value;rec.notes=$('expNotes').value;}else if(kind==='taxi'){rec.income=businessMoney($('taxiUber').value)+businessMoney($('taxiCash').value)+businessMoney($('taxiTips').value);rec.expense=businessMoney($('taxiGas').value)+businessMoney($('taxiRepair').value)+businessMoney($('taxiInsurance').value)+businessMoney($('taxiTolls').value);rec.mileage=businessMoney($('taxiMileage').value);rec.notes=$('taxiNotes').value;}else if(kind==='employee'){rec.employeeName=$('empName').value;rec.position=$('empPosition').value;rec.payRate=businessMoney($('empRate').value);rec.contact=$('empContact').value;}else if(kind==='inventory'){rec.item=$('invItem').value;rec.quantity=businessMoney($('invQty').value);rec.cost=businessMoney($('invCost').value);rec.salePrice=businessMoney($('invPrice').value);rec.expense=rec.cost;}else if(kind==='invoice'){rec.customer=$('invoiceCustomer').value;rec.invoiceAmount=businessMoney($('invoiceAmount').value);rec.income=rec.invoiceAmount;rec.description=$('invoiceDesc').value;rec.status='Created';}else if(kind==='estimate'){rec.customer=$('estimateCustomer').value;rec.amount=businessMoney($('estimateAmount').value);rec.description=$('estimateDesc').value;rec.status='Estimate';}else if(kind==='payroll'){rec.employeeName=$('payEmployee').value;rec.payrollAmount=businessMoney($('payAmount').value);rec.expense=rec.payrollAmount;}else if(kind==='contractor'){rec.contractorName=$('conName').value;rec.contractorAmount=businessMoney($('conAmount').value);rec.expense=rec.contractorAmount;rec.service=$('conService').value;}addBusinessLocal(rec);alert('Business record saved. Supabase sync will happen in the background.');}
function addBusiness(){if(!requireLogin())return;let rec={id:'B'+Date.now().toString().slice(-8),recordType:'business_report',businessName:$('bName').value||'Business',name:$('bName').value||'Business',type:$('bType')?$('bType').value:'Other',sales:+$('bSales').value||0,income:+$('bSales').value||0,expenses:+$('bExpenses').value||0,expense:+$('bExpenses').value||0,period:$('bPeriod').value,date:new Date().toISOString().slice(0,10),createdBy:currentUser.email,time:new Date().toLocaleString()};addBusinessLocal(rec);alert('Business report saved.');}
...
async function admin(){
 ...
 $('admin').innerHTML=`<h2>⚙️ Admin Verification Center</h2>${adminBackgroundRefreshing?'<div class="notice"><b>Refreshing Admin data from Supabase...</b> You can keep working while this updates in the background.</div>':''}<div class='notice'><b>V7.8.46 Admin Fast Load Fix:</b> Admin opens immediately; Supabase refreshes in the background.</div>
