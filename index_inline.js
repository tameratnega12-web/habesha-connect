...
async function syncHomeServiceRequestToSupabase(req,service){if(!authReady())return {error:null,data:null};let payload={local_ref:req.id,service_id:(service&&service.dbId)||null,service_local_ref:(service&&service.id)||req.serviceLocalRef,service_title:req.serviceTitle,provider_name:req.providerName,provider_email:req.providerEmail,customer_name:req.customerName,customer_email:req.customerEmail,customer_phone:req.customerPhone,city:req.city,preferred_date:req.preferredDate,details:req.details,status:req.status};let res=await hcSupabase.from('home_service_requests').insert(payload).select().single();if(res.error&&String(res.error.message||'').includes("'city' column")){let {city,...payloadWithoutCity}=payload;res=await hcSupabase.from('home_service_requests').insert(payloadWithoutCity).select().single();}
...
async function submitHomeServiceRequest(){
 ...
 sendAdminEmailNotice('Home service request waiting for approval','A customer requested a home service. Admin approval is required before the provider receives it.',{Customer:currentUser.name,CustomerEmail:currentUser.email,Provider:svc.providerName,ProviderEmail:svc.providerEmail,Service:svc.title,City:city,PreferredDate:preferredDate,Details:details},'admin');
