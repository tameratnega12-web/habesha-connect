...
async function requestTrip(id){
 ...
 if(authReady()&&currentUser.id&&t.dbId){
   ...
   addNote(currentUser.email,'Space request sent to '+t.traveler+'.');sendAdminEmailNotice('Sender requested luggage space','A sender requested traveler luggage space.',{Sender:senderName,SenderEmail:currentUser.email,Traveler:t.traveler,TravelerEmail:t.travelerEmail,Weight:w+' lb',Route:t.route},'admin');
 ...
 addNote(t.travelerEmail,'New '+w+' lb space request from '+senderName+'.');addNote(currentUser.email,'Space request sent to '+t.traveler+'.');sendAdminEmailNotice('Sender requested luggage space','A sender requested traveler luggage space.',{Sender:senderName,SenderEmail:currentUser.email,Traveler:t.traveler,TravelerEmail:t.travelerEmail,Weight:w+' lb',Route:t.route},'admin');save();alert('Your request was entered correctly and submitted.');shipping()}
...
function truck(){
 ...
 $('truck').innerHTML=`<h2>🚚 Trucking Connect</h2>${role==='truck_owner'?ownerQuick:''}${role==='driver'?driverQuick:''}${topMatchPanels}${summary}${(role==='truck_owner'||role==='admin')?ownerPanel:''}${(role==='driver'||role==='admin')?driverPanel:''}${role==='truck_owner'?'':appsTable}${recordsTable}`;
