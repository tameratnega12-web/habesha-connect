...
function truck(){
 ...
 let driverQuick=`<div class="actions"><button class="btn primary" onclick="document.getElementById('availableTruckJobs')?.scrollIntoView({behavior:'smooth'})">See Jobs</button><button class="btn ghost" onclick="document.getElementById('driverProfileCard')?.scrollIntoView({behavior:'smooth'})">Update Driver Profile</button></div>`;
...
async function homeServices(){
 ...
 $('home_services').innerHTML=`<h2>🔧 Home Services</h2><div class="hero"><h1>Find trusted community service providers</h1><p><b>Phase 1:</b> customers request quotes, providers accept or decline, and payment is handled directly between customer and provider.</p></div><div class="grid">${providerForm}<div class="card"><h3>How it works</h3><p>1. Provider submits profile.</p><p>2. Admin approves provider.</p><p>3. Customer requests service / quote.</p><p>4. Provider accepts, starts, and completes the job.</p><button class="btn ghost" onclick="show('messages')">Open Messages</button></div></div>${myProviderTable}${providerRequestsTable}<h3 style="margin-top:22px">Available Home Service Providers</h3><div class="grid">${providerCards}</div>${myReqTable}`;
}
