...
async function approveRentalReq(id){
  ...
  // Update the screen immediately so admin/owner does not need to refresh.
  // A viewing approval should approve the request only. It should NOT mark the property as Rented.
  q.status='Approved';
  if(r && ['Viewing Pending','Pending Viewing'].includes(String(r.status||''))) r.status='Approved';
  ...
  // Keep the property available after a viewing approval. Do not change it to Rented here.
  if(!res.error&&authReady()&&r&&r.dbId&&['Viewing Pending','Pending Viewing'].includes(String(oldRentalStatus||''))){
    res=await hcSupabase.from('properties').update({status:'Approved'}).eq('id',r.dbId);
  }
  if(res.error){
    q.status=oldStatus;
