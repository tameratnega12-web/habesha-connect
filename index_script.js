...
const ROLE_PAGES={
 ...
 admin:['home','services','profile','shipping','rentals','marketplace','jobs','truck','home_services','taxi','business','messages','notifications','admin',...PUBLIC_PAGES],
 ...
 taxi_limo_owner:['home','services','profile','taxi','messages','notifications',...PUBLIC_PAGES],
 taxi_limo_driver:['home','services','profile','taxi','messages','notifications',...PUBLIC_PAGES],
...
function roleWelcome(){
 ...
 if(role==='taxi_limo_driver'||role==='taxi_driver')return '<p><b>Taxi/Limo driver dashboard:</b> apply for approval, review approved ride requests, accept rides, and update ride status.</p>';
...
async function requestViewing(id){
 ...
 addNote('admin.habeshaconnect@gmail.com','New rental viewing request for '+r.title+' from '+currentUser.name+'.');sendAdminEmailNotice('Seeker requested a viewing','A rent seeker requested a property viewing.',{Seeker:currentUser.name,Email:currentUser.email,Property:r.title,Owner:r.owner,OwnerEmail:r.ownerEmail},'admin');
...
function marketplace(){
 ...
 <div class="card"><h3>Marketplace Dashboard</h3><p class="muted">Choose what you want to do in Marketplace.</p><div class="actions"><button class="btn primary" onclick="marketJump('marketPostPanel')">➕ Post Item</button><button class="btn ghost" onclick="marketJump('marketBrowsePanel')">🔍 Browse Marketplace</button></div></div>
 <div class="grid two">
 ...
 <h3 style="margin-top:22px">Marketplace Items</h3><div id="marketList" class="grid">${visible.map(marketCard).join('')||'<div class="card"><p>No marketplace listings yet.</p></div>'}</div>
...
function marketAdminApproveListing(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let m=data.market.find(x=>x.id===id);if(!m)return;m.status='Approved';addNote(m.sellerEmail,'Your marketplace listing was approved: '+m.title);persistOnly();marketplace();}
function marketAdminDeclineListing(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let m=data.market.find(x=>x.id===id);if(!m)return;m.status='Declined';addNote(m.sellerEmail,'Your marketplace listing was declined: '+m.title);persistOnly();marketplace();}
function marketAdminApproveBuyerReq(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=data.marketRequests.find(x=>x.id===id);if(!r)return;r.status='Admin Approved - Waiting Seller';addNote(r.sellerEmail,'Marketplace buyer request approved by admin. Please accept or decline: '+r.itemTitle);addNote(r.buyerEmail,'Your marketplace request is approved by admin and waiting for seller response: '+r.itemTitle);persistOnly();marketplace();}
function marketAdminDeclineBuyerReq(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=data.marketRequests.find(x=>x.id===id);if(!r)return;r.status='Admin Declined';addNote(r.buyerEmail,'Your marketplace buyer request was declined: '+r.itemTitle);persistOnly();marketplace();}
function marketSellerAcceptReq(id){let r=data.marketRequests.find(x=>x.id===id);if(!r||!currentUser||r.sellerEmail!==currentUser.email)return alert('Seller only.');r.status='Seller Accepted - Waiting Admin';addNote('admin.habeshaconnect@gmail.com','Marketplace seller accepted buyer request. Admin connection approval needed: '+r.itemTitle);if(typeof sendAdminEmailNotice==='function')sendAdminEmailNotice('Marketplace seller accepted request','Seller accepted a buyer request and admin approval is needed before contact sharing.',{Seller:r.sellerName,Buyer:r.buyerName,Item:r.itemTitle,Status:r.status},'marketplace');persistOnly();marketplace();}
function marketSellerDeclineReq(id){let r=data.marketRequests.find(x=>x.id===id);if(!r||!currentUser||r.sellerEmail!==currentUser.email)return alert('Seller only.');r.status='Seller Declined';addNote(r.buyerEmail,'Seller declined your marketplace request: '+r.itemTitle);persistOnly();marketplace();}
function marketAdminApproveConnection(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=data.marketRequests.find(x=>x.id===id);if(!r)return;r.status='Connection Approved';addNote(r.buyerEmail,'Marketplace connection approved. Seller contact is now available for '+r.itemTitle);addNote(r.sellerEmail,'Marketplace connection approved. Buyer contact is now available for '+r.itemTitle);persistOnly();marketplace();}
...
function marketAdminVerifySale(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let m=data.market.find(x=>x.id===id);if(!m)return;m.status='Sold Verified';addNote(m.sellerEmail,'Marketplace sold item verified: '+m.title);persistOnly();marketplace();}
...
async function adminApproveTrailerRequest(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer request not found.');let old={...item};item.status='Pending Owner Review';item.adminApprovedAt=new Date().toLocaleString();let r=await updateTrailerRentalDb(item,{status:'Pending Owner Review'});if(r.error){Object.assign(item,old);return alert('Could not approve trailer request: '+r.error.message);}sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer rental request approved for your review',summary:'Admin approved a trailer rental request and sent it to you for acceptance or decline.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Requester:item.renterName,RequesterEmail:item.renterEmail,Trailer:item.trailerType,Location:item.location,Price:item.price}});sendEmailNotice({to:item.renterEmail,name:item.renterName,subject:'Trailer request approved by admin',summary:'Admin approved your trailer request and sent it to the trailer owner for review.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Owner:item.ownerName,Status:'Pending Owner Review'}});persistOnly();adminSuccess();alert('Trailer request approved and sent to the trailer owner.');}
async function adminFinalizeTrailerRental(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer request not found.');let old={...item};item.status='Rented';item.rentedAt=new Date().toLocaleString();let r=await updateTrailerRentalDb(item,{status:'Rented',rented_at:new Date().toISOString()});if(r.error){Object.assign(item,old);return alert('Could not finalize trailer rental: '+r.error.message);}sendEmailNotice({to:item.renterEmail,name:item.renterName,subject:'Complete your trailer rental',summary:'The trailer owner accepted your request and admin gave final approval. You can now complete the rent.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Owner:item.ownerName,OwnerEmail:item.ownerEmail,Location:item.location,Price:item.price,Status:'Ready to Complete Rent'}});sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer rental final approved',summary:'Admin gave final approval for the trailer rental request.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Requester:item.renterName,RequesterEmail:item.renterEmail,Trailer:item.trailerType,Status:'Rented'}});persistOnly();adminSuccess();alert('Trailer rental final approved. Requester was notified to complete the rent.');}
async function adminDeclineTrailerRequest(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer request not found.');let renterEmail=item.renterEmail,renterName=item.renterName,trailerType=item.trailerType,ownerEmail=item.ownerEmail,ownerName=item.ownerName;item.status='Available';item.renterName='';item.renterEmail='';item.renterPhone='';let r=await updateTrailerRentalDb(item,{status:'Available',renter_name:'',renter_email:'',renter_phone:'',rented_at:null});if(r.error)return alert('Could not decline trailer request: '+r.error.message);if(renterEmail)sendEmailNotice({to:renterEmail,name:renterName,subject:'Trailer rental request declined',summary:'Your trailer rental request was declined by admin.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:trailerType,Status:'Declined'}});if(ownerEmail)sendEmailNotice({to:ownerEmail,name:ownerName,subject:'Trailer rental request declined by admin',summary:'Admin declined a trailer rental request for your trailer.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Requester:renterName,Trailer:trailerType,Status:'Declined'}});persistOnly();adminSuccess();alert('Trailer request declined and trailer is available again.');}
...
function taxiAdminHtml(){
 let apps=data.taxiDriverApps||[], rides=data.taxiRideRequests||[], owners=data.taxiLimoOwners||[], vehicles=data.taxiLimoVehicles||[], assignments=data.taxiLimoDriverAssignments||[];
 ...
 const pendingAssignments=assignments.filter(a=>a.status==='Pending Admin Approval').length;
 const pendingRides=rides.filter(r=>['Pending Admin Approval','Driver Accepted - Waiting Admin Approval','Completed - Waiting Admin Verification'].includes(r.status)).length;
 function adminTaxiCard(title,subtitle,status,actions){return `<div class="card"><h4>${title}</h4><p class="small">${subtitle||''}</p><p>${taxiBadge(status||'')}</p><div class="actions">${actions||'-'}</div></div>`}
 let ownerCards=owners.map(o=>adminTaxiCard('Owner: '+(o.ownerName||'Owner'),`${o.companyName||''} • ${o.city||''}<br>${o.ownerEmail||''} • ${o.phone||''}`,o.status,o.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiOwner('${o.id}')">Approve</button><button class="btn bad" onclick="adminDeclineTaxiOwner('${o.id}')">Decline</button>`:'-')).join('')||'<div class="card"><p class="muted">No taxi/limo owner applications yet.</p></div>';
 let vehicleCards=vehicles.map(v=>adminTaxiCard('Vehicle: '+[v.year,v.make,v.model].filter(Boolean).join(' '),`${v.ownerName||''} • ${v.ownerEmail||''}<br>${v.vehicleType||''} • Plate: ${v.plate||''}`,v.status,v.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiVehicle('${v.id}')">Approve</button><button class="btn bad" onclick="adminDeclineTaxiVehicle('${v.id}')">Decline</button>`:'-')).join('')||'<div class="card"><p class="muted">No taxi/limo vehicles yet.</p></div>';
 let driverCards=apps.map(a=>adminTaxiCard('Driver: '+(a.name||'Driver'),`${a.email||''} • ${a.phone||''}<br>License: ${a.license||''} • Availability: ${a.availability||''} • Area: ${a.area||''}`,a.status,a.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiDriver('${a.id}')">Approve</button><button class="btn bad" onclick="adminDeclineTaxiDriver('${a.id}')">Decline</button>`:'-')).join('')||'<div class="card"><p class="muted">No taxi/limo driver applications yet.</p></div>';
 let assignmentCards=assignments.map(a=>adminTaxiCard('Hire: '+(a.ownerName||'Owner')+' → '+(a.driverName||'Driver'),`${a.ownerEmail||''}<br>${a.driverEmail||''}<br>${a.vehicleLabel||''}`,a.status,a.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiAssignment('${a.id}')">Approve</button><button class="btn bad" onclick="adminDeclineTaxiAssignment('${a.id}')">Decline</button>`:'-')).join('')||'<div class="card"><p class="muted">No owner/driver hire requests yet.</p></div>';
 let rideCards=rides.map(r=>adminTaxiCard('Ride: '+(r.riderName||'Rider'),`${r.riderEmail||''} • ${r.riderPhone||''}<br>Pickup: ${r.pickup||''}<br>Destination: ${r.destination||''}<br>Driver: ${r.driverName||'Not assigned'} ${r.driverPhone?('• '+r.driverPhone):''}`,r.status,taxiAdminActions(r))).join('')||'<div class="card"><p class="muted">No taxi/limo ride requests yet.</p></div>';
 return `<h3>🚕 Taxi/Limo Management</h3><div class="notice"><b>Mobile admin fix:</b> Taxi/Limo transactions now show as phone-friendly cards and scrollable tables on the Admin dashboard.</div><div class="notice"><b>Taxi/Limo flow:</b> Owner application → admin approval. Vehicle post → admin approval. Driver application → admin approval. Owner requests driver → driver accepts/declines → admin approval. Rider request → admin approval. Driver accept → admin approval. Complete ride → admin verification.</div>
 <div class="grid"><div class="card"><p>Pending Owners</p><div class="stat">${pendingOwners}</div></div><div class="card"><p>Pending Vehicles</p><div class="stat">${pendingVehicles}</div></div><div class="card"><p>Pending Drivers</p><div class="stat">${pendingDrivers}</div></div><div class="card"><p>Pending Hires</p><div class="stat">${pendingAssignments}</div></div><div class="card"><p>Pending Rides</p><div class="stat">${pendingRides}</div></div></div>
 <h4>Phone View: Taxi/Limo Transactions</h4><div class="grid">${ownerCards}${vehicleCards}${driverCards}${assignmentCards}${rideCards}</div>
 <h4>Taxi/Limo Owner Applications</h4><div class="tableWrap"><table><tr><th>Owner</th><th>Company</th><th>Phone</th><th>Status</th><th>Action</th></tr>${owners.map(o=>`<tr><td>${o.ownerName}<br><span class="small">${o.ownerEmail}</span></td><td>${o.companyName||''}<br><span class="small">${o.city||''}</span></td><td>${o.phone||''}</td><td>${taxiBadge(o.status)}</td><td>${o.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiOwner('${o.id}')">Approve</button> <button class="btn bad" onclick="adminDeclineTaxiOwner('${o.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="5">No taxi/limo owner applications yet.</td></tr>'}</table></div>
 <h4>Taxi/Limo Vehicles</h4><div class="tableWrap"><table><tr><th>Owner</th><th>Vehicle</th><th>Plate</th><th>Status</th><th>Action</th></tr>${vehicles.map(v=>`<tr><td>${v.ownerName}<br><span class="small">${v.ownerEmail}</span></td><td>${v.year||''} ${v.make||''} ${v.model||''}<br><span class="small">${v.vehicleType||''}</span></td><td>${v.plate||''}</td><td>${taxiBadge(v.status)}</td><td>${v.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiVehicle('${v.id}')">Approve</button> <button class="btn bad" onclick="adminDeclineTaxiVehicle('${v.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="5">No taxi/limo vehicles yet.</td></tr>'}</table></div>
 <h4>Taxi/Limo Driver Applications</h4><div class="tableWrap"><table><tr><th>Driver</th><th>Phone</th><th>Availability / License</th><th>Status</th><th>Action</th></tr>${apps.map(a=>`<tr><td>${a.name}<br><span class="small">${a.email}</span></td><td>${a.phone||''}</td><td>${a.availability||''}<br><span class="small">Area: ${a.area||''} • License: ${a.license||''}</span></td><td>${taxiBadge(a.status)}</td><td>${a.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiDriver('${a.id}')">Approve</button> <button class="btn bad" onclick="adminDeclineTaxiDriver('${a.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="5">No taxi/limo driver applications yet.</td></tr>'}</table></div>
 <h4>Owner / Driver Hire Requests</h4><div class="tableWrap"><table><tr><th>Owner</th><th>Driver</th><th>Vehicle</th><th>Status</th><th>Action</th></tr>${assignments.map(a=>`<tr><td>${a.ownerName}<br><span class="small">${a.ownerEmail}</span></td><td>${a.driverName}<br><span class="small">${a.driverEmail}</span></td><td>${a.vehicleLabel||''}</td><td>${taxiBadge(a.status)}</td><td>${a.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiAssignment('${a.id}')">Approve</button> <button class="btn bad" onclick="adminDeclineTaxiAssignment('${a.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="5">No owner/driver hire requests yet.</td></tr>'}</table></div>
 <h4>Taxi/Limo Ride Requests</h4><div class="tableWrap"><table><tr><th>Rider</th><th>Pickup</th><th>Destination</th><th>Driver</th><th>Status</th><th>Action</th></tr>${rides.map(r=>`<tr><td>${r.riderName}<br><span class="small">${r.riderEmail} • ${r.riderPhone||''}</span></td><td>${r.pickup}<br><span class="small">${r.pickupTime||'Now'}</span></td><td>${r.destination}<br><span class="small">Passengers: ${r.passengers||1}</span></td><td>${r.driverName||'Not assigned'}<br><span class="small">${r.driverPhone||''}</span></td><td>${taxiBadge(r.status)}</td><td>${taxiAdminActions(r)}</td></tr>`).join('')||'<tr><td colspan="6">No taxi/limo ride requests yet.</td></tr>'}</table></div>`;
}
...
function taxi(){
 ...
 let available=rides.filter(r=>r.status==='Approved - Waiting Driver' && isDriver && r.driverEmail!==currentUser.email);
 let myRides=rides.filter(r=>r.riderEmail===currentUser.email||r.driverEmail===currentUser.email||currentUser.role==='admin');
 let showRiderForm=!(currentUser.role==='taxi_limo_owner'||currentUser.role==='taxi_limo_driver'||currentUser.role==='taxi_driver');
 let riderForm=showRiderForm?`<div class="card"><h3>Request Taxi/Limo</h3><p class="muted">Your taxi/limo request goes to admin first. After admin approves, approved taxi/limo drivers can accept it.</p><label>Pickup Location</label><input id="taxiPickup" placeholder="Address or pickup area"><label>Destination</label><input id="taxiDest" placeholder="Where are you going?"><div class="row"><div><label>Pickup Time</label><select id="taxiTime"><option>Now</option><option>Schedule</option></select></div><div><label>Passengers</label><input id="taxiPassengers" type="number" value="1" min="1"></div></div><label>Phone Number</label><input id="taxiPhone" value="${currentUser.phone||''}" placeholder="404-555-1234"><label>Ride Note</label><textarea id="taxiNote" placeholder="Gate code, luggage, airport terminal, etc."></textarea><button class="btn primary" onclick="submitTaxiRide()">Submit Taxi/Limo Request</button></div>`:'';
 let driverForm='';
 if(currentUser.role==='taxi_limo_driver'||currentUser.role==='taxi_driver'){
   driverForm=!myApp?`<div class="card"><h3>Apply as Taxi/Limo Driver</h3><p class="muted">Drivers apply first and show availability. Vehicle information is not needed because owners provide the taxi/limo vehicle.</p><label>Driver License Number</label><input id="taxiLicense" placeholder="License number"><label>Years of Driving Experience</label><input id="taxiExperience" placeholder="Example: 5 years"><label>Availability</label><select id="taxiAvailability"><option>Full-time</option><option>Part-time</option><option>Weekdays</option><option>Weekends</option><option>Temporary</option><option>Monthly</option><option>Yearly</option><option>On Call</option></select><label>City / Service Area</label><input id="taxiArea" placeholder="Atlanta, Clarkston, airport..."><label>Notes</label><textarea id="taxiDriverNotes" placeholder="Airport experience, limo experience, preferred schedule, languages, etc."></textarea><button class="btn primary" onclick="applyTaxiDriver()">Submit Driver Application</button></div>`:`<div class="card"><h3>Taxi/Limo Driver Status</h3><p>Status: ${taxiBadge(myApp.ownerEmail?'Hired / Not Available':myApp.status)}</p><p class="muted">${myApp.ownerEmail?'You are currently hired and hidden from other Taxi/Limo Owner dashboards. If the job ends, click Leave Current Employer to become available again.':(myApp.status==='Approved'?'Owners can see your approved driver profile and request/assign you through admin approval.':'Wait for admin approval before owners can see your profile.')}</p><p class="small"><b>Availability:</b> ${myApp.availability||'Not entered'}<br><b>Service Area:</b> ${myApp.area||''}<br><b>Experience:</b> ${myApp.experience||''}<br><b>Current Vehicle:</b> ${myApp.assignedVehicle||'Not assigned'}</p>${myApp.ownerEmail?`<button class="btn bad" onclick="taxiDriverLeaveEmployer()">Leave Current Employer</button>`:''}</div>`;
 }
 ...
 let assignmentRows=myAssignments.map(a=>`<tr><td>${a.driverName}</td><td>${a.vehicleLabel}</td><td>${taxiBadge(a.status)}</td><td>${a.status==='Approved'?`<button class="btn bad" onclick="taxiOwnerEndEmployment('${a.id}')">End Employment</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="4">No driver hire requests yet.</td></tr>';
 let driverHireRows=myDriverHireRequests.map(a=>`<tr><td>${a.ownerName}<br><span class="small">${a.ownerEmail}</span></td><td>${a.vehicleLabel||''}</td><td>${taxiBadge(a.status)}</td><td>${a.status==='Waiting Driver Acceptance'?`<button class="btn primary" onclick="taxiDriverAcceptHire('${a.id}')">Accept</button> <button class="btn bad" onclick="taxiDriverDeclineHire('${a.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="4">No owner hire requests yet.</td></tr>';
...
function applyTaxiOwner(){let company=($('taxiOwnerCompany')?.value||'').trim(),city=($('taxiOwnerCity')?.value||'').trim(),phone=($('taxiOwnerPhone')?.value||'').trim();if(!company||!city||!phone)return alert('Please complete company name, city/service area, and phone.');let existing=findTaxiOwner(currentUser.email);if(existing)return alert('You already submitted a Taxi/Limo owner application.');let o={id:'TLO'+Date.now().toString().slice(-6),ownerName:currentUser.name,ownerEmail:currentUser.email,phone,companyName:company,city,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.taxiLimoOwners.unshift(o);sendAdminEmailNotice('Taxi/Limo owner application waiting for approval','A taxi/limo owner submitted an application.',{Owner:o.ownerName,Email:o.ownerEmail,Company:company,City:city,Phone:phone},'admin');persistOnly();alert('Taxi/Limo owner application submitted. Admin approval is required.');taxi();}
function addTaxiVehicle(){let owner=findTaxiOwner(currentUser.email);if(!owner||owner.status!=='Approved')return alert('Admin must approve your Taxi/Limo owner application first.');let vehicleType=$('taxiVehicleType').value,make=($('taxiVehicleMake').value||'').trim(),model=($('taxiVehicleModel').value||'').trim(),year=($('taxiVehicleYear').value||'').trim(),plate=($('taxiVehiclePlate').value||'').trim(),docs=($('taxiVehicleDocs').value||'').trim();if(!make||!model||!year||!plate)return alert('Please complete make, model, year, and plate.');let v={id:'TLV'+Date.now().toString().slice(-6),ownerId:owner.id,ownerName:owner.ownerName,ownerEmail:owner.ownerEmail,companyName:owner.companyName,vehicleType,make,model,year,plate,docs,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.taxiLimoVehicles.unshift(v);sendAdminEmailNotice('Taxi/Limo vehicle waiting for approval','A taxi/limo owner added a vehicle. Admin approval is required.',{Owner:owner.ownerName,Company:owner.companyName,Vehicle:`${year} ${make} ${model}`,Plate:plate,Type:vehicleType},'admin');persistOnly();alert('Vehicle submitted. Admin approval is required.');taxi();}
function assignTaxiDriverToVehicle(){let owner=findTaxiOwner(currentUser.email);if(!owner||owner.status!=='Approved')return alert('Admin must approve your Taxi/Limo owner application first.');let driverEmail=$('taxiAssignDriver').value,vehicleId=$('taxiAssignVehicle').value;if(!driverEmail||!vehicleId)return alert('Choose an approved driver and an approved vehicle.');let d=(data.taxiDriverApps||[]).find(a=>a.email===driverEmail&&a.status==='Approved'),v=findTaxiVehicle(vehicleId);if(!d||!v)return alert('Driver or vehicle was not found.');let existing=(data.taxiLimoDriverAssignments||[]).find(x=>x.driverEmail===driverEmail&&x.vehicleId===vehicleId&&['Waiting Driver Acceptance','Pending Admin Approval','Approved'].includes(x.status));if(existing)return alert('A hire request for this driver and vehicle already exists.');let a={id:'TLA'+Date.now().toString().slice(-6),ownerId:owner.id,ownerName:owner.ownerName,ownerEmail:owner.ownerEmail,driverName:d.name,driverEmail:d.email,vehicleId:v.id,vehicleLabel:`${v.year} ${v.make} ${v.model} - ${v.plate}`,status:'Waiting Driver Acceptance',createdAt:new Date().toLocaleString()};data.taxiLimoDriverAssignments.unshift(a);addNote(d.email,'Taxi/Limo owner wants to hire you: '+owner.companyName+' / '+a.vehicleLabel);sendEmailNotice({to:d.email,name:d.name,subject:'Taxi/Limo owner hire request',summary:'A taxi/limo owner wants to hire you. Please accept or decline in your Taxi/Limo dashboard.',buttonText:'Open Taxi/Limo',page:'taxi',details:{Owner:owner.ownerName,Company:owner.companyName,Vehicle:a.vehicleLabel,Status:a.status}});persistOnly();alert('Hire request sent to the driver. If the driver accepts, admin will approve the connection.');taxi();}
function adminApproveTaxiOwner(id){let o=(data.taxiLimoOwners||[]).find(x=>x.id===id);if(!o)return;o.status='Approved';addNote(o.ownerEmail,'Your Taxi/Limo owner application was approved. You can add vehicles.');sendEmailNotice({to:o.ownerEmail,name:o.ownerName,subject:'Taxi/Limo owner approved',summary:'Your Taxi/Limo owner application was approved.',buttonText:'Open Taxi/Limo',page:'taxi',details:{Company:o.companyName,Status:'Approved'}});persistOnly();adminSuccess();}
function adminDeclineTaxiOwner(id){let o=(data.taxiLimoOwners||[]).find(x=>x.id===id);if(!o)return;o.status='Declined';addNote(o.ownerEmail,'Your Taxi/Limo owner application was declined.');persistOnly();adminSuccess();}
function adminApproveTaxiVehicle(id){let v=findTaxiVehicle(id);if(!v)return;v.status='Approved';addNote(v.ownerEmail,'Your Taxi/Limo vehicle was approved: '+v.year+' '+v.make+' '+v.model);persistOnly();adminSuccess();}
function adminDeclineTaxiVehicle(id){let v=findTaxiVehicle(id);if(!v)return;v.status='Declined';addNote(v.ownerEmail,'Your Taxi/Limo vehicle was declined: '+v.year+' '+v.make+' '+v.model);persistOnly();adminSuccess();}
function taxiDriverAcceptHire(id){let a=(data.taxiLimoDriverAssignments||[]).find(x=>x.id===id);if(!a)return alert('Hire request not found.');if(a.driverEmail!==currentUser.email)return alert('Only this driver can accept the hire request.');if(a.status!=='Waiting Driver Acceptance')return alert('This hire request is not waiting for driver acceptance.');a.status='Pending Admin Approval';a.driverAcceptedAt=new Date().toLocaleString();addNote(a.ownerEmail,'Taxi/Limo driver accepted your hire request. Waiting for admin approval: '+a.driverName);sendAdminEmailNotice('Taxi/Limo hire request accepted - waiting admin approval','A taxi/limo driver accepted an owner hire request. Admin approval is now required before they are connected.',{Owner:a.ownerName,OwnerEmail:a.ownerEmail,Driver:a.driverName,DriverEmail:a.driverEmail,Vehicle:a.vehicleLabel},'admin');persistOnly();alert('You accepted the hire request. Admin approval is now required.');taxi();}
function taxiDriverDeclineHire(id){let a=(data.taxiLimoDriverAssignments||[]).find(x=>x.id===id);if(!a)return alert('Hire request not found.');if(a.driverEmail!==currentUser.email)return alert('Only this driver can decline the hire request.');if(a.status!=='Waiting Driver Acceptance')return alert('This hire request is not waiting for driver acceptance.');a.status='Driver Declined';a.driverDeclinedAt=new Date().toLocaleString();addNote(a.ownerEmail,'Taxi/Limo driver declined your hire request: '+a.driverName);sendEmailNotice({to:a.ownerEmail,name:a.ownerName,subject:'Taxi/Limo driver declined hire request',summary:'The driver declined your hire request.',buttonText:'Open Taxi/Limo',page:'taxi',details:{Driver:a.driverName,Vehicle:a.vehicleLabel,Status:a.status}});persistOnly();alert('You declined the hire request.');taxi();}
function adminApproveTaxiAssignment(id){let a=(data.taxiLimoDriverAssignments||[]).find(x=>x.id===id);if(!a)return;a.status='Approved';a.hiredAt=new Date().toLocaleString();let d=(data.taxiDriverApps||[]).find(x=>x.email===a.driverEmail);if(d){d.ownerEmail=a.ownerEmail;d.assignedVehicleId=a.vehicleId;d.assignedVehicle=a.vehicleLabel;d.availabilityStatus='Hired';}addNote(a.ownerEmail,'Taxi/Limo hire connection approved by admin: '+a.driverName);addNote(a.driverEmail,'Your Taxi/Limo hire connection was approved by admin. Vehicle: '+a.vehicleLabel);persistOnly();adminSuccess();}
function adminDeclineTaxiAssignment(id){let a=(data.taxiLimoDriverAssignments||[]).find(x=>x.id===id);if(!a)return;a.status='Declined';addNote(a.ownerEmail,'Taxi/Limo driver assignment declined: '+a.driverName);persistOnly();adminSuccess();}


function endTaxiEmploymentByAssignment(a,endedBy){
 ...
 if(d){d.ownerEmail='';d.assignedVehicleId='';d.assignedVehicle='';d.availabilityStatus='Available';}
...
function taxiOwnerEndEmployment(id){let a=(data.taxiLimoDriverAssignments||[]).find(x=>x.id===id);if(!a)return alert('Employment record not found.');if(a.ownerEmail!==currentUser.email&&currentUser.role!=='admin')return alert('Only this owner can end this employment.');if(!confirm('End employment for '+a.driverName+'? The driver will become available to other Taxi/Limo owners.'))return;endTaxiEmploymentByAssignment(a,'Owner');}
function taxiDriverLeaveEmployer(){let a=(data.taxiLimoDriverAssignments||[]).find(x=>x.driverEmail===currentUser.email&&x.status==='Approved');if(!a)return alert('You do not have an active Taxi/Limo employment record.');if(!confirm('Leave your current Taxi/Limo employer? Your driver profile will become available again.'))return;endTaxiEmploymentByAssignment(a,'Driver');}

async function applyTaxiDriver(){let license=($('taxiLicense')?.value||'').trim(),experience=($('taxiExperience')?.value||'').trim(),availability=($('taxiAvailability')?.value||'').trim(),area=($('taxiArea')?.value||'').trim(),notes=($('taxiDriverNotes')?.value||'').trim();if(!license||!experience||!availability||!area)return alert('Please complete license, experience, availability, and service area.');let app={id:'TD'+Date.now().toString().slice(-6),name:currentUser.name,email:currentUser.email,phone:currentUser.phone||'',license,experience,availability,area,notes,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.taxiDriverApps.unshift(app);addNote('admin.habeshaconnect@gmail.com','Taxi/Limo driver application needs approval: '+currentUser.name);sendAdminEmailNotice('Taxi/Limo driver application waiting for approval','A taxi/limo driver submitted availability for owner matching. Vehicle information is not required because the owner provides the vehicle.',{Driver:currentUser.name,Email:currentUser.email,Phone:currentUser.phone||'',License:license,Experience:experience,Availability:availability,Area:area,Notes:notes},'admin');persistOnly();alert('Taxi/Limo driver application submitted. Admin approval is required.');taxi();}
async function submitTaxiRide(){let pickup=($('taxiPickup').value||'').trim(),destination=($('taxiDest').value||'').trim(),pickupTime=$('taxiTime').value,passengers=($('taxiPassengers').value||'1'),phone=($('taxiPhone').value||'').trim(),note=($('taxiNote').value||'').trim();if(!pickup||!destination||!phone)return alert('Please complete pickup, destination, and phone number.');let ride={id:'TX'+Date.now().toString().slice(-6),riderName:currentUser.name,riderEmail:currentUser.email,riderPhone:phone,pickup,destination,pickupTime,passengers,note,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.taxiRideRequests.unshift(ride);addNote('admin.habeshaconnect@gmail.com','Taxi ride request needs approval from '+currentUser.name);sendAdminEmailNotice('Taxi ride request waiting for approval','A rider submitted a taxi/limo request. Admin approval is required before drivers see it.',{Rider:currentUser.name,Email:currentUser.email,Phone:phone,Pickup:pickup,Destination:destination,Passengers:passengers,Note:note},'admin');persistOnly();alert('Taxi request submitted. Admin must approve before drivers can accept.');taxi();}
...
function adminApproveTaxiDriver(id){let a=(data.taxiDriverApps||[]).find(x=>x.id===id);if(!a)return; a.status='Approved';addNote(a.email,'Your taxi driver application was approved. You can now accept approved taxi ride requests.');sendEmailNotice({to:a.email,name:a.name,subject:'Taxi driver approved',summary:'Your taxi driver application was approved.',buttonText:'Open Taxi',page:'taxi',details:{Status:'Approved',Availability:a.availability||'',Area:a.area||''}});persistOnly();adminSuccess();}
function adminDeclineTaxiDriver(id){let a=(data.taxiDriverApps||[]).find(x=>x.id===id);if(!a)return; a.status='Declined';addNote(a.email,'Your taxi driver application was declined.');sendEmailNotice({to:a.email,name:a.name,subject:'Taxi driver application declined',summary:'Your taxi driver application was declined.',buttonText:'Open Taxi',page:'taxi',details:{Status:'Declined'}});persistOnly();adminSuccess();}
...
function business(){let totalSales=data.businesses.reduce((s,b)=>s+(+b.sales||0),0),totalExpenses=data.businesses.reduce((s,b)=>s+(+b.expenses||0),0);$('business').innerHTML=`<h2>📊 Business Manager</h2><div class="actions" style="margin-bottom:16px"><button class="btn primary" onclick="showBusinessTool('dashboard')">📊 Dashboard</button><button class="btn primary" onclick="showBusinessTool('businessType')">🏪 Business Type</button><button class="btn primary" onclick="showBusinessTool('taxi')">🚕 Taxi / Rideshare</button><button class="btn primary" onclick="showBusinessTool('income')">💰 Income</button><button class="btn primary" onclick="showBusinessTool('expenses')">🧾 Expenses</button><button class="btn primary" onclick="showBusinessTool('employees')">👥 Employees</button><button class="btn primary" onclick="showBusinessTool('inventory')">📦 Inventory</button><button class="btn primary" onclick="showBusinessTool('invoices')">🧾 Invoices</button><button class="btn primary" onclick="showBusinessTool('estimates')">📄 Estimates</button><button class="btn primary" onclick="showBusinessTool('payroll')">💵 Payroll W-2</button><button class="btn primary" onclick="showBusinessTool('contractors')">📋 Contractors 1099</button><button class="btn primary" onclick="showBusinessTool('reports')">📈 Reports</button><button class="btn primary" onclick="showBusinessTool('taxes')">🧮 Taxes</button></div><div id="businessTool" class="card" style="margin-bottom:16px"><h3>Business Dashboard</h3><div class="grid"><div><p class="muted">Total Sales</p><div class="stat">${money(totalSales)}</div></div><div><p class="muted">Total Expenses</p><div class="stat">${money(totalExpenses)}</div></div><div><p class="muted">Estimated Profit</p><div class="stat">${money(totalSales-totalExpenses)}</div></div></div><p class="muted">Choose a business type to organize records for restaurants, stores, trucking, taxi/rideshare, home services, or other small businesses.</p></div><div class="grid"><div class="card"><h3>Add Business Report</h3><input id="bName" placeholder="Business name"><select id="bType"><option>Restaurant</option><option>Store / Retail</option><option>Trucking</option><option>Taxi / Rideshare</option><option>Home Service</option><option>Other</option></select><input id="bSales" type="number" placeholder="Sales / income"><input id="bExpenses" type="number" placeholder="Expenses"><select id="bPeriod"><option>This Month</option><option>This Quarter</option><option>This Year</option></select><button class="btn primary" onclick="addBusiness()">Save Report</button></div><div class="card"><h3>Tax Ready Reports</h3><p>Track sales, expenses, payroll, inventory, invoices, estimates, profit & loss, cash flow, mileage, and year-end summaries.</p><p class="muted">Taxi drivers can track Uber/Lyft income, cash rides, tips, gas, repairs, insurance, tolls, parking, phone, and mileage.</p></div></div><table><tr><th>Business</th><th>Type</th><th>Period</th><th>Sales</th><th>Expenses</th><th>Profit</th></tr>${data.businesses.map(b=>`<tr><td>${b.name}</td><td>${b.type||'Other'}</td><td>${b.period}</td><td>${money(b.sales)}</td><td>${money(b.expenses)}</td><td>${money(b.sales-b.expenses)}</td></tr>`).join('')}</table>`}
function showBusinessTool(tool){let titles={dashboard:'📊 Business Dashboard',businessType:'🏪 Business Type',taxi:'🚕 Taxi / Rideshare',income:'💰 Income',expenses:'🧾 Expenses',employees:'👥 Employees',inventory:'📦 Inventory',invoices:'🧾 Invoices',estimates:'📄 Estimates',payroll:'💵 Payroll W-2',contractors:'📋 Contractors 1099',reports:'📈 Reports',taxes:'🧮 Taxes'};let body='';if(tool==='dashboard'){let totalSales=data.businesses.reduce((s,b)=>s+(+b.sales||0),0),totalExpenses=data.businesses.reduce((s,b)=>s+(+b.expenses||0),0);body=`<div class="grid"><div><p class="muted">Total Sales</p><div class="stat">${money(totalSales)}</div></div><div><p class="muted">Total Expenses</p><div class="stat">${money(totalExpenses)}</div></div><div><p class="muted">Estimated Profit</p><div class="stat">${money(totalSales-totalExpenses)}</div></div></div>`;}else if(tool==='businessType'){body='<p class="muted">Use one Business Manager for all small businesses. Select the business type, then record income and expenses for that business.</p><div class="grid"><div class="card"><h4>Restaurant</h4><p>Food sales, supplies, rent, payroll, delivery fees.</p></div><div class="card"><h4>Store / Retail</h4><p>Sales, inventory, cost of goods, rent, utilities.</p></div><div class="card"><h4>Trucking</h4><p>Loads, fuel, maintenance, insurance, dispatch, trailer rent.</p></div><div class="card"><h4>Taxi / Rideshare</h4><p>Cash rides, Uber/Lyft, tips, gas, repair, tolls, mileage.</p></div><div class="card"><h4>Home Service</h4><p>Service income, supplies, tools, helper payments, estimates.</p></div><div class="card"><h4>Other</h4><p>General income and expenses for any small business.</p></div></div>';}else if(tool==='taxi'){body='<p class="muted">Taxi and rideshare drivers can use this to organize Schedule C records. This is frontend only; database saving can be added later.</p><div class="grid"><div><h4>Income</h4><input placeholder="Uber / Lyft income"><input placeholder="Cash rides"><input placeholder="Airport trips"><input placeholder="Tips"></div><div><h4>Expenses</h4><input placeholder="Gas"><input placeholder="Car repair / maintenance"><input placeholder="Insurance"><input placeholder="Parking / tolls"><input placeholder="Phone"><input placeholder="Car wash"><input placeholder="License / permit"></div></div><textarea placeholder="Mileage notes or weekly summary"></textarea><button class="btn primary" onclick="alert(&quot;Taxi / rideshare screen is ready. Database save can be added later.&quot;)">Save Taxi Record</button>';}else if(tool==='income'){body='<div class="row"><input placeholder="Customer / source"><input type="number" placeholder="Income amount"></div><select><option>General Sales</option><option>Cash Rides</option><option>Uber / Lyft</option><option>Tips</option><option>Service Income</option><option>Other Income</option></select><textarea placeholder="Income notes"></textarea><button class="btn primary" onclick="alert(&quot;Income record screen is ready. Database save can be added later.&quot;)">Save Income</button>';}else if(tool==='expenses'){body='<div class="row"><input placeholder="Vendor / expense name"><input type="number" placeholder="Expense amount"></div><select><option>Rent</option><option>Food / Supplies</option><option>Fuel / Gas</option><option>Car Repair / Maintenance</option><option>Insurance</option><option>Parking / Tolls</option><option>Phone</option><option>Utilities</option><option>Payroll</option><option>Other</option></select><textarea placeholder="Expense notes"></textarea><button class="btn primary" onclick="alert(&quot;Expense record screen is ready. Database save can be added later.&quot;)">Save Expense</button>';}else if(tool==='employees'){body='<div class="row"><input placeholder="Employee name"><input placeholder="Position"></div><div class="row"><input type="number" placeholder="Hourly rate / salary"><input placeholder="Phone or email"></div><button class="btn primary" onclick="alert(&quot;Employee screen is ready. Payroll database can be added later.&quot;)">Save Employee</button>';}else if(tool==='inventory'){body='<div class="row"><input placeholder="Item name"><input type="number" placeholder="Quantity"></div><div class="row"><input type="number" placeholder="Cost"><input type="number" placeholder="Sale price"></div><button class="btn primary" onclick="alert(&quot;Inventory screen is ready. Database save can be added later.&quot;)">Save Inventory</button>';}else if(tool==='invoices'){body='<div class="row"><input placeholder="Customer name"><input type="number" placeholder="Invoice amount"></div><textarea placeholder="Invoice description"></textarea><button class="btn primary" onclick="alert(&quot;Invoice screen is ready. PDF/payment can be added later.&quot;)">Create Invoice</button>';}else if(tool==='estimates'){body='<div class="row"><input placeholder="Customer name"><input type="number" placeholder="Estimate amount"></div><textarea placeholder="Work / service description"></textarea><button class="btn primary" onclick="alert(&quot;Estimate screen is ready. Convert-to-invoice can be added later.&quot;)">Create Estimate</button>';}else if(tool==='payroll'){body='<p class="muted">Keep employee wages organized for year-end W-2 preparation. This app can collect payroll totals, but official filing should be reviewed by a tax professional.</p><div class="row"><input placeholder="Employee name"><input type="number" placeholder="Gross wages"></div><button class="btn primary" onclick="alert(&quot;Payroll W-2 screen is ready. Tax filing logic can be added later.&quot;)">Save Payroll</button>';}else if(tool==='contractors'){body='<p class="muted">Track contractor payments for possible 1099 records.</p><div class="row"><input placeholder="Contractor name"><input type="number" placeholder="Amount paid"></div><input placeholder="Service provided"><button class="btn primary" onclick="alert(&quot;Contractor 1099 screen is ready. Database save can be added later.&quot;)">Save Contractor</button>';}else if(tool==='reports'){body='<div class="grid"><div><p class="muted">Profit & Loss</p><button class="btn primary" onclick="alert(&quot;Profit & Loss report will use saved income and expenses.&quot;)">Open Report</button></div><div><p class="muted">Cash Flow</p><button class="btn primary" onclick="alert(&quot;Cash flow report will be added later.&quot;)">Open Report</button></div><div><p class="muted">Year-End Summary</p><button class="btn primary" onclick="alert(&quot;Year-end summary will be added later.&quot;)">Open Report</button></div><div><p class="muted">Taxi Schedule C Summary</p><button class="btn primary" onclick="alert(&quot;Taxi summary will use income, expenses, and mileage records.&quot;)">Open Report</button></div></div>';}else if(tool==='taxes'){body='<p class="muted">For sole proprietors, this section can organize Schedule C information: income, expenses, mileage, inventory, payroll, contractor payments, and year-end totals.</p><button class="btn primary" onclick="alert(&quot;Tax organizer screen is ready. Official tax forms can be added later.&quot;)">Open Tax Organizer</button>';}let box=$('businessTool');if(box)box.innerHTML='<h3>'+titles[tool]+'</h3>'+body;}
function addBusiness(){if(!requireLogin())return;data.businesses.unshift({id:'B'+Date.now().toString().slice(-5),name:$('bName').value||'Business',type:$('bType')?$('bType').value:'Other',sales:+$('bSales').value||0,expenses:+$('bExpenses').value||0,period:$('bPeriod').value});save()}

function about(){
 $('about').innerHTML=`<h2>About Habesha Connect</h2><div class="card"><p><b>Habesha Connect</b> is a beta community platform built to help Ethiopian and Habesha communities connect around shipping, rentals, trucking, jobs, marketplace listings, and business tools.</p><p class="muted">This platform is for community use. Users are responsible for confirming identity, details, safety, pricing, pickup, delivery, property information, and job/trucking terms before completing any agreement.</p><div class="actions"><button class="btn primary" onclick="show('account')">Create Account</button><button class="btn ghost" onclick="show('contact')">Contact Us</button></div></div>`;
}
function contact(){
...
function resetAppData(){if(confirm('Clear all saved app data?')){Object.keys(localStorage).filter(k=>k.startsWith('hc')).forEach(k=>localStorage.removeItem(k));data=seed();currentUser=null;save();location.reload();}}
