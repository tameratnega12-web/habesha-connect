...
function roleDashboardCards(){
 ...
 if(currentUser.role==='truck_owner')return `<div class="grid"><div class="card"><h3>📢 Post Driver Hiring Job</h3><p class="muted">Create a driver job post from your profile. Admin approval is still required before drivers can apply.</p><button class="btn primary" onclick="openTruckSection('jobPostCard')">Post Driver Hiring Job</button></div><div class="card"><h3>👥 Driver Applicants</h3><p class="muted">Review admin-approved driver applications for your jobs.</p><button class="btn" onclick="openTruckSection('truckApplicationsTable')">See Driver Applicants</button></div><div class="card"><h3>🚛 Trailer Rentals</h3><p class="muted">Post trailers for rent and manage trailer rental requests.</p><button class="btn" onclick="openTruckSection('trailerRentCard')">Post Trailer Rent</button></div></div>`;
 if(currentUser.role==='driver')return `<div class="grid"><div class="card"><h3>🚚 See Available Truck Jobs</h3><p class="muted">Open approved truck owner hiring posts from your profile.</p><button class="btn primary" onclick="openTruckSection('availableTruckJobs')">See Available Truck Jobs</button></div><div class="card"><h3>🪪 Driver Job Profile</h3><p class="muted">Keep your license, experience, phone, city, and notes updated before applying.</p><button class="btn" onclick="openTruckSection('driverProfileCard')">Update Driver Profile</button></div><div class="card"><h3>📋 My Applications</h3><p class="muted">Track your truck driving applications and approval status.</p><button class="btn" onclick="openTruckSection('truckApplicationsTable')">See My Applications</button></div></div>`;
...
async function addRental(){
  ...
  addNote('admin.habeshaconnect@gmail.com','Rental listing from '+currentUser.name+' needs approval.');sendAdminEmailNotice('Property owner published a property','A property owner submitted a property listing for admin approval.',{Owner:currentUser.name,Email:currentUser.email,Property:title,City:city,Price:money(price)},'admin');
...
function truck(){
 ...
 let driverQuick=`<div class="actions"><button class="btn ghost" onclick="document.getElementById('driverProfileCard')?.scrollIntoView({behavior:'smooth'})">Update Driver Profile</button></div>`;
...
/* V7.6.9 Home Services workflow: provider post -> admin approval -> customer request -> admin approval -> provider accept/decline */
const HOME_SERVICE_CATEGORIES=['Cleaning','Moving','Repair','Plumbing','Electrical','Painting','Yard Work','Handyman','Appliance Repair','Other'];
function mapHomeServicePost(row){return {dbId:row.id,id:row.local_ref||row.id,providerName:row.provider_name||'',providerEmail:row.provider_email||'',providerPhone:row.provider_phone||'',category:row.category||'Other',title:row.title||'',city:row.city||'',price:row.price||'',description:row.description||'',status:row.status||'Pending Admin Approval',createdAt:row.created_at?new Date(row.created_at).toLocaleString():'',approvedAt:row.approved_at?new Date(row.approved_at).toLocaleString():'',declinedAt:row.declined_at?new Date(row.declined_at).toLocaleString():''};}
function mapHomeServiceRequest(row){return {dbId:row.id,id:row.local_ref||row.id,serviceId:row.service_id||'',serviceLocalRef:row.service_local_ref||'',serviceTitle:row.service_title||'',providerName:row.provider_name||'',providerEmail:row.provider_email||'',customerName:row.customer_name||'',customerEmail:row.customer_email||'',customerPhone:row.customer_phone||'',city:row.city||'',preferredDate:row.preferred_date||'',details:row.details||'',status:row.status||'Pending Admin Approval',createdAt:row.created_at?new Date(row.created_at).toLocaleString():'',adminApprovedAt:row.admin_approved_at?new Date(row.admin_approved_at).toLocaleString():'',providerRespondedAt:row.provider_responded_at?new Date(row.provider_responded_at).toLocaleString():''};}
async function loadSupabaseHomeServices(){
 if(!authReady()){return;}
 let posts=await hcSupabase.from('home_service_posts').select('*').order('created_at',{ascending:false});
 if(!posts.error)data.homeServicePosts=(posts.data||[]).map(mapHomeServicePost);else console.warn('Home service posts load error',posts.error);
...
function homeServiceStatusBadge(st){return `<span class="pill ${st==='Approved'||st==='Available'||st==='Provider Accepted'||st==='Completed'?'good':st==='Declined'||st==='Provider Declined'?'bad':'warn'}">${st||'Pending'}</span>`}
function homeServices(){
 ...
 let providerForm=canPost?`<div class="card"><h3>Post Home Service</h3><p class="muted">Your service goes to admin first. After admin approves it, customers can request it.</p><label>Category</label><select id="hsCategory">${HOME_SERVICE_CATEGORIES.map(c=>`<option>${c}</option>`).join('')}</select><label>Service Title</label><input id="hsTitle" placeholder="Example: House cleaning, moving help"><label>City / Area</label><input id="hsCity" placeholder="Atlanta, Clarkston, Decatur"><label>Price</label><input id="hsPrice" placeholder="Example: $80, hourly, estimate"><label>Description</label><textarea id="hsDescription" rows="4" placeholder="Describe your service, availability, experience, and what customer should know"></textarea><button class="btn primary" onclick="postHomeService()">Submit Service for Admin Approval</button></div>`:'';
 let customerBox=canRequest?`<div class="card"><h3>Available Home Services</h3><p class="muted">Request a service. Admin reviews the request before sending it to the service provider.</p></div>`:'';
 ...
 let reqRows=myReqs.map(r=>{let canProviderAnswer=(r.providerEmail===currentUser.email&&r.status==='Pending Provider Review');return `<tr><td>${r.serviceTitle}</td><td>${r.customerName}<br><span class="small">${r.customerEmail}</span></td><td>${r.customerPhone||''}</td><td>${r.city||''}</td><td>${r.preferredDate||''}</td><td>${r.details||''}</td><td>${homeServiceStatusBadge(r.status)}</td><td>${canProviderAnswer?`<button class="btn primary" onclick="providerAcceptHomeServiceRequest('${r.id}')">Accept</button> <button class="btn bad" onclick="providerDeclineHomeServiceRequest('${r.id}')">Decline</button>`:'-'}</td></tr>`}).join('')||'<tr><td colspan="8">No home service requests yet.</td></tr>';
 $('home_services').innerHTML=`<h2>🛠️ Home Services</h2><div class="hero"><h1>Home Services</h1><p><b>Cleaners, movers, repair, electricians, plumbers, handymen, and local helpers.</b></p><p class="muted">Flow: provider posts service → admin approves → customer requests → admin approves → provider accepts/declines.</p></div><div class="grid">${providerForm}${customerBox}</div><h3>Approved Services</h3><table><tr><th>Category</th><th>Service</th><th>City</th><th>Price</th><th>Provider</th><th>Status</th><th>Action</th></tr>${serviceRows}</table><h3>My Service Posts</h3><table><tr><th>Service</th><th>Category</th><th>City</th><th>Price</th><th>Status</th><th>Created</th></tr>${postRows}</table><h3>My Home Service Requests</h3><table><tr><th>Service</th><th>Customer</th><th>Phone</th><th>City</th><th>Date</th><th>Details</th><th>Status</th><th>Action</th></tr>${reqRows}</table>`;
}
async function postHomeService(){if(!requireLogin())return;if(currentUser.role!=='service_provider'&&currentUser.role!=='business_owner'&&currentUser.role!=='admin')return alert('Only service providers, business owners, or admin can post home services. Add Home Service Provider in My Services first.');let title=($('hsTitle').value||'').trim(),category=$('hsCategory').value,city=($('hsCity').value||'').trim(),price=($('hsPrice').value||'').trim(),description=($('hsDescription').value||'').trim();if(!title||!city||!price||!description)return alert('Please complete category, title, city, price, and description.');let item={id:'HS'+Date.now().toString().slice(-6),providerName:currentUser.name,providerEmail:currentUser.email,providerPhone:currentUser.phone||'',category,title,city,price,description,status:currentUser.role==='admin'?'Approved':'Pending Admin Approval',createdAt:new Date().toLocaleString()};let res=await syncHomeServicePostToSupabase(item);if(res.error)return alert('Home service was not saved to Supabase. Please run the Home Services SQL, then try again. '+res.error.message);data.homeServicePosts.unshift(item);addNote(currentUser.email,'Home service submitted for admin approval: '+title+'.');sendAdminEmailNotice('Home service waiting for approval','A service provider submitted a new home service for admin approval.',{Provider:currentUser.name,Email:currentUser.email,Service:title,Category:category,City:city,Price:price},'admin');persistOnly();alert('Home service submitted. Admin approval is required before customers can request it.');homeServices();}
async function requestHomeService(id){if(!requireLogin())return;let svc=(data.homeServicePosts||[]).find(p=>p.id===id);if(!svc)return alert('Service not found.');if(svc.providerEmail===currentUser.email)return alert('You cannot request your own service.');let phone=prompt('Enter your phone number for this request',currentUser.phone||'');if(!phone)return;let city=prompt('Enter service city / address area','');if(!city)return;let preferredDate=prompt('Preferred date or time','');if(!preferredDate)return;let details=prompt('Describe what you need','');if(!details)return;let req={id:'HSR'+Date.now().toString().slice(-6),serviceId:svc.dbId||'',serviceLocalRef:svc.id,serviceTitle:svc.title,providerName:svc.providerName,providerEmail:svc.providerEmail,customerName:currentUser.name,customerEmail:currentUser.email,customerPhone:phone,city,preferredDate,details,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};let res=await syncHomeServiceRequestToSupabase(req,svc);if(res.error)return alert('Request was not saved to Supabase. Please run the Home Services SQL, then try again. '+res.error.message);data.homeServiceRequests.unshift(req);addNote(currentUser.email,'Home service request sent for admin approval: '+svc.title+'.');sendAdminEmailNotice('Home service request waiting for approval','A customer requested a home service. Admin approval is required before the provider receives it.',{Customer:currentUser.name,CustomerEmail:currentUser.email,Provider:svc.providerName,ProviderEmail:svc.providerEmail,Service:svc.title,City:city,PreferredDate:preferredDate},'admin');persistOnly();alert('Request sent. Admin approval is required before the provider receives it.');homeServices();}
