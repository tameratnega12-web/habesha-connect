...
const pages=['home','account','profile','services','shipping','rentals','marketplace','jobs','truck','business','messages','notifications','admin','about','contact','help','report','privacy','terms'];
const labels={home:'Home',account:'Account',profile:'Profile',services:'My Services',shipping:'Shipping',rentals:'Rentals',marketplace:'Marketplace',jobs:'Jobs',truck:'Trucking',business:'Business Manager',messages:'Messages',notifications:'Notifications',admin:'Admin',about:'About',contact:'Contact',help:'Help / FAQ',report:'Report Problem',privacy:'Privacy',terms:'Terms'};
...
const ROLE_PAGES={
 ...
 admin:['home','services','profile','shipping','rentals','marketplace','jobs','truck','business','messages','notifications','admin',...PUBLIC_PAGES],
 customer:['home','services','profile','marketplace','jobs','messages','notifications',...PUBLIC_PAGES],
 ...
 business_owner:['home','services','profile','business','marketplace','jobs','messages','notifications',...PUBLIC_PAGES]
};
...
async function show(p){
 ...
 try{
   ...
   let target=$(p);
   if(target){
     target.innerHTML='<div class="card"><h3>Could not open this page</h3><p class="muted">The app stopped while loading this dashboard. Please refresh once. Error: '+escapeHtml(msg)+'</p><button class="btn" onclick="location.reload()">Refresh App</button> <button class="btn ghost" onclick="show(\'home\')">Go Home</button></div>';
   }
 }finally{
   renderingPage=false;
...
async function shipping(){
 ...
 let tripBox = role==='traveler' ? `<div class="card"><h3>Post Traveler Trip</h3>${routeSelects('trip')}<label>Travel Date</label><input id="tripDate" type="date" min="${datePlusDays(5)}"><label>Available Space lb</label><input id="tripSpace" type="number" value="100"><p class="muted">Post your available luggage space. Sender and traveler can agree on details before payment.</p><button class="btn primary" onclick="addTrip()">Publish Trip</button></div>` : '';
 let shipFilter=($('shipSearch')&&$('shipSearch').value||'').toLowerCase();
 let visibleTrips=data.trips.filter(t=>{let remaining=Number(t.availableSpace ?? t.remaining_space_lb ?? 0);let status=String(t.status||'Open');let openForBooking=role==='admin'||(remaining>0 && !['Full','Closed','Cancelled','Canceled','Delivered','Completed'].includes(status));let matches=!shipFilter||String(t.route||'').toLowerCase().includes(shipFilter)||String(t.travelDate||'').includes(shipFilter)||String(t.fromCity||'').toLowerCase().includes(shipFilter)||String(t.toCity||'').toLowerCase().includes(shipFilter);return openForBooking&&matches;});
...
async function addTrip(){if(!requireLogin())return;if(currentUser.role!=='traveler'&&currentUser.role!=='admin')return alert('Only travelers can post trips.');let from=$('tripFrom').value,to=$('tripTo').value;if(from===to)return alert('From and To cannot be the same.');let tripDate=$('tripDate').value;let days=daysUntilDate(tripDate);if(days===null)return alert('Please select a travel date.');if(days<0)return alert('You cannot post a traveler trip for a previous day. Please choose a future date.');if(days<5)return alert('Traveler trips must be posted at least 5 days before the travel date.');let space=+$('tripSpace').value||0;if(space<=0)return alert('Enter available space.');let fee=0;if(!confirm('Publish this traveler trip for beta testing? Sender and traveler will agree on shipping details before payment.'))return;if(authReady()&&currentUser.id){let payload={traveler_id:currentUser.id,from_city:from,to_city:to,travel_date:tripDate,original_space_lb:space,remaining_space_lb:space,status:'Open',listing_fee_paid:true,listing_fee:fee,listing_fee_refunded:false,listing_fee_paid_at:new Date().toISOString()};let {data:row,error}=await hcSupabase.from('trips').insert(payload).select('*, profiles:traveler_id(name,email,phone,verified)').single();if(error){if(String(error.message||'').toLowerCase().includes('column'))return alert('Please run supabase/v6_7_traveler_listing_fee_migration.sql once, then try again.');return alert('Could not save trip to Supabase: '+error.message);}data.trips.unshift(mapDbTrip(row));addNote('all','New traveler trip posted by '+currentUser.name+' with '+space+' lb available.');addNote('admin@habeshaconnect.com','Traveler listing fee paid by '+currentUser.name+' for '+from+' → '+to+'.');save();show('shipping');return;}let id='T'+Date.now().toString().slice(-5);data.trips.unshift({id,route:from+' → '+to,travelDate:tripDate,totalSpace:space,availableSpace:space,traveler:currentUser.name,travelerPhone:currentUser.phone||'',travelerEmail:currentUser.email,travelerVerified:!!currentUser.verified,status:'Open',listingFeePaid:true,listingFeeAmount:fee,listingFeeRefunded:false,listingFeePaidAt:new Date().toLocaleString()});addNote('all','New traveler trip posted by '+currentUser.name+' with '+space+' lb available.');save();shipping()}
async function requestTrip(id){
 ...
 if(authReady()&&currentUser.id&&t.dbId){
   ...
   addNote(currentUser.email,'Space request sent to '+t.traveler+'.');
 ...
 addNote(t.travelerEmail,'New '+w+' lb space request from '+senderName+'.');addNote(currentUser.email,'Space request sent to '+t.traveler+'.');save();alert('Your request was entered correctly and submitted.');shipping()}
...
async function declineShip(id){let s=data.shipments.find(x=>x.id===id);if(!s)return;if(authReady()&&s.dbId){let {error}=await hcSupabase.from('shipments').update({status:'Declined'}).eq('id',s.dbId);if(error)return alert('Could not decline request: '+error.message);}s.status='Declined';addNote(s.senderEmail||'all','Your shipment request was declined.');save();shipping()}
...
async function addRental(){
  ...
  let moveDate=$('rentMoveDate').value;let moveDays=daysUntilDate(moveDate);if(moveDate&&moveDays<0)return alert('Property available date cannot be a previous day. Please choose today or a future date.');
  let fee=0;
  ...
  addNote('admin@habeshaconnect.com','Rental listing from '+currentUser.name+' needs approval.');
...
async function requestViewing(id){
 ...
 addNote('admin@habeshaconnect.com','New rental viewing request for '+r.title+' from '+currentUser.name+'.');
...
async function approveRentalReq(id){
 ...
 // Keep long-term rental records in owner/seeker dashboards until they delete them.
 if(r)r.status='Rented';
 let res={error:null};
 if(authReady()&&q.dbId)res=await hcSupabase.from('rental_requests').update({status:'Approved'}).eq('id',q.dbId);
 if(res.error){q.status=oldStatus;if(r)r.status=oldRentalStatus;return alert('Could not approve rental request: '+res.error.message)}
 if(authReady()&&r&&r.dbId){let propRes=await hcSupabase.from('properties').update({status:'Rented'}).eq('id',r.dbId);if(propRes.error)console.warn('Property status update failed',propRes.error);}
 addNote(q.seekerEmail,'Your rental request was approved. Owner contact is now available.');
 persistOnly();
 if(currentPage==='admin'){await refreshAdminData();adminSuccess('Rental request approved.');}else{await rentals();}
}
...
function truck(){
 ...
 let availableJobs=(data.truckJobs||[]).filter(j=>j.status!=='Closed');
 let myApps=(data.truckApplications||[]).filter(a=>role==='admin'||a.driverEmail===currentUser.email||a.ownerEmail===currentUser.email);
 ...
 let expiryWarnings=myTruckRecords.flatMap(t=>{let rows=[];let ins=daysUntilDate(t.insurance), reg=daysUntilDate(t.registration);if(ins!==null&&ins<0)rows.push(`Insurance expired for ${t.truck}.`);else if(ins!==null&&ins<=30)rows.push(`Insurance for ${t.truck} will expire in ${ins} days.`);if(reg!==null&&reg<0)rows.push(`Registration expired for ${t.truck}.`);else if(reg!==null&&reg<=30)rows.push(`Registration for ${t.truck} will expire in ${reg} days.`);return rows;});
 if(expiryWarnings.length && !sessionStorage.getItem('hc_truck_expiry_notice_'+currentUser.email)){sessionStorage.setItem('hc_truck_expiry_notice_'+currentUser.email,'1');setTimeout(()=>alert('Truck document notice:\n\n'+expiryWarnings.join('\n')),250);}
 let quickActions=(role==='truck_owner')?`<div class="actions"><button class="btn primary" onclick="document.getElementById('truckJobPost')?.scrollIntoView({behavior:'smooth'});document.getElementById('jobTitle')?.focus();">Post Job</button></div>`:(role==='driver')?`<div class="actions"><button class="btn primary" onclick="document.getElementById('availableTruckJobs')?.scrollIntoView({behavior:'smooth'});">See Jobs</button></div>`:'';
 let ownerPanel=`<div class="grid"><div class="card"><h3>🚛 Truck Owner Record</h3><label>Truck / Unit Number</label><input id="tTruck" placeholder="Unit 101 / Freightliner Cascadia"><label>License Plate</label><input id="tPlate" placeholder="Plate number"><label>VIN</label><input id="tVin" placeholder="VIN"><label>Driver Name</label><input id="tDriver" placeholder="Assigned driver"><label>Driver Email</label><input id="tDriverEmail" placeholder="driver@email.com"><label>Insurance Expiration</label><input id="tInsurance" type="date"><label>Registration Expiration</label><input id="tRegistration" type="date"><label>Maintenance / Notes</label><textarea id="tMaintenance" placeholder="Oil change, tires, brakes, repair notes"></textarea><div class="row"><div><label>Income</label><input id="tIncome" type="number" placeholder="Income"></div><div><label>Expenses</label><input id="tExpenses" type="number" placeholder="Fuel, toll, repair, permit expenses"></div></div><div class="row"><div><label>Mileage</label><input id="tMileage" type="number" placeholder="Mileage"></div><div><label>Status / Tracking</label><select id="tStatus"><option>Available</option><option>Assigned</option><option>Pickup</option><option>On the road</option><option>Delivered</option><option>Maintenance</option></select></div></div><button class="btn primary" onclick="addTruck()">Save Truck Record</button></div><div class="card" id="truckJobPost"><h3>📢 Post Driver Job</h3><label>Job Title</label><input id="jobTitle" placeholder="CDL Driver Needed"><label>Location / Route</label><input id="jobRoute" placeholder="Atlanta to regional / local"><label>Pay</label><input id="jobPay" placeholder="$1,500 weekly or 30%"><label>Schedule</label><input id="jobSchedule" placeholder="Full time, weekends, local"><label>Requirements</label><textarea id="jobReq" placeholder="CDL class, experience, clean MVR, language, documents"></textarea><label>Truck Details</label><textarea id="jobTruck" placeholder="Truck type, trailer, automatic/manual, owner operator/company driver"></textarea><button class="btn primary" onclick="postTruckJob()">Post Driver Hiring Job</button></div></div>`;
 let driverPanel=`<div class="grid"><div class="card"><h3>👤 Driver Job Profile</h3><label>Full Name</label><input id="dpName" value="${driverProfile?.name||currentUser.name||''}"><label>Phone</label><input id="dpPhone" value="${driverProfile?.phone||currentUser.phone||''}"><label>City</label><input id="dpCity" value="${driverProfile?.city||currentUser.city||''}"><label>CDL / License Type</label><input id="dpLicense" value="${driverProfile?.license||''}" placeholder="CDL A, CDL B, Box truck"><label>Experience</label><input id="dpExperience" value="${driverProfile?.experience||''}" placeholder="5 years, local, OTR"><label>Looking For</label><select id="dpLooking"><option ${driverProfile?.looking==='Full Time'?'selected':''}>Full Time</option><option ${driverProfile?.looking==='Part Time'?'selected':''}>Part Time</option><option ${driverProfile?.looking==='Owner Operator'?'selected':''}>Owner Operator</option><option ${driverProfile?.looking==='Box Truck'?'selected':''}>Box Truck</option></select><label>Insurance Expiration</label><input id="dpInsurance" type="date" value="${driverProfile?.insurance||''}"><label>Registration Expiration</label><input id="dpRegistration" type="date" value="${driverProfile?.registration||''}"><label>Skills / Notes</label><textarea id="dpNotes" placeholder="Routes, languages, availability, safety record">${driverProfile?.notes||''}</textarea><button class="btn primary" onclick="saveDriverProfile()">Save Driver Profile</button></div><div class="card"><h3>🔎 Driver Job Search</h3><p>Drivers can apply to truck owner posts. Owner will see full driver details and can approve or decline.</p><button class="btn primary" onclick="document.getElementById('availableTruckJobs')?.scrollIntoView({behavior:'smooth'});">See Jobs</button></div></div>`;
 let jobsTable=`<h3 id="availableTruckJobs">🚛 Owner Hiring Posts — Drivers See This First</h3><table><tr><th>Owner</th><th>Job</th><th>Route</th><th>Pay</th><th>Schedule</th><th>Requirements</th><th>Action</th></tr>${availableJobs.map(j=>{let already=(data.truckApplications||[]).find(a=>a.jobId===j.id&&a.driverEmail===currentUser.email);let action=role==='driver'?(already?`<span class="pill warn">${already.status}</span>`:`<button class="btn primary" onclick="applyTruckJob('${j.id}')">Apply</button>`):(role==='truck_owner'&&j.ownerEmail===currentUser.email?`<span class="pill good">Owner Post</span>`:'-');return `<tr><td>${j.ownerName}<br><span class="small">${j.ownerPhone||''}</span></td><td>${j.title}<br><span class="small">${j.truckDetails||''}</span></td><td>${j.route}</td><td>${j.pay}</td><td>${j.schedule}</td><td>${j.requirements}</td><td>${action}</td></tr>`}).join('')||'<tr><td colspan="7">No truck owner hiring posts yet.</td></tr>'}</table>`;
 let driverProfilesTable=`<h3>👤 Drivers Looking for Truck Driving Jobs — Owners See This First</h3><table><tr><th>Driver</th><th>Phone</th><th>City</th><th>License</th><th>Experience</th><th>Looking For</th><th>Insurance</th><th>Registration</th><th>Skills / Notes</th></tr>${driverProfiles.map(p=>`<tr><td>${p.name||p.driverName||'Driver'}<br><span class="small">${p.driverEmail||''}</span></td><td>${p.phone||''}</td><td>${p.city||''}</td><td>${p.license||''}</td><td>${p.experience||''}</td><td>${p.looking||''}</td><td>${expiryBadge(p.insurance,'Insurance')}</td><td>${expiryBadge(p.registration,'Registration')}</td><td>${p.notes||''}</td></tr>`).join('')||'<tr><td colspan="9">No driver looking-for-job posts yet.</td></tr>'}</table>`;
 let appsTable=`<h3>Driver Applications / Hiring Records</h3><table><tr><th>Job</th><th>Driver</th><th>Phone</th><th>License</th><th>Experience</th><th>Looking For</th><th>Notes</th><th>Status</th><th>Action</th></tr>${myApps.map(a=>{let canManage=(role==='truck_owner'||role==='admin')&&a.ownerEmail===currentUser.email&&a.status==='Pending';if(role==='admin'&&a.status==='Pending')canManage=true;return `<tr><td>${a.jobTitle}</td><td>${a.driverName}<br><span class="small">${a.driverEmail}</span></td><td>${a.driverPhone}</td><td>${a.license}</td><td>${a.experience}</td><td>${a.looking||''}</td><td>${a.notes||''}</td><td><span class="pill ${a.status==='Approved'?'good':a.status==='Declined'?'bad':'warn'}">${a.status}</span></td><td>${canManage?`<button class="btn primary" onclick="approveTruckApp('${a.id}')">Approve / Hire</button> <button class="btn bad" onclick="declineTruckApp('${a.id}')">Decline</button>`:'-'}</td></tr>`}).join('')||'<tr><td colspan="9">No applications yet.</td></tr>'}</table>`;
 ...
 let summary=`<div class="grid"><div class="card"><p>Total Truck Records</p><div class="stat">${myTruckRecords.length}</div></div><div class="card"><p>Open Driver Jobs</p><div class="stat">${availableJobs.length}</div></div><div class="card"><p>Applications</p><div class="stat">${myApps.length}</div></div><div class="card"><p>Truck Profit</p><div class="stat">${money(myTruckRecords.reduce((a,t)=>a+(+t.income||0)-(+t.expenses||0),0))}</div></div></div>`;
 let warningPanel=expiryWarnings.length?`<div class="notice"><b>Document alerts:</b><br>${expiryWarnings.map(w=>'• '+w).join('<br>')}</div>`:'';
 let topMatchPanels=(role==='driver')?jobsTable:(role==='truck_owner')?driverProfilesTable:driverProfilesTable+jobsTable;
 $('truck').innerHTML=`<h2>🚚 Trucking Connect</h2>${quickActions}${warningPanel}${topMatchPanels}${summary}${(role==='truck_owner'||role==='admin')?ownerPanel:''}${(role==='driver'||role==='admin')?driverPanel:''}${appsTable}${recordsTable}`;
}
function addTruck(){if(!requireLogin())return;if(currentUser.role!=='truck_owner'&&currentUser.role!=='admin')return alert('Only truck owners or admin can save truck owner records.');let truck=($('tTruck').value||'').trim();if(!truck)return alert('Truck / Unit is required.');data.trucks.unshift({id:'TRK'+Date.now().toString().slice(-6),ownerName:currentUser.name,ownerEmail:currentUser.email,ownerPhone:currentUser.phone||'',truck,plate:($('tPlate').value||'').trim(),vin:($('tVin').value||'').trim(),driver:($('tDriver').value||'Unassigned').trim(),driverEmail:cleanEmail($('tDriverEmail').value),insurance:$('tInsurance').value,registration:$('tRegistration').value,maintenance:($('tMaintenance').value||'').trim(),status:$('tStatus').value,income:+$('tIncome').value||0,expenses:+$('tExpenses').value||0,mileage:+$('tMileage').value||0,createdAt:new Date().toLocaleString()});let ins=daysUntilDate($('tInsurance').value),reg=daysUntilDate($('tRegistration').value);let notes=[];if(ins!==null&&ins<0)notes.push('Insurance is expired.');else if(ins!==null&&ins<=30)notes.push('Insurance will expire in '+ins+' days.');if(reg!==null&&reg<0)notes.push('Registration is expired.');else if(reg!==null&&reg<=30)notes.push('Registration will expire in '+reg+' days.');addNote(currentUser.email,'Truck record saved for '+truck+'.');save();if(notes.length)alert(notes.join('\n'));truck()}
function postTruckJob(){if(!requireLogin())return;if(currentUser.role!=='truck_owner'&&currentUser.role!=='admin')return alert('Only truck owners can post driver hiring jobs.');let title=($('jobTitle').value||'').trim(),route=($('jobRoute').value||'').trim(),pay=($('jobPay').value||'').trim();if(!title||!route||!pay)return alert('Job title, route/location, and pay are required.');data.truckJobs.unshift({id:'TJ'+Date.now().toString().slice(-6),ownerName:currentUser.name,ownerEmail:currentUser.email,ownerPhone:currentUser.phone||'',title,route,pay,schedule:($('jobSchedule').value||'').trim(),requirements:($('jobReq').value||'').trim(),truckDetails:($('jobTruck').value||'').trim(),status:'Open',createdAt:new Date().toLocaleString()});addNote(currentUser.email,'Driver hiring job posted: '+title+'.');save()}
function saveDriverProfile(){if(!requireLogin())return;if(currentUser.role!=='driver'&&currentUser.role!=='admin')return alert('Only drivers or admin can save a driver job profile.');let profile={driverName:($('dpName').value||currentUser.name).trim(),driverEmail:currentUser.email,name:($('dpName').value||currentUser.name).trim(),phone:($('dpPhone').value||currentUser.phone||'').trim(),city:($('dpCity').value||currentUser.city||'').trim(),license:($('dpLicense').value||'').trim(),experience:($('dpExperience').value||'').trim(),looking:$('dpLooking').value,insurance:$('dpInsurance')?$('dpInsurance').value:'',registration:$('dpRegistration')?$('dpRegistration').value:'',notes:($('dpNotes').value||'').trim(),updatedAt:new Date().toLocaleString()};let old=(data.truckDriverProfiles||[]).findIndex(p=>p.driverEmail===currentUser.email);if(old>=0)data.truckDriverProfiles[old]=profile;else data.truckDriverProfiles.unshift(profile);addNote(currentUser.email,'Driver job profile saved.');save()}
function applyTruckJob(id){if(!requireLogin())return;if(currentUser.role!=='driver')return alert('Only truck drivers can apply for truck driving jobs.');let j=(data.truckJobs||[]).find(x=>x.id===id);if(!j)return alert('Job not found.');if((data.truckApplications||[]).some(a=>a.jobId===id&&a.driverEmail===currentUser.email))return alert('You already applied for this job.');let p=(data.truckDriverProfiles||[]).find(x=>x.driverEmail===currentUser.email)||{};if(!p.license||!p.experience){alert('Please complete your Driver Job Profile before applying.');return;}data.truckApplications.unshift({id:'TA'+Date.now().toString().slice(-6),jobId:j.id,jobTitle:j.title,ownerName:j.ownerName,ownerEmail:j.ownerEmail,driverName:p.name||currentUser.name,driverEmail:currentUser.email,driverPhone:p.phone||currentUser.phone||'',city:p.city||'',license:p.license||'',experience:p.experience||'',looking:p.looking||'',notes:p.notes||'',insurance:p.insurance||'',registration:p.registration||'',status:'Pending',createdAt:new Date().toLocaleString()});addNote(j.ownerEmail,'New driver application for '+j.title+' from '+(p.name||currentUser.name)+'.');addNote(currentUser.email,'Application sent for '+j.title+'.');save();alert('Application sent. The truck owner can now see your driver information in their dashboard.');truck()}
function approveTruckApp(id){let a=(data.truckApplications||[]).find(x=>x.id===id);if(!a)return alert('Application not found.');if(currentUser.role!=='admin'&&a.ownerEmail!==currentUser.email)return alert('Only the truck owner can approve this application.');a.status='Approved';a.approvedAt=new Date().toLocaleString();addNote(a.driverEmail,'Your truck driving application for '+a.jobTitle+' was approved.');addNote(a.ownerEmail,'You approved '+a.driverName+' for '+a.jobTitle+'.');save()}
function declineTruckApp(id){let a=(data.truckApplications||[]).find(x=>x.id===id);if(!a)return alert('Application not found.');if(currentUser.role!=='admin'&&a.ownerEmail!==currentUser.email)return alert('Only the truck owner can decline this application.');a.status='Declined';a.declinedAt=new Date().toLocaleString();addNote(a.driverEmail,'Your truck driving application for '+a.jobTitle+' was declined.');save()}
...
async function admin(){
 ...
 <h3>Truck Owner / Truck Driver Management</h3><div class="grid"><div class="card"><p>Truck Owners</p><div class="stat">${managedUsers.filter(u=>(u.roles||[]).includes('truck_owner')||u.role==='truck_owner').length}</div></div><div class="card"><p>Truck Drivers</p><div class="stat">${managedUsers.filter(u=>(u.roles||[]).includes('driver')||u.role==='driver').length}</div></div><div class="card"><p>Truck Jobs</p><div class="stat">${(data.truckJobs||[]).length}</div></div><div class="card"><p>Driver Applications</p><div class="stat">${(data.truckApplications||[]).length}</div></div></div>
 <table><tr><th>Type</th><th>Name / Title</th><th>Contact</th><th>Details</th><th>Status</th></tr>${(data.truckJobs||[]).map(j=>`<tr><td>Owner Job Post</td><td>${j.title}</td><td>${j.ownerName}<br>${j.ownerPhone||''}<br>${j.ownerEmail||''}</td><td>${j.route} • ${j.pay}<br>${j.requirements||''}</td><td>${j.status||'Open'}</td></tr>`).join('')}${(data.truckApplications||[]).map(a=>`<tr><td>Driver Application</td><td>${a.driverName}</td><td>${a.driverPhone||''}<br>${a.driverEmail||''}</td><td>${a.jobTitle}<br>${a.license||''} • ${a.experience||''}</td><td>${a.status||'Pending'}</td></tr>`).join('')}${(data.trucks||[]).map(t=>`<tr><td>Truck Record</td><td>${t.truck}</td><td>${t.ownerName}<br>${t.ownerPhone||''}<br>${t.ownerEmail||''}</td><td>Plate ${t.plate||''} • Driver ${t.driver||''}<br>${expiryBadge(t.insurance,'Insurance')} ${expiryBadge(t.registration,'Registration')}</td><td>${t.status||'Available'}</td></tr>`).join('')||'<tr><td colspan="5">No trucking records yet.</td></tr>'}</table>
...
async function approveShipment(id){
  ...
  addNote(s.senderEmail||'all','Your shipment '+(s.tracking||id)+' was approved by admin. Traveler contact is now available.');
  if(s.travelerEmail)addNote(s.travelerEmail,'Shipment '+(s.tracking||id)+' was approved by admin. Sender and receiver contact are now available.');
  persistOnly(); await refreshAdminData(); adminSuccess();
}
async function markDelivered(id){
  let s=data.shipments.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));
  if(!s)return alert('Shipment not found.');
  if(!confirm('Mark shipment '+(s.tracking||id)+' as delivered/completed? This will close the related traveler trip and remove remaining space from sender dashboards.'))return;
  let old=s.status;s.status='Delivered';
  let trip=s.tripId?data.trips.find(t=>String(t.id)===String(s.tripId)||String(t.dbId)===String(s.tripId)):null;
  let oldTrip=trip?{status:trip.status,availableSpace:trip.availableSpace}:null;
  if(trip){trip.status='Delivered';trip.availableSpace=0;}
  let res={error:null};
  if(authReady()&&s.dbId)res=await hcSupabase.from('shipments').update({status:'Delivered'}).eq('id',s.dbId);
  if(!res.error&&authReady()&&trip&&trip.dbId)res=await hcSupabase.from('trips').update({status:'Delivered',remaining_space_lb:0}).eq('id',trip.dbId);
  if(res.error){s.status=old;if(trip&&oldTrip)Object.assign(trip,oldTrip);return alert('Could not mark delivered in Supabase: '+res.error.message)}
  addNote('admin@habeshaconnect.com','Shipment '+(s.tracking||id)+' was marked delivered and the traveler trip was closed.');
  persistOnly(); await refreshAdminData(); adminSuccess();
}
...
async function approveRental(id){
  ...
  addNote(r.ownerEmail||'all','Your rental listing '+id+' was approved by admin.');
