...
const labels={home:'Home',account:'Account',profile:'Profile',services:'My Services',shipping:'Shipping',rentals:'Rentals',marketplace:'Marketplace',jobs:'Jobs',truck:'Trucking',home_services:'Home Services',business:'Business Manager',messages:'Messages',notifications:'Notifications',admin:'Admin',about:'About',contact:'Contact',help:'Help / FAQ',report:'Report Problem',privacy:'Privacy',terms:'Terms'};
...
if(!localStorage.getItem(BETA_CLEANUP_KEY)){
  ['payments','messages','notifications','favorites','reviews','shipments','trips','rentals','rentalRequests','market','jobs','trucks','truckJobs','truckApplications','truckDriverProfiles','trailerRentals','homeServicePosts','homeServiceRequests','businesses'].forEach(k=>{data[k]=[];});
...
function roleDashboardCards(){
 ...
 if(currentUser.role==='truck_owner')return `<div class="grid"><div class="card"><h3>📢 Post Driver Hiring Job</h3><p class="muted">Create a driver job post from your profile. Admin approval is still required before drivers can apply.</p><button class="btn primary" onclick="openTruckSection('jobPostCard')">Post Driver Hiring Job</button></div><div class="card"><h3>👥 Driver Applicants</h3><p class="muted">Review admin-approved driver applications for your jobs.</p><button class="btn" onclick="openTruckSection('truckApplicationsTable')">See Driver Applicants</button></div><div class="card"><h3>🚛 Post Trailer Rent</h3><p class="muted">Post your trailer for rent and manage trailer rental requests.</p><button class="btn" onclick="openTruckSection('trailerRentCard')">Post Trailer Rent</button></div><div class="card"><h3>🔎 Available Trailers for Rent</h3><p class="muted">See trailers posted by other owners that are available to rent.</p><button class="btn" onclick="openTruckSection('trailerRentalsTable')">Available Trailers for Rent</button></div></div>`;
