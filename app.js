...
async function shipping(){
 ...
 let createBox = '';
 let tripBox = role==='traveler' ? `<div class="card"><h3>Post Traveler Trip</h3>${routeSelects('trip')}<label>Travel Date</label><input id="tripDate" type="date"><label>Available Space lb</label><input id="tripSpace" type="number" value="100"><p class="muted">Publishing a trip costs ${money(settings().travelerTripListingFee)}. If you complete at least one approved shipment, this fee is added back to your traveler payout.</p><button class="btn primary" onclick="addTrip()">Pay ${money(settings().travelerTripListingFee)} & Publish Trip</button></div>` : '';
...
async function requestTrip(id){
 ...
 if(authReady()&&currentUser.id&&t.dbId){
   let payload={tracking_number:'HC-'+Date.now().toString().slice(-6),sender_name:senderName||currentUser.name,sender_phone:senderPhone||currentUser.phone,sender_id:currentUser.id,traveler_id:t.travelerId||null,trip_id:t.dbId,from_city:t.fromCity||'Atlanta, GA',to_city:t.toCity||'Addis Ababa',weight_lb:w,receiver_name:receiver||'Receiver not entered',receiver_phone:receiverPhone||'',paid:false,status:'Requested'};
