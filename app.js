...
function requestHomeService(id){
 if(!requireLogin())return;
 let svc=(data.homeServicePosts||[]).find(p=>p.id===id);
 ...
 activeHomeServiceRequestId=id;
 homeServices();
 setTimeout(()=>{let el=$('homeServiceRequestForm');if(el)el.scrollIntoView({behavior:'smooth',block:'start'});},50);
}
...
async function submitHomeServiceRequest(){
 ...
 let req={id:'HSR'+Date.now().toString().slice(-6),serviceId:svc.dbId||'',serviceLocalRef:svc.id,serviceTitle:svc.title,providerName:svc.providerName,providerEmail:svc.providerEmail,customerName:currentUser.name,customerEmail:currentUser.email,customerPhone:phone,city,preferredDate,details,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};
 ...
 sendAdminEmailNotice('Home service request waiting for approval','A customer requested a home service. Admin approval is required before the provider receives it.',{Customer:currentUser.name,CustomerEmail:currentUser.email,Provider:svc.providerName,ProviderEmail:svc.providerEmail,Service:svc.title,City:city,PreferredDate:preferredDate},'admin');
