...
async function shipping(){
 ...
 let visibleTrips=data.trips.filter(t=>(role==='admin'||t.status==='Open')&&(!shipFilter||String(t.route||'').toLowerCase().includes(shipFilter)||String(t.travelDate||'').includes(shipFilter)||String(t.fromCity||'').toLowerCase().includes(shipFilter)||String(t.toCity||'').toLowerCase().includes(shipFilter)));
