...
async function shipping(){
 ...
 let terminalStatuses=['Completed','Delivered'];
...
async function rentals(){
 ...
 if(role==='owner'){
   ...
   $('rentals').innerHTML=`<h2>🏠 Rental Connect</h2>${paymentPrivacyNotice()}${searchFilters}<h3>Available Owners / Rentals</h3><div id="rentalList" class="grid">${rentalCards(visibleRentals,role)}</div>${seekerRequestsHtml}${seekerSummary}`;
 } else if(role==='admin'){
   ...
   $('rentals').innerHTML=`<h2>🏠 Rental Connect</h2>${paymentPrivacyNotice()}${searchFilters}<h3>Available Owners / Rentals</h3><div id="rentalList" class="grid">${rentalCards(visibleRentals,role)}</div>`;
 }
}
