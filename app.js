...
const labels={home:'Home',account:'Account',profile:'Profile',services:'My Services',shipping:'Shipping',rentals:'Rentals',marketplace:'Marketplace',jobs:'Jobs',truck:'Trucking',home_services:'Home Services',taxi:'Taxi',business:'Business Manager',messages:'Messages',notifications:'Notifications',admin:'Admin',about:'About',contact:'Contact',help:'Help / FAQ',report:'Report Problem',privacy:'Privacy',terms:'Terms'};
...
if(!localStorage.getItem(BETA_CLEANUP_KEY)){
  ['payments','messages','notifications','favorites','reviews','shipments','trips','rentals','rentalRequests','market','jobs','trucks','truckJobs','truckApplications','truckDriverProfiles','trailerRentals','homeServicePosts','homeServiceRequests','taxiDriverApps','taxiRideRequests','businesses'].forEach(k=>{data[k]=[];});
...
const ROLE_PAGES={
 guest:['home','account','shipping','rentals','marketplace','jobs','truck','home_services','taxi','business',...PUBLIC_PAGES],
 traveler:['home','services','profile','shipping','taxi','messages','notifications',...PUBLIC_PAGES],
 sender:['home','services','profile','shipping','taxi','messages','notifications',...PUBLIC_PAGES],
 owner:['home','services','profile','rentals','taxi','messages','notifications',...PUBLIC_PAGES],
 rent_seeker:['home','services','profile','rentals','taxi','messages','notifications',...PUBLIC_PAGES],
 ...
 customer:['home','services','profile','home_services','taxi','marketplace','jobs','messages','notifications',...PUBLIC_PAGES],
...
const ROLE_NAV_PAGES={
 ...
 traveler:['home','profile','services','shipping','taxi'],
 sender:['home','profile','services','shipping','taxi'],
 owner:['home','profile','services','rentals','taxi'],
 rent_seeker:['home','profile','services','rentals','taxi'],
 truck_owner:['home','profile','services','truck','taxi'],
 driver:['home','profile','services','truck','taxi'],
...
function marketplace(){
 ...
 <div class="grid two">
  <div class="card"><h3>➕ Post Item for Sale</h3>
   ...
   <label>Photo Names / Links</label><input id="mPhotos" placeholder="Optional: photo1, photo2, photo3">
...
function addMarket(){if(!requireLogin())return;let title=($('mTitle').value||'').trim(),price=+$('mPrice').value||0,city=($('mCity').value||'').trim();if(!title||!price||!city)return alert('Please enter item title, price, and city.');let item={id:'M'+Date.now().toString().slice(-5),title,category:$('mCategory').value,condition:$('mCondition').value,price,city,description:$('mDesc').value||'',photos:($('mPhotos').value||'').split(',').map(x=>x.trim()).filter(Boolean).slice(0,10),seller:currentUser.name,sellerEmail:currentUser.email,sellerPhone:currentUser.phone||'',status:currentUser.role==='admin'?'Approved':'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.market.unshift(item);addNote('admin.habeshaconnect@gmail.com','Marketplace listing waiting for approval: '+title+' by '+currentUser.name);if(typeof sendAdminEmailNotice==='function')sendAdminEmailNotice('Marketplace listing waiting for approval','A seller posted a marketplace item.',{Seller:currentUser.name,Email:currentUser.email,Item:title,Price:money(price),City:city,Status:item.status},'marketplace');persistOnly();marketplace();alert('Item submitted. Admin must approve before buyers can see it.');}
...
function taxiAdminHtml(){
 let apps=data.taxiDriverApps||[], rides=data.taxiRideRequests||[];
 return `<h3>🚕 Taxi Management</h3><div class="notice"><b>Taxi flow:</b> Driver application → admin approval. Rider request → admin approval. Driver accept → admin approval. Complete ride → admin verification.</div><h4>Taxi Driver Applications</h4><table><tr><th>Driver</th><th>Phone</th><th>Vehicle</th><th>Status</th><th>Action</th></tr>${apps.map(a=>`<tr><td>${a.name}<br><span class="small">${a.email}</span></td><td>${a.phone||''}</td><td>${a.vehicle||''}<br><span class="small">License: ${a.license||''} • Insurance: ${a.insurance||''}</span></td><td>${taxiBadge(a.status)}</td><td>${a.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTaxiDriver('${a.id}')">Approve</button> <button class="btn bad" onclick="adminDeclineTaxiDriver('${a.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="5">No taxi driver applications yet.</td></tr>'}</table><h4>Taxi Ride Requests</h4><table><tr><th>Rider</th><th>Pickup</th><th>Destination</th><th>Driver</th><th>Status</th><th>Action</th></tr>${rides.map(r=>`<tr><td>${r.riderName}<br><span class="small">${r.riderEmail} • ${r.riderPhone||''}</span></td><td>${r.pickup}<br><span class="small">${r.pickupTime||'Now'}</span></td><td>${r.destination}<br><span class="small">Passengers: ${r.passengers||1}</span></td><td>${r.driverName||'Not assigned'}<br><span class="small">${r.driverPhone||''}</span></td><td>${taxiBadge(r.status)}</td><td>${taxiAdminActions(r)}</td></tr>`).join('')||'<tr><td colspan="6">No taxi ride requests yet.</td></tr>'}</table>`;
}
...
function taxi(){
 ...
 let isDriver=currentUser.role==='taxi_driver'||(myApp&&myApp.status==='Approved');
 let approvedDrivers=apps.filter(a=>a.status==='Approved');
 ...
 let riderForm=`<div class="card"><h3>Request Taxi</h3><p class="muted">Your taxi request goes to admin first. After admin approves, approved taxi drivers can accept it.</p><label>Pickup Location</label><input id="taxiPickup" placeholder="Address or pickup area"><label>Destination</label><input id="taxiDest" placeholder="Where are you going?"><div class="row"><div><label>Pickup Time</label><select id="taxiTime"><option>Now</option><option>Schedule</option></select></div><div><label>Passengers</label><input id="taxiPassengers" type="number" value="1" min="1"></div></div><label>Phone Number</label><input id="taxiPhone" value="${currentUser.phone||''}" placeholder="404-555-1234"><label>Ride Note</label><textarea id="taxiNote" placeholder="Gate code, luggage, airport terminal, etc."></textarea><button class="btn primary" onclick="submitTaxiRide()">Submit Taxi Request</button></div>`;
 let driverForm='';
 if(currentUser.role==='taxi_driver'){driverForm=!myApp?`<div class="card"><h3>Apply as Taxi Driver</h3><p class="muted">Submit your driver information for admin approval.</p><label>Vehicle</label><input id="taxiVehicle" placeholder="Toyota Camry 2018"><label>Driver License Number</label><input id="taxiLicense" placeholder="License number"><label>Insurance Information</label><input id="taxiInsurance" placeholder="Insurance company / policy"><label>Service Area</label><input id="taxiArea" placeholder="Atlanta, Clarkston, airport..."><button class="btn primary" onclick="applyTaxiDriver()">Submit Driver Application</button></div>`:`<div class="card"><h3>Taxi Driver Status</h3><p>Status: ${taxiBadge(myApp.status)}</p><p class="muted">${myApp.status==='Approved'?'You can see approved ride requests below.':'Wait for admin approval before accepting rides.'}</p></div>`;}
 ...
 let myRows=myRides.map(r=>`<tr><td>${r.riderName}</td><td>${r.pickup}</td><td>${r.destination}</td><td>${r.driverName||'Waiting'}</td><td>${taxiBadge(r.status)}</td><td>${taxiRideActions(r)}</td></tr>`).join('')||'<tr><td colspan="6">No taxi rides yet.</td></tr>';
 $('taxi').innerHTML=`<h2>🚕 Taxi</h2><div class="hero"><h1>Habesha Taxi</h1><p><b>Request a taxi or drive for the community.</b></p><p class="muted">Admin approval is required between important steps for safety and control.</p></div><div class="grid">${riderForm}${driverForm}</div>${isDriver?`<h3>Available Ride Requests</h3><table><tr><th>Pickup</th><th>Destination</th><th>Rider Phone</th><th>Time</th><th>Action</th></tr>${availableRows}</table>`:''}<h3>My Taxi Rides</h3><table><tr><th>Rider</th><th>Pickup</th><th>Destination</th><th>Driver</th><th>Status</th><th>Action</th></tr>${myRows}</table>`;
}
...
async function applyTaxiDriver(){let vehicle=($('taxiVehicle').value||'').trim(),license=($('taxiLicense').value||'').trim(),insurance=($('taxiInsurance').value||'').trim(),area=($('taxiArea').value||'').trim();if(!vehicle||!license||!insurance||!area)return alert('Please complete vehicle, license, insurance, and service area.');let app={id:'TD'+Date.now().toString().slice(-6),name:currentUser.name,email:currentUser.email,phone:currentUser.phone||'',vehicle,license,insurance,area,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.taxiDriverApps.unshift(app);addNote('admin.habeshaconnect@gmail.com','Taxi driver application needs approval: '+currentUser.name);sendAdminEmailNotice('Taxi driver application waiting for approval','A taxi driver submitted an application.',{Driver:currentUser.name,Email:currentUser.email,Phone:currentUser.phone||'',Vehicle:vehicle,Area:area},'admin');persistOnly();alert('Taxi driver application submitted. Admin approval is required.');taxi();}
async function submitTaxiRide(){let pickup=($('taxiPickup').value||'').trim(),destination=($('taxiDest').value||'').trim(),pickupTime=$('taxiTime').value,passengers=($('taxiPassengers').value||'1'),phone=($('taxiPhone').value||'').trim(),note=($('taxiNote').value||'').trim();if(!pickup||!destination||!phone)return alert('Please complete pickup, destination, and phone number.');let ride={id:'TX'+Date.now().toString().slice(-6),riderName:currentUser.name,riderEmail:currentUser.email,riderPhone:phone,pickup,destination,pickupTime,passengers,note,status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};data.taxiRideRequests.unshift(ride);addNote('admin.habeshaconnect@gmail.com','Taxi ride request needs approval from '+currentUser.name);sendAdminEmailNotice('Taxi ride request waiting for approval','A rider submitted a taxi request. Admin approval is required before drivers see it.',{Rider:currentUser.name,Email:currentUser.email,Phone:phone,Pickup:pickup,Destination:destination,Passengers:passengers,Note:note},'admin');persistOnly();alert('Taxi request submitted. Admin must approve before drivers can accept.');taxi();}
...
function driverAcceptTaxi(id){let r=findTaxiRide(id),a=findTaxiApp(currentUser.email);if(!r)return alert('Taxi ride not found.');
 if(!a)return alert('Admin must approve your taxi driver application before you accept rides.');r.driverName=currentUser.name;r.driverEmail=currentUser.email;r.driverPhone=currentUser.phone||a.phone||'';r.vehicle=a.vehicle;r.status='Driver Accepted - Waiting Admin Approval';addNote('admin.habeshaconnect@gmail.com','Taxi driver accepted ride '+id+'. Admin approval is required.');sendAdminEmailNotice('Taxi driver acceptance waiting for approval','A taxi driver accepted a ride. Admin approval is required before contact details are shared.',{Ride:id,Driver:currentUser.name,Rider:r.riderName,Pickup:r.pickup,Destination:r.destination},'admin');persistOnly();alert('You accepted the ride. Admin approval is required before the rider receives your information.');taxi();}
