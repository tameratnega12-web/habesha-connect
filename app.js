...
function truckAdminManagementHtml(){
 let trucks=data.trucks||[], jobs=data.truckJobs||[], apps=data.truckApplications||[], profiles=data.truckDriverProfiles||[];
 return `<h3>🚛 Truck Owner / Truck Driver Management</h3>
 <div class="grid"><div class="card"><p>Truck Owners / Trucks</p><div class="stat">${trucks.length}</div></div><div class="card"><p>Driver Job Posts</p><div class="stat">${jobs.length}</div></div><div class="card"><p>Driver Applications</p><div class="stat">${apps.length}</div></div><div class="card"><p>Driver Profiles</p><div class="stat">${profiles.length}</div></div></div>
...
function truck(){
 ...
 let ownerQuick=`<div class="actions"><button class="btn primary" onclick="document.getElementById('jobPostCard')?.scrollIntoView({behavior:'smooth'})">Post Job</button><button class="btn ghost" onclick="document.getElementById('driverProfilesTable')?.scrollIntoView({behavior:'smooth'})">See Drivers Looking for Jobs</button></div>`;
 ...
 $('truck').innerHTML=`<h2>🚚 Trucking Connect</h2>${role==='truck_owner'?ownerQuick:''}${role==='driver'?driverQuick:''}${topMatchPanels}${summary}${(role==='truck_owner'||role==='admin')?ownerPanel:''}${(role==='driver'||role==='admin')?driverPanel:''}${appsTable}${recordsTable}`;
