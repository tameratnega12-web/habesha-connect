...
const ROLE_PAGES={
 ...
 truck_owner:['home','services','profile','truck','messages','notifications'],
...
function home(){$('home').innerHTML=`<div class="hero"><h1>Habesha Connect</h1><p><b>Connecting the Ethiopian community through shipping, rentals, jobs, marketplace, services, and business tools.</b></p><p>V7.0.4 moves Available Owners / Rentals to the top of the Rent Seeker dashboard.</p>${roleWelcome()}<div class="actions">${currentUser?`<button class="btn primary" onclick="show('profile')">Open My Dashboard</button>`:`<button class="btn primary" onclick="show('account')">Create Account</button>`}${isAllowedPage('shipping')?`<button class="btn dark" onclick="show('shipping')">Start Shipping</button>`:''}${isAllowedPage('rentals')?`<button class="btn" onclick="show('rentals')">Find Rentals</button>`:''}${isAllowedPage('admin')?`<button class="btn ghost" onclick="show('admin')">Admin Dashboard</button>`:''}</div></div><h2 style="margin-top:22px">Available Services</h2><div class="grid">${roleServices()}</div>`}
...
function rentalRequestPaymentDisplay(q,role){
 ...
 if(role==='rent_seeker'||role==='customer')return q.amountPaid?('Your viewing fee '+money(q.amountPaid)):(q.paymentStatus||'Paid');
...
function truck(){
 ...
 let appsTable=`<h3>Driver Applications / Hiring Records</h3><table><tr><th>Job</th><th>Driver</th><th>Phone</th><th>License</th><th>Experience</th><th>Status</th><th>Action</th></tr>${myApps.map(a=>{let canManage=role==='truck_owner'&&a.ownerEmail===currentUser.email&&a.status==='Pending';return `<tr><td>${a.jobTitle}</td><td>${a.driverName}<br><span class="small">${a.driverEmail}</span></td><td>${a.driverPhone}</td><td>${a.license}</td><td>${a.experience}</td><td><span class="pill ${a.status==='Approved'?'good':a.status==='Declined'?'bad':'warn'}">${a.status}</span></td><td>${canManage?`<button class="btn primary" onclick="approveTruckApp('${a.id}')">Approve / Hire</button> <button class="btn bad" onclick="declineTruckApp('${a.id}')">Decline</button>`:'-'}</td></tr>`}).join('')||'<tr><td colspan="7">No applications yet.</td></tr>'}</table>`;
 let recordsTable=`<h3>Truck Owner / Driver Records</h3><table><tr><th>Truck</th><th>Plate</th><th>Driver</th><th>Status</th><th>Insurance</th><th>Registration</th><th>Income</th><th>Expenses</th><th>Profit</th><th>Mileage</th><th>Maintenance</th></tr>${myTruckRecords.map(t=>`<tr><td>${t.truck}<br><span class="small">VIN: ${t.vin||''}</span></td><td>${t.plate||''}</td><td>${t.driver}<br><span class="small">${t.driverEmail||''}</span></td><td>${t.status||'Available'}</td><td>${t.insurance||''}</td><td>${t.registration||''}</td><td>${money(t.income)}</td><td>${money(t.expenses)}</td><td>${money((+t.income||0)-(+t.expenses||0))}</td><td>${t.mileage}</td><td>${t.maintenance||''}</td></tr>`).join('')||'<tr><td colspan="11">No truck records yet.</td></tr>'}</table>`;
...
async function admin(){
 ...
 $('admin').innerHTML=`<h2>⚙️ Admin Verification Center</h2><div class='notice'><b>V6.8.3 Admin Scroll Stability Fix:</b> admin page no longer continuously refreshes while scrolling. Buttons refresh once after actions and keep your scroll position stable.</div>
...
function adminSuccess(message){
  ...
  const adminVisible=$('admin')&&!$('admin').classList.contains('hide');
  if(adminVisible){
    setTimeout(async()=>{
      try{ await admin(); }catch(e){ console.error('Admin refresh error',e); }
