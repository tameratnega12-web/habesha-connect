...
const HC_EMAIL_ADMIN_BACKUPS=['habeshaconnect@gmail.com'];
...
function adminEmailRecipients(){
  let emails=[HC_EMAIL_ADMIN,...HC_EMAIL_ADMIN_BACKUPS];
  try{
    (data.users||[]).forEach(u=>{
      let roles=[u.role,u.active_role,...(Array.isArray(u.roles)?u.roles:[])].filter(Boolean).map(String);
      if(roles.includes('admin')&&u.email)emails.push(u.email);
    });
  }catch(e){}
  return [...new Set(emails.map(cleanEmail).filter(Boolean))];
}
function sendAdminEmailNotice(subject,summary,details={},page='admin'){
  adminEmailRecipients().forEach(to=>sendEmailNotice({to,name:'Admin',subject,summary,buttonText:'Open Admin Dashboard',page,details}));
}
...
async function postTrailerRental(){
 if(!requireLogin())return;
 if(currentUser.role!=='truck_owner'&&currentUser.role!=='admin')return alert('Only truck owners can post trailer rentals.');
 let trailerType=($('trType').value||'').trim(),location=($('trLocation').value||'').trim(),price=($('trPrice').value||'').trim();
 if(!trailerType||!location||!price)return alert('Trailer type, location, and price are required.');
 let status=currentUser.role==='admin'?'Available':'Pending Admin Approval';
 let item={id:'TLR'+Date.now().toString().slice(-6),ownerName:currentUser.name,ownerEmail:currentUser.email,ownerPhone:currentUser.phone||'',trailerType,location,price,deposit:($('trDeposit').value||'').trim(),availability:($('trAvailability').value||'').trim(),description:($('trDescription').value||'').trim(),status,renterName:'',renterEmail:'',renterPhone:'',createdAt:new Date().toLocaleString()};
 let res=await syncTrailerRentalToSupabase(item);
 if(res.error)return alert('Trailer rental was not saved to Supabase. Please run the trailer rent SQL first, then try again. '+res.error.message);
 data.trailerRentals.unshift(item);
 addNote(currentUser.email,'Trailer rental submitted: '+trailerType+'. Admin approval is required before it is published.');
 if(currentUser.role!=='admin'){
   sendAdminEmailNotice('Trailer rental post waiting for approval','A trailer owner posted a trailer rental. Admin approval is required before it is published.',{Owner:currentUser.name,Email:currentUser.email,Trailer:trailerType,Location:location,Price:price},'admin');
   alert('Trailer rental submitted. Admin approval is required before truck owners and drivers can request it.');
 }else{
   alert('Trailer rental posted and published.');
 }
 save();truck();
}
async function requestTrailerRental(id){
 if(!requireLogin())return;
 if(currentUser.role!=='driver'&&currentUser.role!=='truck_owner')return alert('Only truck drivers or truck owners can request trailer rentals.');
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 if(item.ownerEmail===currentUser.email)return alert('You cannot request your own trailer.');
 if(item.status!=='Available')return alert('This trailer is not available.');
 let old={...item};item.status='Request Pending Admin Approval';item.renterName=currentUser.name||'';item.renterEmail=currentUser.email;item.renterPhone=currentUser.phone||'';
 let res=await updateTrailerRentalDb(item,{status:item.status,renter_name:item.renterName,renter_email:item.renterEmail,renter_phone:item.renterPhone});
 if(res.error){Object.assign(item,old);return alert('Could not request trailer rental: '+res.error.message);}
 addNote(currentUser.email,'Trailer rental request submitted for '+item.trailerType+'. Admin approval is required.');
 sendAdminEmailNotice('Trailer rental request waiting for approval','A truck owner/driver requested a trailer rental. Admin approval is required before the trailer owner can review it.',{Requester:item.renterName,RequesterEmail:item.renterEmail,Trailer:item.trailerType,TrailerOwner:item.ownerName,OwnerEmail:item.ownerEmail,Location:item.location,Price:item.price},'admin');
 save();alert('Trailer request submitted. Admin approval is required before the trailer owner can review it.');truck();
}
async function adminApproveTrailerPost(id){
 if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 let old={...item};item.status='Available';
 let res=await updateTrailerRentalDb(item,{status:'Available'});
 if(res.error){Object.assign(item,old);return alert('Could not approve trailer rental: '+res.error.message);}
 sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer Rental Post Approved',summary:'Your trailer rental post has been approved and is now visible to truck owners and drivers.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Location:item.location,Price:item.price,Status:'Published'}});
 addNote(item.ownerEmail,'Your trailer rental was approved and published.');
 persistOnly();alert('Trailer rental approved and published.');adminSuccess();
}
async function adminDeclineTrailerPost(id){
 if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 let old={...item};item.status='Declined by Admin';
 let res=await updateTrailerRentalDb(item,{status:item.status});
 if(res.error){Object.assign(item,old);return alert('Could not decline trailer rental: '+res.error.message);}
 sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer Rental Post Declined',summary:'Your trailer rental post was declined by admin.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Location:item.location,Price:item.price,Status:'Declined'}});
 addNote(item.ownerEmail,'Your trailer rental post was declined by admin.');
 persistOnly();alert('Trailer rental post declined.');adminSuccess();
}
async function adminApproveTrailerRequest(id){
 if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 let old={...item};item.status='Requested';
 let res=await updateTrailerRentalDb(item,{status:'Requested'});
 if(res.error){Object.assign(item,old);return alert('Could not approve trailer request: '+res.error.message);}
 sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer Rental Request Approved by Admin',summary:'A truck owner/driver requested your trailer and admin approved the request. Please accept or decline it in your Trucking Dashboard.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Requester:item.renterName,RequesterEmail:item.renterEmail,Trailer:item.trailerType,Location:item.location,Price:item.price}});
 addNote(item.ownerEmail,'Admin approved a trailer rental request from '+item.renterName+'.');
 persistOnly();alert('Trailer request approved and sent to trailer owner.');adminSuccess();
}
async function adminDeclineTrailerRequest(id){
 if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 let renterEmail=item.renterEmail,renterName=item.renterName,trailerType=item.trailerType;
 let old={...item};item.status='Available';item.renterName='';item.renterEmail='';item.renterPhone='';
 let res=await updateTrailerRentalDb(item,{status:'Available',renter_name:'',renter_email:'',renter_phone:'',rented_at:null});
 if(res.error){Object.assign(item,old);return alert('Could not decline trailer request: '+res.error.message);}
 if(renterEmail)sendEmailNotice({to:renterEmail,name:renterName,subject:'Trailer Rental Request Declined by Admin',summary:'Your trailer rental request was declined by admin.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:trailerType,Status:'Declined'}});
 persistOnly();alert('Trailer rental request declined.');adminSuccess();
}
async function approveTrailerRental(id){
 if(!requireLogin())return;
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 if(item.ownerEmail!==currentUser.email&&currentUser.role!=='admin')return alert('Only the trailer owner can accept this request.');
 if(item.status!=='Requested')return alert('This trailer request must be approved by admin before the trailer owner can accept it.');
 let old={...item};item.status='Rented';item.rentedAt=new Date().toLocaleString();
 let res=await updateTrailerRentalDb(item,{status:'Rented',rented_at:new Date().toISOString()});
 if(res.error){Object.assign(item,old);return alert('Could not approve trailer rental: '+res.error.message);}
 addNote(item.renterEmail,'Your trailer rental request was accepted by the trailer owner.');
 sendEmailNotice({to:item.renterEmail,name:item.renterName,subject:'Trailer Rental Accepted',summary:'The trailer owner accepted your trailer rental request.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Owner:item.ownerName,OwnerEmail:item.ownerEmail,Location:item.location,Price:item.price,Status:'Accepted / Reserved'}});
 save();alert('Trailer rental accepted. The truck owner/driver has been notified.');truck();
}
async function declineTrailerRental(id){
 if(!requireLogin())return;
 let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');
 if(item.ownerEmail!==currentUser.email&&currentUser.role!=='admin')return alert('Only the trailer owner can decline this request.');
 if(item.status!=='Requested')return alert('Only admin-approved trailer requests can be declined by the trailer owner.');
 let renterEmail=item.renterEmail,renterName=item.renterName,trailerType=item.trailerType;
 item.status='Available';item.renterName='';item.renterEmail='';item.renterPhone='';
 let res=await updateTrailerRentalDb(item,{status:'Available',renter_name:'',renter_email:'',renter_phone:'',rented_at:null});
 if(res.error)return alert('Could not decline trailer rental: '+res.error.message);
 if(renterEmail)sendEmailNotice({to:renterEmail,name:renterName,subject:'Trailer Rental Request Declined',summary:'Your trailer rental request was declined by the trailer owner.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:trailerType,Status:'Declined'}});
 save();alert('Trailer rental request declined. The truck owner/driver has been notified.');truck();
}
