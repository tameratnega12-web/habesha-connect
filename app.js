...
const pages=['home','account','profile','services','shipping','rentals','marketplace','jobs','truck','home_services','business','messages','notifications','admin','about','contact','help','report','privacy','terms'];
const labels={home:'Home',account:'Account',profile:'Profile',services:'My Services',shipping:'Shipping',rentals:'Rentals',marketplace:'Marketplace',jobs:'Jobs',truck:'Trucking',business:'Business Manager',home_services:'Home Services',messages:'Messages',notifications:'Notifications',admin:'Admin',about:'About',contact:'Contact',help:'Help / FAQ',report:'Report Problem',privacy:'Privacy',terms:'Terms'};
...
if(!localStorage.getItem(BETA_CLEANUP_KEY)){
  ['payments','messages','notifications','favorites','reviews','shipments','trips','rentals','rentalRequests','market','jobs','trucks','truckJobs','truckApplications','truckDriverProfiles','trailerRentals','businesses'].forEach(k=>{data[k]=[];});
...
const ROLE_PAGES={
 ...
 customer:['home','services','profile','marketplace','jobs','home_services','messages','notifications',...PUBLIC_PAGES],
 truck_owner:['home','services','profile','truck','messages','notifications',...PUBLIC_PAGES],
 driver:['home','services','profile','truck','messages','notifications',...PUBLIC_PAGES],
 business_owner:['home','services','profile','business','marketplace','jobs','home_services','messages','notifications',...PUBLIC_PAGES],
...
async function show(p){
 ...
 if(['shipping','rentals','profile','marketplace','jobs','truck','business'].includes(p)){
...
async function login(){let e=cleanEmail($('loginEmail')?.value),p=String($('loginPass')?.value||'');if(!e||!p)return alert('Enter your email and password.');if(!e.includes('@'))return alert('Please enter the full email address you used to create the account.');if(authReady()){let {data:authData,error}=await hcSupabase.auth.signInWithPassword({email:e,password:p});if(error){let localAdmin=data.users.find(x=>cleanEmail(x.email)===e&&x.pass===p&&x.role==='admin');if(localAdmin){currentUser=localAdmin;addNote(localAdmin.email,'You signed in with local admin access.');save();show('admin');return;}let msg=String(error.message||'');if(msg.toLowerCase().includes('invalid'))msg='Login failed. Please check the email and password. Make sure there are no spaces and use the same email used during signup.';return alert(msg);}let u=await loadSupabaseProfile(authData.user);if(!u)return alert('Login worked, but profile was not found.');currentUser=u;addNote(u.email,'You signed in successfully.');save();show('services');return;}let u=data.users.find(x=>cleanEmail(x.email)===e&&x.pass===p);if(!u)return alert('Wrong email or password');currentUser=u;addNote(u.email,'You signed in successfully.');save();show('services')}
...
function sendHomeServicesAdminApprovalEmail(item){
  if(!item)return Promise.resolve({ok:false,error:'Missing item'});
...
function sendHomeServicesAdminRequestEmail(req){
  if(!req)return Promise.resolve({ok:false,error:'Missing request'});
  return sendEmailNotice({
    to:adminNotificationRecipients(),
    name:'Admin',
...
async function renderPage(p){
 ...
 if(p==='jobs')jobs();
 if(p==='truck')truck();
 if(p==='home_services')await homeServices();
...
function openTruckJobPostFromProfile(){show('truck');setTimeout(()=>document.getElementById('jobPostCard')?.scrollIntoView({behavior:'smooth'}),150)}
function openTrailerRentalsFromProfile(){show('truck');setTimeout(()=>document.getElementById('trailerRentalsTable')?.scrollIntoView({behavior:'smooth'}),150)}
function openPostTrailerRentFromProfile(){show('truck');setTimeout(()=>document.getElementById('trailerRentCard')?.scrollIntoView({behavior:'smooth'}),150)}
function openDriverProfileFromProfile(){show('truck');setTimeout(()=>document.getElementById('driverProfileCard')?.scrollIntoView({behavior:'smooth'}),150)}
function openTruckJobsFromProfile(){show('truck');setTimeout(()=>document.getElementById('availableTruckJobs')?.scrollIntoView({behavior:'smooth'}),150)}
function roleDashboardCards(){
 ...
 if(currentUser.role==='truck_owner')return `<div class="grid"><div class="card"><h3>📢 Post Driver Job</h3><p class="muted">Create a driver hiring job from your profile dashboard. Admin approval is required before drivers see it.</p><button class="btn primary" onclick="openTruckJobPostFromProfile()">Post Job</button></div><div class="card"><h3>🚛 Trailer Rentals</h3><p class="muted">See available trailers, your posted trailers, and rental requests.</p><button class="btn primary" onclick="openTrailerRentalsFromProfile()">See Trailer Rental</button><button class="btn" onclick="openPostTrailerRentFromProfile()">Post Trailer Rent</button></div><div class="card"><h3>👤 Driver Applicants</h3><p class="muted">Review admin-approved driver applications.</p><button class="btn" onclick="show('truck')">Open Truck Owner Dashboard</button></div></div>`;
 if(currentUser.role==='driver')return `<div class="grid"><div class="card"><h3>🚛 Trailer Rentals</h3><p class="muted">See approved trailers available for rent and track your requests.</p><button class="btn primary" onclick="openTrailerRentalsFromProfile()">See Trailer Rental</button></div><div class="card"><h3>🚚 Driver Jobs</h3><p class="muted">See approved truck owner hiring posts.</p><button class="btn" onclick="openTruckJobsFromProfile()">See Jobs</button></div><div class="card"><h3>👤 Driver Profile</h3><p class="muted">Complete your profile before applying.</p><button class="btn" onclick="openDriverProfileFromProfile()">Update Driver Profile</button></div></div>`;
 if(currentUser.role==='service_provider')return `<div class="grid"><div class="card"><h3>🔧 Home Service</h3><p class="muted">Post your service from your profile dashboard. Admin approval is required before customers see it.</p><button class="btn primary" onclick="openHomeServicePostFromProfile()">Post Home Service</button></div><div class="card"><h3>📢 My Service Posts</h3><p class="muted">Check approval status for your service posts.</p><button class="btn" onclick="openHomeServicePostsFromProfile()">See My Services</button></div><div class="card"><h3>📩 Service Requests</h3><p class="muted">After admin approves a customer request, you can see and accept it here.</p><button class="btn" onclick="openHomeServiceRequestsFromProfile()">See Requests</button></div></div>`;
...
async function shipping(){
 ...
 let myTrips=data.trips.filter(t=>{let mine=currentUser&&(String(t.travelerId||'')===String(currentUser.id||'')||t.travelerEmail===currentUser.email);return mine&&isTripBookable(t);}).sort(activeTripSort);
 let statsHtml='';
 if(role==='admin'){
...
async function addTrip(){if(!requireLogin())return;if(currentUser.role!=='traveler'&&currentUser.role!=='admin')return alert('Only travelers can post trips.');let from=$('tripFrom').value,to=$('tripTo').value;if(from===to)return alert('From and To cannot be the same.');let travelDate=$('tripDate').value;if(!travelDate)return alert('Please enter travel date.');if(travelDate<addDaysIso(0))return alert('You cannot post a trip for a previous day.');if(travelDate<addDaysIso(5))return alert('Please post traveler trips at least 5 days before the flight date.');let space=+$('tripSpace').value||0;if(space<=0)return alert('Enter available space.');let fee=0;if(!confirm('Publish this traveler trip for beta testing? Sender and traveler will agree on shipping details before payment.'))return;if(authReady()&&currentUser.id){let payload={traveler_id:currentUser.id,from_city:from,to_city:to,travel_date:travelDate,original_space_lb:space,remaining_space_lb:space,status:'Open',listing_fee_paid:true,listing_fee:fee,listing_fee_refunded:false,listing_fee_paid_at:new Date().toISOString()};let {data:row,error}=await hcSupabase.from('trips').insert(payload).select('*, profiles:traveler_id(name,email,phone,verified)').single();if(error){if(String(error.message||'').toLowerCase().includes('column'))return alert('Please run supabase/v6_7_traveler_listing_fee_migration.sql once, then try again.');return alert('Could not save trip to Supabase: '+error.message);}data.trips.unshift(mapDbTrip(row));addNote('all','New traveler trip posted by '+currentUser.name+' with '+space+' lb available.');addNote('admin@habeshaconnect.com','Traveler listing fee paid by '+currentUser.name+' for '+from+' → '+to+'.');sendAdminEmailNotice('Traveler posted a trip','A traveler posted a new trip and it is ready for admin review.',{Traveler:currentUser.name,Email:currentUser.email,Route:from+' → '+to,Space:space+' lb'},'shipping');save();show('shipping');return;}let id='T'+Date.now().toString().slice(-5);data.trips.unshift({id,route:from+' → '+to,travelDate:travelDate,totalSpace:space,availableSpace:space,traveler:currentUser.name,travelerPhone:currentUser.phone||'',travelerEmail:currentUser.email,travelerVerified:!!currentUser.verified,status:'Open',listingFeePaid:true,listingFeeAmount:fee,listingFeeRefunded:false,listingFeePaidAt:new Date().toLocaleString()});addNote('all','New traveler trip posted by '+currentUser.name+' with '+space+' lb available.');sendAdminEmailNotice('Traveler posted a trip','A traveler posted a new trip and it is ready for admin review.',{Traveler:currentUser.name,Email:currentUser.email,Route:from+' → '+to,Space:space+' lb'},'shipping');save();shipping()}
async function requestTrip(id){
 ...
 if(authReady()&&currentUser.id&&t.dbId){
   ...
   if(inserted.error){const msg=String(inserted.error.message||'');if(msg.toLowerCase().includes('sender_name')||msg.toLowerCase().includes('sender_phone')||msg.toLowerCase().includes('package_description'))return alert('Please run the V6.7.2 sender shipment SQL first, then try again. Missing shipment sender/package columns.');return alert('Could not save request to Supabase: '+inserted.error.message);}
...
async function acceptShip(id){
 ...
 if(s.tripId){let t=data.trips.find(x=>String(x.id)===String(s.tripId)||String(x.dbId)===String(s.tripId));if(t&&s.weight>t.availableSpace)return alert('Not enough space left. Remaining: '+t.availableSpace+' lb');}
 ...
 s.status='Accepted';s.traveler=currentUser.name;s.travelerPhone=currentUser.phone||'';s.travelerEmail=currentUser.email;s.travelerId=currentUser.id||s.travelerId||'';s.travelerVerified=!!currentUser.verified;
 const senderEmail=cleanEmail(s.senderEmail||'');
 const acceptMsg='Your shipment '+(s.tracking||id)+' was accepted by '+currentUser.name+'. Please open Shipping and confirm the agreement so admin can approve and reserve the space.';
 if(senderEmail){
   addNote(senderEmail,acceptMsg);
   await sendEmailNotice({to:senderEmail,name:s.sender||'Sender',subject:'Traveler accepted your shipping request',summary:acceptMsg,buttonText:'Open Shipping Dashboard',page:'shipping',details:{Tracking:s.tracking||id,Traveler:currentUser.name,Route:s.route||'',Weight:(s.weight||'')+' lb',Status:'Accepted'}});
 }else{
   addNote('all',acceptMsg);
 }
 save();shipping()}
...
function rentalRequestsTable(reqs,role){
  reqs=reqs||[];
  return `<h3>Rental Requests / Contacts</h3><table><tr><th>Property</th><th>Rent Seeker</th><th>Phone</th><th>Owner</th><th>Owner Phone</th><th>Status</th><th>Beta Status</th><th>Action</th></tr>${reqs.map(q=>`<tr><td>${q.propertyTitle}</td><td>${q.seekerName}</td><td>${q.seekerPhone}</td><td>${q.ownerName}</td><td>${q.status==='Approved'||role==='admin'||role==='owner'?q.ownerPhone:'Shown after approval'}</td><td>${q.status}</td><td>${rentalRequestPaymentDisplay(q,role)}</td><td>${(role==='owner'||role==='admin')&&q.status==='Pending'?`<button class="btn" onclick="approveRentalReq('${q.id}')">Approve</button> <button class="btn bad" onclick="declineRentalReq('${q.id}')">Decline</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="8">No rental requests yet.</td></tr>'}</table>`;
}
...
async function rentals(){
 ...
 if(role==='owner'){
   $('rentals').innerHTML=`${addPropertyCard}${ownerPanel}<h3>My Properties</h3><div id="rentalList" class="grid">${rentalCards(myRentals,role)}</div>${ownerRequestsHtml}${ownerSummary}`;
...
async function addRental(){
  ...
  let fee=0;
  if(!confirm('Publish this property for beta testing? Admin approval is required before seekers can see it.'))return;
  ...
  if(authReady()){
    ...
    if(res.error){
      alert('Payment was recorded, but property could not be saved to Supabase: '+res.error.message+'\nPlease send this screenshot/message so we can fix the property columns.');
      return;
    }
...
async function payPublishRental(id){
  ...
  addNote('admin@habeshaconnect.com','Rental listing from '+r.owner+' needs approval.');
...
async function requestViewing(id){
 ...
 let localReq={id:res.data.id,dbId:res.data.id,rentalId:propertyKey,propertyTitle:r.title,seekerName:currentUser.name,seekerPhone:currentUser.phone||'',seekerEmail:currentUser.email,seekerId:currentUser.id||'',ownerName:r.owner,ownerPhone:r.ownerPhone||'',ownerEmail:r.ownerEmail||'',ownerId:r.ownerId||'',status:'Pending',paymentStatus:'Requested',paid:true,amountPaid:fee,time:new Date().toLocaleString(),source:'supabase'};
 data.rentalRequests.unshift(localReq);
 addNote(r.ownerEmail||'all','New rental viewing request from '+currentUser.name+'.');
 addNote('admin@habeshaconnect.com','New rental viewing request for '+r.title+' from '+currentUser.name+'.');sendAdminEmailNotice('Seeker requested a viewing','A rent seeker requested a property viewing.',{Seeker:currentUser.name,Email:currentUser.email,Property:r.title,Owner:r.owner,OwnerEmail:r.ownerEmail},'admin');
 if(r.ownerEmail){sendEmailNotice({to:r.ownerEmail,name:r.owner||'Property Owner',subject:'Someone requested to view your property',summary:'A rent seeker requested to view your property.',buttonText:'Open Owner Dashboard',page:'rentals',details:{Seeker:currentUser.name,SeekerEmail:currentUser.email,Property:r.title,Status:'Pending'}});}
 addNote(currentUser.email,'Your viewing request was sent to the owner and admin.');
 persistOnly();
...
async function approveRentalReq(id){
  ...
  let r=data.rentals.find(x=>String(x.id)===String(q.rentalId)||String(x.dbId)===String(q.rentalId));
  let oldRentalStatus=r?r.status:null;

  // Update the screen immediately so admin does not need to refresh.
  q.status='Approved';
  if(r)r.status='Rented';
  addNote(q.seekerEmail,'Your rental request was approved. Owner contact is now available.');sendEmailNotice({to:q.seekerEmail,name:q.seekerName,subject:'Rental viewing approved',summary:'Your rental viewing request was approved. Owner contact is now available.',buttonText:'Open Rental Dashboard',page:'rentals',details:{Property:q.propertyTitle,Status:'Approved'}});
  persistOnly();
  if(currentPage==='admin') adminSuccess(); else await rentals();

  // Then save to Supabase. If Supabase fails, roll back and show the error.
  let res={error:null};
  if(authReady()&&q.dbId){
    res=await hcSupabase.from('rental_requests').update({status:'Approved'}).eq('id',q.dbId);
  }
  if(!res.error&&authReady()&&r&&r.dbId){
    res=await hcSupabase.from('properties').update({status:'Rented'}).eq('id',r.dbId);
  }
  if(res.error){
    q.status=oldStatus;
    if(r)r.status=oldRentalStatus;
    persistOnly();
    if(currentPage==='admin') adminSuccess(); else await rentals();
    return alert('Could not approve rental request in Supabase: '+res.error.message);
  }

  // Refresh silently in the background, but keep the approved status visible now.
  if(currentPage==='admin'){
    refreshAdminData().then(()=>adminSuccess()).catch(()=>{});
  }else if(currentPage==='rentals'){
    loadSupabaseRentalRequests().then(()=>rentals()).catch(()=>{});
  }
}
...
function truckAdminManagementHtml(){
 ...
 <h3>Truck Driver Applications</h3><table><tr><th>Job</th><th>Owner</th><th>Driver</th><th>Phone</th><th>License</th><th>Status</th><th>Action</th></tr>${apps.map(a=>`<tr><td>${a.jobTitle||''}</td><td>${a.ownerName||''}<br><span class="small">${a.ownerEmail||''}</span></td><td>${a.driverName||''}<br><span class="small">${a.driverEmail||''}</span></td><td>${a.driverPhone||''}</td><td>${a.license||''}</td><td><span class="pill ${(a.status==='Approved'||a.status==='Hired')?'good':a.status==='Declined'?'bad':'warn'}">${a.status||'Pending Admin Approval'}</span></td><td>${a.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveTruckApplication('${a.id}')">Approve to Owner</button> `:''}<button class="btn primary" onclick="approveTruckApp('${a.id}')">Hire / Final Approve</button> <button class="btn bad" onclick="declineTruckApp('${a.id}')">Decline</button> <button class="btn ghost" onclick="deleteTruckApplication('${a.id}')">Delete</button></td></tr>`).join('')||'<tr><td colspan="8">No applications yet.</td></tr>'}</table>
...
function truck(){
 ...
 let appsTable=`<h3 id="truckApplicationsTable">Driver Applications / Hiring Records</h3><table><tr><th>Job</th><th>Driver</th><th>Phone</th><th>License</th><th>Experience</th><th>Looking For / Notes</th><th>Status</th><th>Action</th></tr>${myApps.map(a=>{let canManage=role==='truck_owner'&&a.ownerEmail===currentUser.email&&a.status==='Pending Owner Review';return `<tr><td>${a.jobTitle}</td><td>${a.driverName}<br><span class="small">${a.driverEmail}</span></td><td>${a.driverPhone}</td><td>${a.license}</td><td>${a.experience}</td><td>${a.looking||''}<br><span class="small">${a.notes||''}</span></td><td><span class="pill ${(a.status==='Approved'||a.status==='Hired')?'good':a.status==='Declined'?'bad':'warn'}">${a.status}</span></td><td>${canManage?`<button class="btn primary" onclick="approveTruckApp('${a.id}')">Approve / Hire</button> <button class="btn bad" onclick="declineTruckApp('${a.id}')">Decline</button>`:'-'}</td></tr>`}).join('')||'<tr><td colspan="8">No applications yet.</td></tr>'}</table>`;
 ...
 $('truck').innerHTML=`<h2>🚚 Trucking Connect</h2>${role==='truck_owner'?ownerQuick:''}${role==='driver'?driverQuick:''}${trailerTable}${topMatchPanels}${summary}${(role==='truck_owner'||role==='admin')?ownerPanel:''}${(role==='driver'||role==='admin')?driverPanel:''}${role==='truck_owner'?'':appsTable}${recordsTable}`;
...
async function postTruckJob(){if(!requireLogin())return;if(currentUser.role!=='truck_owner'&&currentUser.role!=='admin')return alert('Only truck owners can post driver hiring jobs.');let title=($('jobTitle').value||'').trim(),route=($('jobRoute').value||'').trim(),pay=($('jobPay').value||'').trim();if(!title||!route||!pay)return alert('Job title, route/location, and pay are required.');let status=currentUser.role==='admin'?'Open':'Pending Admin Approval';let job={id:'TJ'+Date.now().toString().slice(-6),ownerName:currentUser.name,ownerEmail:currentUser.email,ownerPhone:currentUser.phone||'',title,route,pay,schedule:($('jobSchedule').value||'').trim(),requirements:($('jobReq').value||'').trim(),truckDetails:($('jobTruck').value||'').trim(),status,createdAt:new Date().toLocaleString()};let res=await syncTruckJobToSupabase(job);if(res.error)return alert('Job was not posted to the shared dashboard. Please run the trucking Supabase migration, then try again. '+res.error.message);data.truckJobs.unshift(job);addNote(currentUser.email,'Driver hiring job submitted: '+title+'. Admin approval is required before it is published.');addNote('admin@habeshaconnect.com','Truck owner job waiting for approval: '+title+' by '+currentUser.name+'.');sendAdminEmailNotice('Truck owner posted a job','A truck owner posted a driver hiring job for admin approval.',{Owner:currentUser.name,Email:currentUser.email,Job:title,Route:route,Pay:pay},'admin');save();alert('Job submitted. Admin approval is required before drivers can see and apply.')}

async function postTrailerRental(){if(!requireLogin())return;if(currentUser.role!=='truck_owner'&&currentUser.role!=='admin')return alert('Only truck owners can post trailer rentals.');let trailerType=($('trType').value||'').trim(),location=($('trLocation').value||'').trim(),price=($('trPrice').value||'').trim();if(!trailerType||!location||!price)return alert('Trailer type, location, and price are required.');let item={id:'TLR'+Date.now().toString().slice(-6),ownerName:currentUser.name,ownerEmail:currentUser.email,ownerPhone:currentUser.phone||'',trailerType,location,price,deposit:($('trDeposit').value||'').trim(),availability:($('trAvailability').value||'').trim(),description:($('trDescription').value||'').trim(),status:(currentUser.role==='admin'?'Available':'Pending Admin Approval'),renterName:'',renterEmail:'',renterPhone:'',createdAt:new Date().toLocaleString()};let res=await syncTrailerRentalToSupabase(item);if(res.error)return alert('Trailer rental was not saved to Supabase. Please run the trailer rent SQL first, then try again. '+res.error.message);data.trailerRentals.unshift(item);addNote(currentUser.email,'Trailer rental submitted: '+trailerType+'. Admin approval is required before owners/drivers can see it.');addNote('admin@habeshaconnect.com','Trailer rental waiting for approval: '+trailerType+' by '+currentUser.name+'.');sendAdminEmailNotice('Trailer Rental Needs Approval','A truck owner posted a trailer rental for admin approval.',{Owner:currentUser.name,Email:currentUser.email,Trailer:trailerType,Location:location,Price:price},'admin');save();alert(currentUser.role==='admin'?'Trailer rental posted. Drivers and truck owners can now request it.':'Trailer rental submitted. Admin approval is required before truck owners/drivers can see it.');truck()}
async function adminApproveTrailerRentalPost(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');let old={...item};item.status='Available';let res=await updateTrailerRentalDb(item,{status:'Available'});if(res.error){Object.assign(item,old);return alert('Could not approve trailer rental: '+res.error.message);}addNote(item.ownerEmail,'Your trailer rental was approved and published: '+item.trailerType+'.');sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer Rental Approved',summary:'Your trailer rental was approved by admin and is now visible to truck owners/drivers.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Location:item.location,Price:item.price,Status:'Available'}});save();alert('Trailer rental approved and published.');if(currentPage==='admin')adminSuccess();else truck()}
async function requestTrailerRental(id){if(!requireLogin())return;if(currentUser.role!=='driver'&&currentUser.role!=='truck_owner')return alert('Only truck drivers or truck owners can request trailer rentals.');let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');if(item.ownerEmail===currentUser.email)return alert('You cannot request your own trailer.');if(item.status!=='Available')return alert('This trailer is not available.');let old={...item};item.status='Requested';item.renterName=currentUser.name||'';item.renterEmail=currentUser.email;item.renterPhone=currentUser.phone||'';let res=await updateTrailerRentalDb(item,{status:'Requested',renter_name:item.renterName,renter_email:item.renterEmail,renter_phone:item.renterPhone});if(res.error){Object.assign(item,old);return alert('Could not request trailer rental: '+res.error.message);}addNote(item.ownerEmail,'Trailer rental request from '+item.renterName+' for '+item.trailerType+'.');sendEmailNotice({to:item.ownerEmail,name:item.ownerName,subject:'Trailer Rental Request',summary:'Someone requested to rent your trailer.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Renter:item.renterName,RenterEmail:item.renterEmail,Trailer:item.trailerType,Location:item.location,Price:item.price}});addNote(currentUser.email,'Trailer rental request sent for '+item.trailerType+'.');save();alert('Trailer request sent to the owner.');truck()}
async function approveTrailerRental(id){if(!requireLogin())return;let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');if(currentUser.role!=='admin'&&item.ownerEmail!==currentUser.email)return alert('Only the trailer owner can approve this request.');let old={...item};item.status='Rented';item.rentedAt=new Date().toLocaleString();let res=await updateTrailerRentalDb(item,{status:'Rented',rented_at:new Date().toISOString()});if(res.error){Object.assign(item,old);return alert('Could not approve trailer rental: '+res.error.message);}addNote(item.renterEmail,'Your trailer rental request was approved.');sendEmailNotice({to:item.renterEmail,name:item.renterName,subject:'Trailer Rental Approved',summary:'Your trailer rental request was approved by the owner.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:item.trailerType,Owner:item.ownerName,OwnerEmail:item.ownerEmail,Location:item.location,Price:item.price}});save();alert('Trailer rental approved.');truck()}
async function declineTrailerRental(id){if(!requireLogin())return;let item=(data.trailerRentals||[]).find(x=>x.id===id);if(!item)return alert('Trailer rental not found.');if(currentUser.role!=='admin'&&item.ownerEmail!==currentUser.email)return alert('Only the trailer owner can decline this request.');let renterEmail=item.renterEmail,renterName=item.renterName,trailerType=item.trailerType;item.status='Available';item.renterName='';item.renterEmail='';item.renterPhone='';let res=await updateTrailerRentalDb(item,{status:'Available',renter_name:'',renter_email:'',renter_phone:'',rented_at:null});if(res.error)return alert('Could not decline trailer rental: '+res.error.message);if(renterEmail)sendEmailNotice({to:renterEmail,name:renterName,subject:'Trailer Rental Request Declined',summary:'Your trailer rental request was declined by the owner.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Trailer:trailerType,Status:'Declined'}});save();alert('Trailer rental request declined.');truck()}
...
async function applyTruckJob(id){if(!requireLogin())return;if(currentUser.role!=='driver')return alert('Only truck drivers can apply for truck driving jobs.');let j=(data.truckJobs||[]).find(x=>x.id===id);if(!j)return alert('Job not found.');if(j.status!=='Open')return alert('This job is waiting for admin approval or is closed.');if((data.truckApplications||[]).some(a=>a.jobId===id&&a.driverEmail===currentUser.email))return alert('You already applied for this job.');let p=(data.truckDriverProfiles||[]).find(x=>x.driverEmail===currentUser.email)||{};if(!p.license||!p.experience){alert('Please complete your Driver Job Profile before applying.');return;}let app={id:'TA'+Date.now().toString().slice(-6),jobId:j.id,jobDbId:j.dbId||'',jobTitle:j.title,ownerName:j.ownerName,ownerEmail:j.ownerEmail,driverName:p.name||currentUser.name,driverEmail:currentUser.email,driverPhone:p.phone||currentUser.phone||'',city:p.city||'',license:p.license||'',experience:p.experience||'',looking:p.looking||'',notes:p.notes||'',status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};let res=await syncTruckApplicationToSupabase(app,j);if(res.error)return alert('Application was not sent to Admin. Please run the trucking Supabase migration, then try again. '+res.error.message);data.truckApplications.unshift(app);addNote('admin@habeshaconnect.com','Driver application waiting for approval: '+(p.name||currentUser.name)+' applied for '+j.title+'.');sendAdminEmailNotice('New Truck Driver Application','A truck driver submitted a job application for admin approval.',{Driver:p.name||currentUser.name,Email:currentUser.email,Job:j.title,Owner:j.ownerName,OwnerEmail:j.ownerEmail},'admin');if(j.ownerEmail){sendEmailNotice({to:j.ownerEmail,name:j.ownerName,subject:'A Driver Applied for Your Job',summary:'A truck driver has applied for your job posting. Admin approval is still required before final hiring.',buttonText:'Open Truck Owner Dashboard',page:'truck',details:{Driver:p.name||currentUser.name,DriverEmail:currentUser.email,Job:j.title,Route:j.route||'',License:p.license||'',Experience:p.experience||''}});}addNote(currentUser.email,'Application submitted for '+j.title+'. Admin approval is required before the owner can review it.');save();alert('Applied. Admin approval is required before the truck owner can see and review your application.');truck()}
async function approveTruckApp(id){let a=(data.truckApplications||[]).find(x=>x.id===id);if(!a)return alert('Application not found.');if(currentUser.role!=='admin'&&a.ownerEmail!==currentUser.email)return alert('Only the truck owner can approve this application.');if(currentUser.role!=='admin'&&a.status!=='Pending Owner Review')return alert('This application is waiting for admin approval.');let oldApp={...a};let now=new Date().toISOString();a.status='Hired';a.approvedAt=new Date().toLocaleString();a.hiredAt=new Date().toLocaleString();let r=await updateTruckApplicationDb(a,{status:'Hired',approved_at:now,hired_at:now});if(r.error){Object.assign(a,oldApp);return alert('Could not hire driver in Supabase: '+r.error.message);}let j=(data.truckJobs||[]).find(x=>x.id===a.jobId||x.dbId===a.jobDbId);let oldJob=j?{...j}:null;if(j){j.status='Hired';j.hiredDriverName=a.driverName||'';j.hiredDriverEmail=a.driverEmail||'';j.hiredAt=new Date().toLocaleString();let jr=await updateTruckJobDb(j,{status:'Hired',hired_driver_name:a.driverName||'',hired_driver_email:a.driverEmail||'',hired_at:now});if(jr.error){Object.assign(a,oldApp);if(oldJob)Object.assign(j,oldJob);return alert('Application changed, but job could not be marked Hired in Supabase: '+jr.error.message);}}for(let other of (data.truckApplications||[])){if((other.jobId===a.jobId||other.jobDbId===a.jobDbId)&&other.id!==a.id&&other.status!=='Declined'){other.status='Closed - Position Filled';other.closedAt=new Date().toLocaleString();await updateTruckApplicationDb(other,{status:'Closed - Position Filled',closed_at:now});}}addNote(a.driverEmail,'You were hired for '+a.jobTitle+'.');sendEmailNotice({to:a.driverEmail,name:a.driverName,subject:'Job hiring completed',summary:'Congratulations. You were hired for the truck driving job.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Job:a.jobTitle,Status:'Hired'}});addNote(a.ownerEmail,'You hired '+a.driverName+' for '+a.jobTitle+'.');sendEmailNotice({to:a.ownerEmail,name:a.ownerName,subject:'Job hiring completed',summary:'You completed the hiring process for this truck driving job.',buttonText:'Open Trucking Dashboard',page:'truck',details:{Driver:a.driverName,Job:a.jobTitle,Status:'Hired'}});if(currentUser.role==='admin')addNote('admin@habeshaconnect.com','Hiring completed: '+a.driverName+' was hired for '+a.jobTitle+'.');persistOnly();if(currentUser.role==='admin')adminSuccess();else truck();alert('Driver hired. Job status changed to Hired.');}
...
/* V7.7.0 Home Services module - added as a separate feature */
const HOME_SERVICE_CATEGORIES=['Plumbing','Electrical','HVAC','Appliance Repair','House Cleaning','Carpet Cleaning','Painting','Flooring','Roofing','Landscaping','Lawn Care','Junk Removal','Moving Help','Furniture Assembly','Handyman','Locksmith','Pest Control','Mobile Mechanic','Mobile Car Wash','Computer Repair','Phone Repair','TV Mounting','Security Camera Installation','Drywall Repair','Tile Installation','Fence Installation','Garage Door Repair','Child Care','Elder Care','Pet Care','Tutoring','Translation','Tax Preparation','Insurance Assistance','Immigration Document Assistance'];
function mapHomeServiceProvider(row){return {dbId:row.id,id:localRefFromDb(row,'HSP'),providerName:row.provider_name||'',providerEmail:row.provider_email||'',providerPhone:row.provider_phone||'',businessName:row.business_name||'',category:row.category||'',serviceArea:row.service_area||'',experience:row.experience||'',priceNote:row.price_note||'',description:row.description||'',licenseInfo:row.license_info||'',status:row.status||'Pending Admin Approval',rating:row.rating||'',createdAt:row.created_at?new Date(row.created_at).toLocaleString():''};}
function mapHomeServiceRequest(row){return {dbId:row.id,id:localRefFromDb(row,'HSR'),providerId:row.provider_local_ref||row.provider_id||'',providerDbId:row.provider_id||'',providerName:row.provider_name||'',providerEmail:row.provider_email||'',customerName:row.customer_name||'',customerEmail:row.customer_email||'',customerPhone:row.customer_phone||'',category:row.category||'',serviceAddress:row.service_address||'',preferredDate:row.preferred_date||'',jobDescription:row.job_description||'',quoteAmount:row.quote_amount||'',status:row.status||'Requested',createdAt:row.created_at?new Date(row.created_at).toLocaleString():'',completedAt:row.completed_at?new Date(row.completed_at).toLocaleString():''};}
async function loadSupabaseHomeServices(){
 if(!authReady())return;
 let providers=await hcSupabase.from('home_service_providers').select('*').order('created_at',{ascending:false});
 if(!providers.error)data.homeServiceProviders=(providers.data||[]).map(mapHomeServiceProvider);else console.warn('Home service providers load error',providers.error);
 let reqs=await hcSupabase.from('home_service_requests').select('*').order('created_at',{ascending:false});
 if(!reqs.error)data.homeServiceRequests=(reqs.data||[]).map(mapHomeServiceRequest);else console.warn('Home service requests load error',reqs.error);
 persistOnly();
}
async function syncHomeServiceProviderToSupabase(item){
 if(!authReady())return {error:null,data:null};
 let payload={local_ref:item.id,provider_name:item.providerName,provider_email:item.providerEmail,provider_phone:item.providerPhone,business_name:item.businessName,category:item.category,service_area:item.serviceArea,experience:item.experience,price_note:item.priceNote,description:item.description,license_info:item.licenseInfo,status:item.status};
 let res=await hcSupabase.from('home_service_providers').insert(payload).select().single();
 if(!res.error&&res.data)item.dbId=res.data.id;
 return res;
}
async function updateHomeServiceProviderDb(item,fields){if(!authReady()||!item)return {error:null};let q=hcSupabase.from('home_service_providers').update(fields);return item.dbId?await q.eq('id',item.dbId):await q.eq('local_ref',item.id);}
async function deleteHomeServiceProviderDb(item){if(!authReady()||!item)return {error:null};let q=hcSupabase.from('home_service_providers').delete();return item.dbId?await q.eq('id',item.dbId):await q.eq('local_ref',item.id);}
async function syncHomeServiceRequestToSupabase(req,provider){
 if(!authReady())return {error:null,data:null};
 let payload={local_ref:req.id,provider_id:(provider&&provider.dbId)||req.providerDbId||null,provider_local_ref:req.providerId,provider_name:req.providerName,provider_email:req.providerEmail,customer_name:req.customerName,customer_email:req.customerEmail,customer_phone:req.customerPhone,category:req.category,service_address:req.serviceAddress,preferred_date:req.preferredDate,job_description:req.jobDescription,quote_amount:req.quoteAmount,status:req.status};
 let res=await hcSupabase.from('home_service_requests').insert(payload).select().single();
 if(!res.error&&res.data)req.dbId=res.data.id;
 return res;
}
...
function homeServiceStatusBadge(s){return `<span class="pill ${s==='Approved'||s==='Accepted'||s==='Completed'?'good':s==='Declined'||s==='Cancelled'?'bad':'warn'}">${s}</span>`;}
function openHomeServicePostFromProfile(){show('home_services');setTimeout(()=>document.getElementById('homeServicePostCard')?.scrollIntoView({behavior:'smooth'}),150)}
function openHomeServicePostsFromProfile(){show('home_services');setTimeout(()=>document.getElementById('homeServiceMyPosts')?.scrollIntoView({behavior:'smooth'}),150)}
function openHomeServiceRequestsFromProfile(){show('home_services');setTimeout(()=>document.getElementById('homeServiceRequestsTop')?.scrollIntoView({behavior:'smooth'}),150)}

async function homeServices(){
 if(!requireLogin())return;
 ensureArrays();
 const role=currentUser.role;
 const providers=(data.homeServiceProviders||[]);
 const requests=(data.homeServiceRequests||[]);
 const approved=providers.filter(p=>p.status==='Approved'||p.providerEmail===currentUser.email||role==='admin');
 const pendingProviders=providers.filter(p=>p.status==='Pending Admin Approval');
 const pendingRequests=requests.filter(r=>r.status==='Pending Admin Approval');
 let providerMine=providers.filter(p=>p.providerEmail===currentUser.email);
 let myCustomerReqs=requests.filter(r=>r.customerEmail===currentUser.email);
 let providerReqs=requests.filter(r=>r.providerEmail===currentUser.email&&r.status!=='Pending Admin Approval');
 let topActions=`<div class="grid"><div class="card"><h3>📢 Service Posts</h3><div class="stat">${providerMine.length}</div><p class="muted">Your posted provider profiles.</p><button class="btn primary" onclick="document.getElementById('homeServicePostCard')?.scrollIntoView({behavior:'smooth'})">Post / Update Service</button></div><div class="card"><h3>📩 Service Requests</h3><div class="stat">${(providerReqs.length||myCustomerReqs.length)}</div><p class="muted">Requests from customers or requests you sent.</p><button class="btn primary" onclick="document.getElementById('homeServiceRequestsTop')?.scrollIntoView({behavior:'smooth'})">View Requests</button></div><div class="card"><h3>⏳ Waiting Approvals</h3><div class="stat">${role==='admin'?(pendingProviders.length+pendingRequests.length):providerMine.filter(p=>p.status==='Pending Admin Approval').length}</div><p class="muted">Provider profiles and customer requests waiting for admin approval.</p>${role==='admin'?`<button class="btn primary" onclick="document.getElementById('homeServicePendingApprovals')?.scrollIntoView({behavior:'smooth'})">Review Approvals</button>`:`<button class="btn ghost" onclick="document.getElementById('homeServiceMyPosts')?.scrollIntoView({behavior:'smooth'})">Check Status</button>`}</div></div>`;
 let providerForm=(role==='service_provider'||role==='business_owner'||role==='admin')?`<div class="card" id="homeServicePostCard"><h3>🔧 Create / Post Home Service Provider Profile</h3><label>Business / Display Name</label><input id="hsBusiness" placeholder="Tame Home Repair"><label>Category</label><select id="hsCategory">${options(HOME_SERVICE_CATEGORIES)}</select><label>Service Area</label><input id="hsArea" placeholder="Clarkston, Atlanta, Decatur"><label>Phone</label><input id="hsPhone" value="${currentUser.phone||''}" placeholder="Phone number"><label>Experience</label><input id="hsExp" placeholder="5 years experience"><label>Price Note</label><input id="hsPrice" placeholder="Free estimate / starts at $75"><label>License / Insurance Info (optional for now)</label><input id="hsLicense" placeholder="Licensed, insured, or optional"><label>Description</label><textarea id="hsDesc" placeholder="Tell customers what services you provide"></textarea><button class="btn primary" onclick="postHomeServiceProvider()">Submit Provider Profile</button><p class="muted">Admin approval is required before customers see your provider profile.</p></div>`:'';
 let providerCards=approved.map(p=>`<div class="card"><h3>${p.businessName||p.providerName}</h3><p><b>${p.category}</b> • ${p.serviceArea||''}</p><p>${p.description||''}</p><p class="muted">${p.priceNote||'Request quote'} ${p.licenseInfo?('• '+p.licenseInfo):''}</p><p>${homeServiceStatusBadge(p.status)}</p>${p.providerEmail!==currentUser.email?`<button class="btn primary" onclick="openHomeServiceRequest('${p.id}')">Request Service</button>`:'<span class="pill good">Your Profile</span>'}</div>`).join('')||'<div class="card"><p>No approved home service providers yet.</p></div>';
 let pendingTable=role==='admin'?`<h3 id="homeServicePendingApprovals">⏳ Waiting Approvals</h3><h4>Provider Posts Waiting Approval</h4><table><tr><th>Provider</th><th>Category</th><th>Area</th><th>Status</th><th>Action</th></tr>${pendingProviders.map(p=>`<tr><td>${p.businessName||p.providerName}<br><span class="small">${p.providerEmail}</span></td><td>${p.category}</td><td>${p.serviceArea}</td><td>${homeServiceStatusBadge(p.status)}</td><td><button class="btn primary" onclick="adminApproveHomeServiceProvider('${p.id}')">Approve</button> <button class="btn bad" onclick="adminRejectHomeServiceProvider('${p.id}')">Reject</button></td></tr>`).join('')||'<tr><td colspan="5">No provider posts waiting approval.</td></tr>'}</table><h4>Customer Requests Waiting Approval</h4><table><tr><th>Customer</th><th>Provider</th><th>Category</th><th>Job</th><th>Status</th><th>Action</th></tr>${pendingRequests.map(r=>`<tr><td>${r.customerName}<br><span class="small">${r.customerEmail}</span></td><td>${r.providerName}<br><span class="small">${r.providerEmail}</span></td><td>${r.category}</td><td>${r.jobDescription}</td><td>${homeServiceStatusBadge(r.status)}</td><td><button class="btn primary" onclick="adminApproveHomeServiceRequest('${r.id}')">Approve to Provider</button> <button class="btn bad" onclick="adminRejectHomeServiceRequest('${r.id}')">Reject</button></td></tr>`).join('')||'<tr><td colspan="6">No customer requests waiting approval.</td></tr>'}</table>`:'';
 let myProviderTable=`<h3 id="homeServiceMyPosts">📢 My Service Posts</h3><table><tr><th>Business</th><th>Category</th><th>Area</th><th>Status</th></tr>${providerMine.map(p=>`<tr><td>${p.businessName||p.providerName}</td><td>${p.category}</td><td>${p.serviceArea}</td><td>${homeServiceStatusBadge(p.status)}</td></tr>`).join('')||'<tr><td colspan="4">No service posts yet.</td></tr>'}</table>`;
 let providerRequestsTable=`<h3 id="homeServiceRequestsTop">📩 Service Requests Sent to Me</h3><table><tr><th>Customer</th><th>Phone</th><th>Category</th><th>Job</th><th>Status</th><th>Action</th></tr>${providerReqs.map(r=>`<tr><td>${r.customerName}<br><span class="small">${r.customerEmail}</span></td><td>${r.customerPhone}</td><td>${r.category}</td><td>${r.jobDescription}<br><span class="small">${r.serviceAddress||''} ${r.preferredDate||''}</span></td><td>${homeServiceStatusBadge(r.status)}</td><td>${r.status==='Requested'?`<button class="btn primary" onclick="acceptHomeServiceRequest('${r.id}')">Accept</button> <button class="btn bad" onclick="declineHomeServiceRequest('${r.id}')">Decline</button>`:''}${r.status==='Accepted'?`<button class="btn ghost" onclick="startHomeServiceRequest('${r.id}')">In Progress</button>`:''}${r.status==='In Progress'?`<button class="btn primary" onclick="completeHomeServiceRequest('${r.id}')">Complete</button>`:''}</td></tr>`).join('')||'<tr><td colspan="6">No service requests sent to you yet.</td></tr>'}</table>`;
 let myReqTable=`<h3>My Service Requests</h3><table><tr><th>Provider</th><th>Category</th><th>Job</th><th>Status</th></tr>${myCustomerReqs.map(r=>`<tr><td>${r.providerName}<br><span class="small">${r.providerEmail}</span></td><td>${r.category}</td><td>${r.jobDescription}<br><span class="small">${r.preferredDate||''}</span></td><td>${homeServiceStatusBadge(r.status)}</td></tr>`).join('')||'<tr><td colspan="4">No service requests yet.</td></tr>'}</table>`;
 $('home_services').innerHTML=`<h2>🔧 Home Services</h2>${topActions}${pendingTable}${myProviderTable}${providerRequestsTable}${myReqTable}<h3 style="margin-top:22px">Available Home Service Providers</h3><div class="grid">${providerCards}</div><div class="grid" style="margin-top:22px">${providerForm}<div class="card"><h3>How it works</h3><p>1. Provider submits profile.</p><p>2. Admin approves provider.</p><p>3. Customer requests service / quote.</p><p>4. Provider accepts, starts, and completes the job.</p><button class="btn ghost" onclick="show('messages')">Open Messages</button></div></div>`;
}
function openHomeServiceRequest(id){
 let p=(data.homeServiceProviders||[]).find(x=>x.id===id);if(!p)return alert('Provider not found.');
 let box=document.createElement('div');box.className='card';box.style.marginTop='18px';
 box.innerHTML=`<h3>Request Service from ${p.businessName||p.providerName}</h3><label>Phone</label><input id="hsReqPhone" value="${currentUser.phone||''}" placeholder="Your phone"><label>Service Address / Area</label><input id="hsReqAddress" placeholder="City or address"><label>Preferred Date</label><input id="hsReqDate" type="date"><label>Describe the job</label><textarea id="hsReqDesc" placeholder="Explain what you need"></textarea><button class="btn primary" onclick="submitHomeServiceRequest('${p.id}')">Send Request</button>`;
 $('home_services').prepend(box);box.scrollIntoView({behavior:'smooth'});
}
async function postHomeServiceProvider(){
 if(!requireLogin())return;
 let businessName=($('hsBusiness').value||'').trim(),category=$('hsCategory').value,serviceArea=($('hsArea').value||'').trim(),phone=($('hsPhone').value||'').trim();
 ...
 data.homeServiceProviders.unshift(item);addNote('admin@habeshaconnect.com','Home service provider waiting for approval: '+businessName+'.');let emailRes=await sendHomeServicesAdminApprovalEmail(item);save();alert(emailRes&&emailRes.ok?'Provider profile submitted. Admin approval is required before customers see it. Admin was notified by email.':'Provider profile submitted. Admin approval is required. The admin in-app notification was saved, but email did not send. Check RESEND_API_KEY and Admin Notification Email setting.');show('home_services');
}
async function submitHomeServiceRequest(id){
 ...
 let req={id:'HSR'+Date.now().toString().slice(-6),providerId:p.id,providerDbId:p.dbId||'',providerName:p.businessName||p.providerName,providerEmail:p.providerEmail,customerName:currentUser.name,customerEmail:currentUser.email,customerPhone:phone,category:p.category,serviceAddress:($('hsReqAddress').value||'').trim(),preferredDate:($('hsReqDate').value||'').trim(),jobDescription:job,quoteAmount:'',status:'Pending Admin Approval',createdAt:new Date().toLocaleString()};
 ...
 data.homeServiceRequests.unshift(req);addNote('admin@habeshaconnect.com','Home service request waiting for approval: '+currentUser.name+' requested '+p.category+'.');let emailRes=await sendHomeServicesAdminRequestEmail(req);save();alert(emailRes&&emailRes.ok?'Service request submitted. Admin approval is required before the provider can see it. Admin was notified by email.':'Service request submitted. Admin approval is required before the provider can see it. The admin in-app notification was saved, but email did not send. Check RESEND_API_KEY and Admin Notification Email setting.');show('home_services');
}
...
function declineHomeServiceRequest(id){setHomeServiceRequestStatus(id,'Declined')}
function startHomeServiceRequest(id){setHomeServiceRequestStatus(id,'In Progress')}
function completeHomeServiceRequest(id){setHomeServiceRequestStatus(id,'Completed')}
async function adminApproveHomeServiceRequest(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=(data.homeServiceRequests||[]).find(x=>x.id===id);if(!r)return alert('Request not found.');r.status='Requested';let res=await updateHomeServiceRequestDb(r,{status:'Requested'});if(res.error)return alert('Could not approve request: '+res.error.message);addNote(r.providerEmail,'New admin-approved home service request from '+r.customerName+' for '+r.category+'.');sendEmailNotice({to:r.providerEmail,name:r.providerName,subject:'New Home Service Request Approved',summary:'Admin approved a customer request. You can now review and accept or decline it.',buttonText:'Open Home Services',page:'home_services',details:{Customer:r.customerName,Phone:r.customerPhone,Category:r.category,Job:r.jobDescription}});addNote(r.customerEmail,'Your home service request was approved by admin and sent to the provider.');sendEmailNotice({to:r.customerEmail,name:r.customerName,subject:'Home service request approved',summary:'Admin approved your service request and sent it to the provider.',buttonText:'Open Home Services',page:'home_services',details:{Provider:r.providerName,Category:r.category,Status:'Sent to provider'}});persistOnly();adminSuccess('Home service request approved and sent to provider.');}
async function adminRejectHomeServiceRequest(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let r=(data.homeServiceRequests||[]).find(x=>x.id===id);if(!r)return alert('Request not found.');r.status='Rejected';let res=await updateHomeServiceRequestDb(r,{status:'Rejected'});if(res.error)return alert('Could not reject request: '+res.error.message);addNote(r.customerEmail,'Your home service request was not approved by admin.');sendEmailNotice({to:r.customerEmail,name:r.customerName,subject:'Home service request not approved',summary:'Your home service request was not approved by admin.',buttonText:'Open Home Services',page:'home_services',details:{Provider:r.providerName,Category:r.category,Status:'Rejected'}});persistOnly();adminSuccess('Home service request rejected.');}
async function adminApproveHomeServiceProvider(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let p=(data.homeServiceProviders||[]).find(x=>x.id===id);if(!p)return alert('Provider not found.');p.status='Approved';let res=await updateHomeServiceProviderDb(p,{status:'Approved',approved_at:new Date().toISOString()});if(res.error)return alert('Could not approve provider: '+res.error.message);addNote(p.providerEmail,'Your Home Services provider profile was approved.');sendEmailNotice({to:p.providerEmail,name:p.providerName,subject:'Home Services profile approved',summary:'Your provider profile was approved and is now visible to customers.',buttonText:'Open Home Services',page:'home_services',details:{Business:p.businessName,Category:p.category,Status:'Approved'}});persistOnly();adminSuccess('Home service provider approved.');}
async function adminRejectHomeServiceProvider(id){if(!currentUser||currentUser.role!=='admin')return alert('Admin only.');let p=(data.homeServiceProviders||[]).find(x=>x.id===id);if(!p)return alert('Provider not found.');p.status='Rejected';let res=await updateHomeServiceProviderDb(p,{status:'Rejected'});if(res.error)return alert('Could not reject provider: '+res.error.message);persistOnly();adminSuccess('Home service provider rejected.');}
function homeServicesAdminHtml(){ensureArrays();let providers=data.homeServiceProviders||[],reqs=data.homeServiceRequests||[],pending=providers.filter(p=>p.status==='Pending Admin Approval'),pendingReqs=reqs.filter(r=>r.status==='Pending Admin Approval');return `<h3>🔧 Home Services Management</h3><div class="notice"><b>Flow:</b> Provider posts service → Admin approves → Customer sees it and requests service → Admin approves request → Provider sees the request.</div><div class="grid"><div class="card"><p>Waiting Approvals</p><div class="stat">${pending.length+pendingReqs.length}</div></div><div class="card"><p>Service Posts</p><div class="stat">${providers.length}</div></div><div class="card"><p>Service Requests</p><div class="stat">${reqs.length}</div></div></div><h3>⏳ Provider Posts Waiting Approval</h3><table><tr><th>Provider</th><th>Category</th><th>Area</th><th>Status</th><th>Action</th></tr>${pending.map(p=>`<tr><td>${p.businessName||p.providerName}<br><span class="small">${p.providerEmail}</span></td><td>${p.category}</td><td>${p.serviceArea}</td><td>${homeServiceStatusBadge(p.status)}</td><td><button class="btn primary" onclick="adminApproveHomeServiceProvider('${p.id}')">Approve</button> <button class="btn bad" onclick="adminRejectHomeServiceProvider('${p.id}')">Reject</button></td></tr>`).join('')||'<tr><td colspan="5">No provider posts waiting approval.</td></tr>'}</table><h3>📩 Customer Requests Waiting Approval</h3><table><tr><th>Customer</th><th>Provider</th><th>Category</th><th>Job</th><th>Status</th><th>Action</th></tr>${pendingReqs.map(r=>`<tr><td>${r.customerName}<br><span class="small">${r.customerEmail}</span></td><td>${r.providerName}<br><span class="small">${r.providerEmail}</span></td><td>${r.category}</td><td>${r.jobDescription}</td><td>${homeServiceStatusBadge(r.status)}</td><td><button class="btn primary" onclick="adminApproveHomeServiceRequest('${r.id}')">Approve to Provider</button> <button class="btn bad" onclick="adminRejectHomeServiceRequest('${r.id}')">Reject</button></td></tr>`).join('')||'<tr><td colspan="6">No customer requests waiting approval.</td></tr>'}</table><h3>📢 Service Posts</h3><table><tr><th>Provider</th><th>Category</th><th>Area</th><th>Status</th><th>Action</th></tr>${providers.map(p=>`<tr><td>${p.businessName||p.providerName}<br><span class="small">${p.providerEmail}</span></td><td>${p.category}</td><td>${p.serviceArea}</td><td>${homeServiceStatusBadge(p.status)}</td><td>${p.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveHomeServiceProvider('${p.id}')">Approve</button> <button class="btn bad" onclick="adminRejectHomeServiceProvider('${p.id}')">Reject</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="5">No providers yet.</td></tr>'}</table><h3>📩 Service Requests</h3><table><tr><th>Customer</th><th>Provider</th><th>Category</th><th>Job</th><th>Status</th><th>Action</th></tr>${reqs.map(r=>`<tr><td>${r.customerName}<br><span class="small">${r.customerEmail}</span></td><td>${r.providerName}<br><span class="small">${r.providerEmail}</span></td><td>${r.category}</td><td>${r.jobDescription}</td><td>${homeServiceStatusBadge(r.status)}</td><td>${r.status==='Pending Admin Approval'?`<button class="btn primary" onclick="adminApproveHomeServiceRequest('${r.id}')">Approve to Provider</button> <button class="btn bad" onclick="adminRejectHomeServiceRequest('${r.id}')">Reject</button>`:'-'}</td></tr>`).join('')||'<tr><td colspan="6">No home service requests yet.</td></tr>'}</table>`;}


function business(){$('business').innerHTML=`<h2>📊 Business Manager</h2><div class="grid"><div class="card"><h3>Add Business Report</h3><input id="bName" placeholder="Business name"><input id="bSales" type="number" placeholder="Sales"><input id="bExpenses" type="number" placeholder="Expenses"><select id="bPeriod"><option>This Month</option><option>This Quarter</option><option>This Year</option></select><button class="btn primary" onclick="addBusiness()">Save Report</button></div><div class="card"><h3>Tax Ready Reports</h3><p>Track sales, expenses, payroll, inventory, invoices, estimates, profit & loss, cash flow, and year-end summaries.</p></div></div><table><tr><th>Business</th><th>Period</th><th>Sales</th><th>Expenses</th><th>Profit</th></tr>${data.businesses.map(b=>`<tr><td>${b.name}</td><td>${b.period}</td><td>${money(b.sales)}</td><td>${money(b.expenses)}</td><td>${money(b.sales-b.expenses)}</td></tr>`).join('')}</table>`}
function addBusiness(){if(!requireLogin())return;data.businesses.unshift({id:'B'+Date.now().toString().slice(-5),name:$('bName').value||'Business',sales:+$('bSales').value||0,expenses:+$('bExpenses').value||0,period:$('bPeriod').value});save()}
...
function terms(){
 $('terms').innerHTML=`<h2>Terms of Service</h2><div class="card"><p><b>Effective:</b> Beta Version</p><p>Habesha Connect is a community marketplace platform. We connect users for shipping, rentals, trucking, jobs, marketplace, and business services. Users are responsible for verifying people, listings, prices, pickup/delivery details, property details, driver/truck details, and any agreement before completing a transaction.</p><p><b>Shipping:</b> Senders and travelers must agree on package details, luggage/airline requirements, prohibited items, pickup, delivery, and payment before moving any item. Do not ship illegal, dangerous, restricted, or prohibited items.</p><p><b>Rentals:</b> Owners are responsible for accurate property information and legal permission to list. Seekers are responsible for verifying the property and agreement before paying rent or deposit.</p><p><b>Trucking:</b> Truck owners and drivers are responsible for licenses, insurance, registration, safety, job terms, and legal compliance.</p><p><b>Accounts:</b> Habesha Connect may suspend or remove accounts, listings, or posts that appear unsafe, false, abusive, illegal, or harmful to the community.</p><p><b>Beta:</b> The app is still being tested and may contain errors. Use at your own risk and report problems.</p><p><b>Limitation:</b> Habesha Connect is not responsible for user agreements, losses, damages, inaccurate listings, missed deliveries, rental disputes, job disputes, or third-party actions.</p><div class="actions"><button class="btn ghost" onclick="show('privacy')">View Privacy</button><button class="btn primary" onclick="show('account')">Back to Account</button></div></div>`;
}
...
async function admin(){
 ...
 let managedRoles=['sender','traveler','owner','rent_seeker','truck_owner','customer','driver','business_owner','admin'];
 ...
 $('admin').innerHTML=`<h2>⚙️ Admin Verification Center</h2><div class='notice'><b>V6.3.5 Admin One-Click Refresh Fix:</b> admin buttons now show Working, prevent duplicate clicks, and refresh the admin tables after each action.</div>
...
async function markDelivered(id){
  ...
  const tracking=s.tracking||id;
  const deliveredMsg='Your shipment '+tracking+' was marked delivered.';
  addNote('admin@habeshaconnect.com','Shipment '+tracking+' was marked delivered. The related traveler trip is removed from sender availability.');
  if(s.senderEmail){
    addNote(s.senderEmail,deliveredMsg);
    await sendEmailNotice({to:s.senderEmail,name:s.sender||'Sender',subject:'Your item was delivered',summary:deliveredMsg,buttonText:'Open Shipping Dashboard',page:'shipping',details:{Tracking:tracking,Status:'Delivered',Traveler:s.traveler||''}});
  }
  if(s.travelerEmail){
    addNote(s.travelerEmail,'Shipment '+tracking+' was marked delivered.');
    await sendEmailNotice({to:s.travelerEmail,name:s.traveler||'Traveler',subject:'Shipment marked delivered',summary:'Shipment '+tracking+' was marked delivered.',buttonText:'Open Shipping Dashboard',page:'shipping',details:{Tracking:tracking,Status:'Delivered',Sender:s.sender||''}});
  }
  persistOnly(); await refreshAdminData(); adminSuccess();
}
...
async function deleteRental(id){
  ...
  data.rentals=data.rentals.filter(x=>String(x.id)!==String(id)&&String(x.dbId)!==String(id)); data.rentalRequests=data.rentalRequests.filter(x=>x.rentalId!==id);
  persistOnly(); adminSuccess();
}
...
const HC_I18N={
  am:{
    'Home':'መነሻ','Account':'መለያ','Profile':'መገለጫ','My Services':'የእኔ አገልግሎቶች','Shipping':'ጭነት','Rentals':'ቤት ኪራይ','Marketplace':'ገበያ','Jobs':'ስራዎች','Trucking':'የጭነት መኪና','Business Manager':'የንግድ አስተዳደር','Messages':'መልዕክቶች','Notifications':'ማሳወቂያዎች','Admin':'አስተዳዳሪ',
    ...
    'Traveler':'ተጓዥ','Sender':'ላኪ','Property Owner':'የቤት ባለቤት','Rent Seeker':'ቤት ፈላጊ','Truck Driver':'የጭነት መኪና አሽከርካሪ','Business Owner':'የንግድ ባለቤት','Customer':'ደንበኛ','Verified':'ተረጋግጧል','Not Verified':'አልተረጋገጠም','Pending':'በመጠባበቅ ላይ','Approved':'ጸድቋል','Paid':'ተከፍሏል','Available':'ይገኛል','Rented':'ተከራይቷል',
