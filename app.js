...
function home(){$('home').innerHTML=`<div class="hero"><h1>Habesha Connect</h1><p><b>Connecting the Ethiopian community through shipping, rentals, jobs, marketplace, services, and business tools.</b></p><p>V7.0.0 adds English and Amharic language support while keeping Shipping and Rentals stable.</p>${roleWelcome()}<div class="actions">${currentUser?`<button class="btn primary" onclick="show('profile')">Open My Dashboard</button>`:`<button class="btn primary" onclick="show('account')">Create Account</button>`}${isAllowedPage('shipping')?`<button class="btn dark" onclick="show('shipping')">Start Shipping</button>`:''}${isAllowedPage('rentals')?`<button class="btn" onclick="show('rentals')">Find Rentals</button>`:''}${isAllowedPage('admin')?`<button class="btn ghost" onclick="show('admin')">Admin Dashboard</button>`:''}</div></div><h2 style="margin-top:22px">Available Services</h2><div class="grid">${roleServices()}</div>`}
...
async function rentals(){
 ...
 let seekerStats=rentalStatsFor(data.rentals.filter(r=>data.rentalRequests.some(q=>q.seekerEmail===(currentUser&&currentUser.email)&&q.rentalId===r.id)),seekerReqs);
 let adminStats=rentalStatsFor(data.rentals,data.rentalRequests);
 let ownerSummary=role==='owner'?rentalStatsCards('🏠 My Rental Performance',ownerStats):'';
 let seekerSummary=(role==='rent_seeker'||role==='customer')?rentalStatsCards('🔍 My Rental Activity',seekerStats):'';
 let adminRentalSummary=role==='admin'?rentalStatsCards('📊 Rental Statistics',adminStats):'';
