...
function home(){$('home').innerHTML=`<div class="hero"><h1>Habesha Connect</h1><p><b>Connecting the Ethiopian community through shipping, rentals, jobs, marketplace, services, and business tools.</b></p><p>V6.8.0 adds rental analytics, friendly property IDs, owner/seeker activity summaries, and improved rental management.</p>${roleWelcome()}<div class="actions">${currentUser?`<button class="btn primary" onclick="show('profile')">Open My Dashboard</button>`:`<button class="btn primary" onclick="show('account')">Create Account</button>`}${isAllowedPage('shipping')?`<button class="btn dark" onclick="show('shipping')">Start Shipping</button>`:''}${isAllowedPage('rentals')?`<button class="btn" onclick="show('rentals')">Find Rentals</button>`:''}${isAllowedPage('admin')?`<button class="btn ghost" onclick="show('admin')">Admin Dashboard</button>`:''}</div></div><h2 style="margin-top:22px">Available Services</h2><div class="grid">${roleServices()}</div>`}
...
async function profile(){
 ...
 let payments=data.payments.filter(p=>p.user===currentUser.email);
...
async function shipping(){
 ...
 let terminalStatuses=['Completed','Delivered'];let shipments=data.shipments.filter(s=>{
   if(role==='admin')return true;
   if(terminalStatuses.includes(s.status||''))return false;
...
function shipCard(s,role){
 ...
 let approvedContact=['Approved','Ready for Delivery','In Transit'].includes(s.status||'');let travelerPayoutDetails=`<p><b>Traveler commission:</b> ${money(payoutAmt)} <span class="muted">(${Number(s.weight||0)} lb × ${money(settings().travelerCommissionPerLb)}/lb)</span></p><p><b>Trip listing fee refund:</b> ${money(listingRefund)}</p><p><b>Total traveler payout:</b> ${money(totalPayout)} ${s.travelerPaid?'<span class="pill good">Traveler Paid</span>':'<span class="pill warn">Not Paid Yet</span>'}</p>`;let travelerInfo = `<p><b>Tracking:</b> ${s.tracking||''}</p><div class="photoStrip">${photos}</div><p><b>Weight:</b> ${s.weight} lb</p>${travelerPayoutDetails}${verifiedBadge(s.senderVerified)} ${approvedContact?contactLine('Sender',s.sender,s.senderPhone,'')+contactLine('Receiver',s.receiver,s.receiverPhone,''):'<p class="muted">Sender and receiver contact will show after payment is approved by admin.</p>'}`;
...
async function requestTrip(id){
 ...
 let w=+($('req_'+id).value)||0;if(w<=0)return alert('Enter requested pounds.');if(w>Number(t.availableSpace||0))return alert('Only '+t.availableSpace+' lb is available.');
 ...
 if(authReady()&&currentUser.id&&t.dbId){
   const payload={
     ...
     traveler_id:t.travelerId||null,
     trip_id:t.dbId,
     from_city:t.fromCity||((t.route||'').split('→')[0]||'Atlanta, GA').trim(),
     to_city:t.toCity||((t.route||'').split('→')[1]||'Addis Ababa').trim(),
     weight_lb:w,
     sender_name:cleanSenderName,
     sender_phone:cleanSenderPhone,
     receiver_name:cleanReceiver,
     receiver_phone:cleanReceiverPhone,
     package_description:cleanDesc,
     paid:false,
     status:'Requested'
   };
   ...
   if(inserted.error){
     ...
     if(msg.toLowerCase().includes('sender_name')||msg.toLowerCase().includes('sender_phone')||msg.toLowerCase().includes('package_description')){
       return alert('Please run the V6.7.2 sender shipment SQL first, then try again. Missing shipment sender/package columns.');
     }
     return alert('Could not save request to Supabase: '+inserted.error.message);
   }
   let row=inserted.data;
   row.sender={name:cleanSenderName,email:currentUser.email,phone:cleanSenderPhone,verified:!!currentUser.verified};
   ...
   addNote(t.travelerEmail||'all','New '+w+' lb space request from '+cleanSenderName+'.');
 ...
 data.shipments.unshift({id:sid,route:t.route,weight:w,fee:w*settings().shippingRatePerLb,status:'Requested',tracking:'HC-'+sid,packagePhotos:[],sender:cleanSenderName,senderPhone:cleanSenderPhone,senderEmail:currentUser.email,senderId:currentUser.id||'',senderVerified:!!currentUser.verified,receiver:cleanReceiver,receiverPhone:cleanReceiverPhone,traveler:t.traveler,travelerPhone:t.travelerPhone,travelerEmail:t.travelerEmail,travelerId:t.travelerId||'',travelerVerified:t.travelerVerified,tripId:t.id,appFee:w*settings().appCommissionPerLb,paid:false,paymentStatus:'Unpaid'});
 addNote(t.travelerEmail,'New '+w+' lb space request from '+cleanSenderName+'.');addNote(currentUser.email,'Space request sent to '+t.traveler+'.');save();shipping()}
...
function mapDbRentalRequest(row){
  let prop=row.property||{}; let seeker=row.seeker||{};
  let r=data.rentals.find(x=>String(x.dbId||x.id)===String(row.property_id));
  return {id:row.id,rentalId:row.property_id,propertyTitle:(r&&r.title)||prop.title||'Rental Property',seekerName:seeker.name||row.seeker_name||'Rent Seeker',seekerPhone:seeker.phone||row.seeker_phone||'',seekerEmail:seeker.email||row.seeker_email||'',ownerName:(r&&r.owner)||prop.owner_name||'Owner',ownerPhone:(r&&r.ownerPhone)||prop.owner_phone||'',ownerEmail:(r&&r.ownerEmail)||prop.owner_email||'',status:row.status||'Pending',paymentStatus:row.paid?'Paid':'Pending',amountPaid:Number(row.amount_paid||settings().seekerViewingFee||0),paid:!!row.paid,time:row.created_at||'',dbId:row.id,source:'supabase'};
}
async function loadSupabaseRentalRequests(){
  if(!authReady())return;
  let {data:rows,error}=await hcSupabase.from('rental_requests').select('*, property:property_id(*), seeker:seeker_id(name,email,phone)').order('created_at',{ascending:false});
  if(error){console.warn('Rental requests relationship load error, retrying basic select',error);let retry=await hcSupabase.from('rental_requests').select('*').order('created_at',{ascending:false});rows=retry.data||[];error=retry.error;}
  if(error){console.warn('Rental requests load error',error);return;}
  let dbReqs=(rows||[]).map(mapDbRentalRequest);
  let localOnly=(data.rentalRequests||[]).filter(x=>x.source!=='supabase'&&!x.dbId);
  data.rentalRequests=[...dbReqs,...localOnly];
}
...
async function rentals(){
 ...
 let ownerSummary=role==='owner'?rentalStatsCards('🏠 My Rental Performance',rentalStatsFor(myRentals,ownerReqs)):'';
 let seekerSummary=role==='rent_seeker'?rentalStatsCards('🔍 My Rental Activity',rentalStatsFor([],seekerReqs)):'';
 let adminRentalSummary=role==='admin'?rentalStatsCards('📊 Rental Statistics',rentalStatsFor(data.rentals,data.rentalRequests)):'';
...
async function addRental(){
  ...
  let fee=settings().ownerListingFee;
  if(!confirm('Publishing this property costs '+money(fee)+'. Continue to pay and publish?'))return;
  pay('Rentals',fee,'Owner listing fee for '+title);
  let localRental={id:'R'+Date.now().toString().slice(-5),title,propertyType,city,price,deposit,bed,bath,moveDate:$('rentMoveDate').value,leaseTerm:$('rentLease').value,pets:$('rentPets').value,parking:$('rentParking').value,furnished:$('rentFurnished').value,utilities:$('rentUtilities').value||'Ask owner',photos:photoNamesFromInput('rentPhotos'),owner:currentUser.name,ownerPhone:currentUser.phone||'',ownerEmail:currentUser.email,ownerId:currentUser.id||'',status:(currentUser.role==='admin'?'Approved':'Pending'),appFee:settings().seekerViewingFee,ownerFee:fee,ownerPaid:true};
  if(authReady()){
  ...
  addNote('admin@habeshaconnect.com','Paid rental listing from '+currentUser.name+' needs approval.');
  addNote(currentUser.email,'Payment received. Your property was submitted and is waiting for admin approval.');
  save();
...
async function payPublishRental(id){
  ...
  if(r.ownerPaid){alert('This listing fee is already paid.');return;}
  pay('Rentals',r.ownerFee||settings().ownerListingFee,'Owner listing fee for '+r.title);
...
async function requestViewing(id){
  ...
  let existing=data.rentalRequests.find(q=>String(q.rentalId)===String(r.id)&&q.seekerEmail===currentUser.email);
  if(existing){alert('You already paid and sent a viewing request for this property.');rentals();return;}
  let fee=r.appFee||settings().seekerViewingFee;
  pay('Rentals',fee,'Viewing request '+friendlyRentalId(r));
  let localReq={id:'RQ'+Date.now().toString().slice(-5),rentalId:r.id,propertyTitle:r.title,seekerName:currentUser.name,seekerPhone:currentUser.phone||'',seekerEmail:currentUser.email,ownerName:r.owner,ownerPhone:r.ownerPhone||'',ownerEmail:r.ownerEmail||'',status:'Pending',paymentStatus:'Paid',amountPaid:fee,paid:true,time:new Date().toLocaleString()};
  if(authReady()&&r.dbId){
    let payload={property_id:r.dbId,seeker_id:currentUser.id||null,paid:true,status:'Pending'};
    let res=await hcSupabase.from('rental_requests').insert(payload).select('*').single();
    if(res.error){
      if(String(res.error.message||'').toLowerCase().includes('duplicate'))return alert('You already sent a viewing request for this property.');
      return alert('Viewing payment was recorded, but the request could not be saved to Supabase: '+res.error.message);
    }
    localReq.dbId=res.data.id;localReq.id=res.data.id;localReq.rentalId=r.dbId;localReq.source='supabase';
  }
  data.rentalRequests.unshift(localReq);
  r.status='Viewing Pending';
  addNote(r.ownerEmail||'all','New paid rental viewing request from '+currentUser.name+'.');
  addNote(currentUser.email,'Payment received. Your viewing request was sent to the owner.');
  save();
  await rentals();
}
...
async function approveRentalReq(id){let q=data.rentalRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!q)return;q.status='Approved';let r=data.rentals.find(x=>String(x.id)===String(q.rentalId)||String(x.dbId)===String(q.rentalId));if(r)r.status='Rented';let res={error:null};if(authReady()&&q.dbId)res=await hcSupabase.from('rental_requests').update({status:'Approved',paid:true}).eq('id',q.dbId);if(res.error)return alert('Could not approve rental request in Supabase: '+res.error.message);if(authReady()&&r&&r.dbId){await hcSupabase.from('properties').update({status:'Rented'}).eq('id',r.dbId);}addNote(q.seekerEmail,'Your rental request was approved. Owner contact is now available.');persistOnly();await rentals();}
async function declineRentalReq(id){let q=data.rentalRequests.find(x=>String(x.id)===String(id)||String(x.dbId)===String(id));if(!q)return;q.status='Declined';let r=data.rentals.find(x=>String(x.id)===String(q.rentalId)||String(x.dbId)===String(q.rentalId));if(r&&r.status==='Viewing Pending')r.status='Approved';let res={error:null};if(authReady()&&q.dbId)res=await hcSupabase.from('rental_requests').update({status:'Declined'}).eq('id',q.dbId);if(res.error)return alert('Could not decline rental request in Supabase: '+res.error.message);addNote(q.seekerEmail,'Your rental request was declined.');persistOnly();await rentals();}
...
async function admin(){
 ...
 let rentalStats=rentalStatsFor(data.rentals,data.rentalRequests);
 let rentalStatsHtml=rentalStatsCards('🏠 Rental Statistics',rentalStats);
