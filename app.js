...
function shipCard(s,role){
 ...
 let adminInfo = `<p><b>Tracking:</b> ${s.tracking||''}</p><div class="photoStrip">${photos}</div><p>${s.weight} lb • Sender pays ${money(s.weight*settings().shippingRatePerLb)} • App fee ${money(s.appFee)} • Traveler commission ${money(s.weight*settings().travelerCommissionPerLb)}</p>${contactLine('Sender',s.sender,s.senderPhone,s.senderEmail)} ${verifiedBadge(s.senderVerified)}${contactLine('Receiver',s.receiver,s.receiverPhone,'')}${contactLine('Traveler',s.traveler,s.travelerPhone,s.travelerEmail)} ${s.travelerEmail?verifiedBadge(s.travelerVerified):''}`;
 ...
 else if(role==='admin') actions=`<button class="btn primary" onclick="approveShipment('${s.id}')">Approve Payment</button><button class="btn ghost" onclick="markDelivered('${s.id}')">Mark Delivered</button><button class="btn bad" onclick="declineShip('${s.id}')">Decline</button>`;
...
async function admin(){
 ...
 <h3>Shipping Management</h3><table><tr><th>ID</th><th>Tracking</th><th>Route</th><th>Sender</th><th>Traveler</th><th>Status</th><th>Payment</th><th>Actions</th></tr>${data.shipments.map(s=>`<tr><td>${s.id}</td><td>${s.tracking||''}</td><td>${s.route}</td><td>${s.sender} ${verifiedBadge(s.senderVerified)}</td><td>${s.traveler} ${s.travelerEmail?verifiedBadge(s.travelerVerified):''}</td><td>${s.status}</td><td>${s.paid?'Paid':'Unpaid'}</td><td><button class="btn primary" onclick="approveShipment('${s.id}')">Approve Payment</button> <button class="btn ghost" onclick="markDelivered('${s.id}')">Mark Delivered</button> <button class="btn ghost" onclick="editShipment('${s.id}')">Edit</button> <button class="btn bad" onclick="deleteShipment('${s.id}')">Delete</button></td></tr>`).join('')}</table>
